...
impl ArchivalWorker {
    pub async fn new(config: ArchivalConfig) -> Result<Self> {
        ...
        let state = AccumulatedState {
            ...
            checkpoint_range: manifest.next_checkpoint_seq_num()..manifest.next_checkpoint_seq_num(),
            buffer: vec![],
            summary_buffer: vec![],
            last_commit_ms: 0,
    ...
    async fn upload(&self, state: &AccumulatedState) -> Result<()> {
        let checkpoint_file_path = format!("epoch_{}/{}.chk", state.epoch, state.checkpoint_range.start);
        let chk_bytes =
            self.upload_file(Path::from(checkpoint_file_path.clone()), CHECKPOINT_FILE_MAGIC, &state.buffer).await?;
        let summary_file_path = format!("epoch_{}/{}.sum", state.epoch, state.checkpoint_range.start);
        let sum_bytes =
            self.upload_file(Path::from(summary_file_path.clone()), SUMMARY_FILE_MAGIC, &state.summary_buffer).await?;
        ...
        manifest.update(state.epoch, state.checkpoint_range.end, checkpoint_file_metadata, summary_file_metadata);
    ...
    async fn process_checkpoint(&self, checkpoint: &CheckpointData) -> Result<()> {
        ...
        if !state.buffer.is_empty()
            && (((state.buffer.len() + blob_size) > self.commit_file_size)
                || state.epoch != epoch
                || checkpoint.checkpoint_summary.timestamp_ms > (self.commit_duration_ms + state.last_commit_ms))
        {

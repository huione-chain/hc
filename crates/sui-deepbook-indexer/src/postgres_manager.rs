        ...
        .transaction(|conn| {
            async move {
                ...
                diesel::insert_into(order_fills::table)
                    ...
                    .execute(conn)
                    .await?;
                diesel::insert_into(flashloans::table).values(&flahloans).on_conflict_do_nothing().execute(conn).await?;
                diesel::insert_into(pool_prices::table)
                    .values(&pool_prices)
                    .on_conflict_do_nothing()
                    .execute(conn)
                    .await?;
                diesel::insert_into(balances::table).values(&balances).on_conflict_do_nothing().execute(conn).await?;
                diesel::insert_into(proposals::table).values(&proposals).on_conflict_do_nothing().execute(conn).await?;
                diesel::insert_into(rebates::table).values(&rebates).on_conflict_do_nothing().execute(conn).await?;
                diesel::insert_into(stakes::table).values(&stakes).on_conflict_do_nothing().execute(conn).await?;
                diesel::insert_into(trade_params_update::table)
                    .values(&trade_params_update)
                    .on_conflict_do_nothing()
                    .execute(conn)
                    .await?;
                diesel::insert_into(votes::table).values(&votes).on_conflict_do_nothing().execute(conn).await?;

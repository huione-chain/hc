...
impl LocalExec {
    ...
    pub async fn execution_engine_execute_with_tx_info_impl(
        ...
        if tx_info.sender_signed_data.transaction_data().is_system_tx() {
            warn!("System TX replay not supported: {}, skipping transaction", tx_digest);
...
impl ParentSync for LocalExec {
    ...
    fn get_latest_parent_entry_ref_deprecated(&self, object_id: ObjectID) -> SuiResult<Option<ObjectRef>> {
        ...
        self.exec_store_events
            .lock()
            .expect("Unable to lock events list")
            .push(ExecutionStoreEvent::ParentSyncStoreGetLatestParentEntryRef { object_id, result: res.clone() });
        res
    }
}

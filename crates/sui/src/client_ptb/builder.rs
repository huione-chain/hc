...
use move_core_types::{account_address::AccountAddress, annotated_value::MoveTypeLayout, ident_str};
...
impl<'a> Resolver<'a> for ToObject {
    async fn resolve_object_id(
        ...
        let owner = obj.owner.ok_or_else(|| err!(loc, "Unable to get owner info for object {obj_id}"))?;
...
impl<'a> PTBBuilder<'a> {
    ...
    async fn resolve_move_call_arg(
        ...
        // need to resolve it.
        if is_primitive {
            return self.resolve(loc.wrap(arg), ToPure::new_from_layout(layout.unwrap())).await;
        }

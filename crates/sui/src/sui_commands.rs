...
pub enum SuiCommand {
    ...
    /// Start HChain interactive console.
    #[clap(name = "console")]
    Console {
    ...
    /// Client for interacting with the HChain network.
    #[clap(name = "client")]
    Client {
        /// Sets the file storing the state of our user accounts (an empty one will be created if missing)
        #[clap(long = "client.config")]
    ...
    pub async fn execute(self) -> Result<(), anyhow::Error> {
        ...
        match self {
            ...
            SuiCommand::Genesis {
                ...
                genesis(from_config, write_config, working_dir, force, epoch_duration_ms, benchmark_ips, with_faucet)
                    .await
            }
...
async fn start(
    ...
    // generated each run.
    if force_regenesis {
        swarm_builder = swarm_builder.committee_size(NonZeroUsize::new(DEFAULT_NUMBER_OF_AUTHORITIES).unwrap());
        ...
        let network_config_path = if let Some(ref config) = config {
            ...
            } else if config.is_file() && config.extension().is_some_and(|ext| (ext == "yml" || ext == "yaml")) {
                config.clone()
            } else {
                config.join(SUI_NETWORK_CONFIG)
            }
        } else {
            config.clone().unwrap_or(sui_config_dir()?).join(SUI_NETWORK_CONFIG)
        };
        let network_config: NetworkConfig = PersistedConfig::read(&network_config_path)
            .map_err(|err| err.context(format!("Cannot open Sui network config file at {:?}", network_config_path)))?;

        swarm_builder = swarm_builder.dir(sui_config_dir()?).with_network_config(network_config);
    }

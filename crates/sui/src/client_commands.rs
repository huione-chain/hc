...
pub enum SuiClientCommands {
    ...
    /// Pay all residual HC coins to the recipient with input coins, after deducting the gas cost.
    ...
    /// Pay HC coins to recipients following following specified amounts, with input coins.
    /// Length of recipients must be the same as that of amounts.
    /// The input coins also include the coin for gas payment, so no extra gas coin is required.
    PayHc {
        /// The input coins to be used for pay recipients, including the gas coin.
    ...
    pub async fn execute(self, context: &mut WalletContext) -> Result<SuiClientCommandResult, anyhow::Error> {
        let ret = match self {
            ...
            SuiClientCommands::Upgrade {
                ...
                let (package_id, compiled_modules, dependencies, package_digest, upgrade_policy) = upgrade_result?;
            ...
            SuiClientCommands::TransferHc { to, sui_coin_object_id: object_id, amount, opts } => {
                let signer = context.get_object_owner(&object_id).await?;
                let to = get_identity_address(Some(to), context)?;
                let client = context.get_client().await?;
            ...
            SuiClientCommands::PayHc { input_coins, recipients, amounts, opts } => {
            ...
            SuiClientCommands::PayAllHc { input_coins, recipient, opts } => {
...
pub(crate) async fn upgrade_package(
    ...
    Ok((package_id, compiled_modules, dependencies, package_digest, upgrade_policy))
}
...
pub async fn request_tokens_from_faucet(address: SuiAddress, url: String) -> Result<(), anyhow::Error> {
    ...
    let resp = client.post(&url).header("Content-Type", "application/json").json(&json_body).send().await?;
...
pub(crate) async fn dry_run_or_execute_or_serialize(
    ...
    let (dry_run, gas_budget, serialize_unsigned_transaction, serialize_signed_transaction) =
        (opts.dry_run, opts.gas_budget, opts.serialize_unsigned_transaction, opts.serialize_signed_transaction);
    ensure!(

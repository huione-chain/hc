...
pub struct BridgeRequestMockHandler {
    ...
    sui_token_events: Arc<Mutex<HashMap<(TransactionDigest, u16), BridgeResult<SignedBridgeAction>>>>,
...
impl BridgeRequestMockHandler {
    ...
    pub fn add_sui_event_response(
        ...
        self.sui_token_events.lock().unwrap().insert((tx_digest, idx), response);
    }

...
impl BridgeTestClusterBuilder {
    ...
    pub async fn build(self) -> BridgeTestCluster {
        ...
        let start_cluster_task = tokio::task::spawn(Self::start_test_cluster(bridge_keys, self.num_validators));
        ...
        let bridge_client = SuiBridgeClient::new(&test_cluster.fullnode_handle.rpc_url, metrics).await.unwrap();
    ...
    async fn start_test_cluster(bridge_keys: Vec<BridgeAuthorityKeyPair>, num_validators: usize) -> TestCluster {
...
impl BridgeTestCluster {
    ...
    pub async fn test_transaction_builder_with_sender(&self, sender: SuiAddress) -> TestTransactionBuilder {
        self.test_cluster.test_transaction_builder_with_sender(sender).await
    }
    ...
    pub async fn sign_and_execute_transaction(&self, tx_data: &TransactionData) -> SuiTransactionBlockResponse {
        self.test_cluster.sign_and_execute_transaction(tx_data).await
    }
...
impl EthBridgeEnvironment {
    ...
    pub(crate) fn get_bridge_config(&self) -> EthBridgeConfig<ethers::prelude::Provider<ethers::providers::Http>> {
...
pub(crate) async fn start_bridge_cluster(
    ...
    let bridge_authority_keys =
        test_cluster.bridge_authority_keys.as_ref().unwrap().iter().map(|k| k.copy()).collect::<Vec<_>>();
...
pub(crate) async fn get_signatures(sui_bridge_client: &SuiBridgeClient, nonce: u64, sui_chain_id: u8) -> Vec<Bytes> {
    let sigs =

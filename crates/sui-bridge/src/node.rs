...
async fn start_client_components(
    ...
    let committee = Arc::new(sui_client.get_bridge_committee().await.expect("Failed to get committee"));
    let bridge_auth_agg = Arc::new(ArcSwap::from(Arc::new(BridgeAuthorityAggregator::new(committee))));
...
mod tests {
    ...
    async fn test_starting_bridge_node_with_client() {
        ...
        bridge_test_cluster.test_cluster.transfer_sui_must_exceed(sender_address, client_sui_address, 1000000000).await;

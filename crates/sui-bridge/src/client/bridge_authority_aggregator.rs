...
use sui_authority_aggregation::{quorum_map_then_reduce_with_timeout_and_prefs, ReduceOutput};
use sui_types::{
...
impl BridgeAuthorityAggregator {
    pub fn new(committee: Arc<BridgeCommittee>) -> Self {
        ...
        Self { committee, clients: Arc::new(clients) }
    }
    ...
    pub async fn request_committee_signatures(
        ...
        request_sign_bridge_action_into_certification(action, self.committee.clone(), self.clients.clone(), state).await
    }
}
...
impl GetSigsState {
    fn new(validity_threshold: StakeUnit, committee: Arc<BridgeCommittee>) -> Self {
        Self { committee, total_bad_stake: 0, total_ok_stake: 0, sigs: BTreeMap::new(), validity_threshold }
    }
...
mod tests {
    ...
    async fn test_bridge_auth_agg_ok() {
        ...
        mock0.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Ok(sign_action_with_key(&action, &secrets[0])));
        mock1.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Ok(sign_action_with_key(&action, &secrets[1])));
        mock2.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Ok(sign_action_with_key(&action, &secrets[2])));
        mock3.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Ok(sign_action_with_key(&action, &secrets[3])));
        agg.request_committee_signatures(action.clone()).await.unwrap();

        // 1 out of 4 authorities returns error
        mock3.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Err(BridgeError::RestAPIError("".into())));
        agg.request_committee_signatures(action.clone()).await.unwrap();

        // 2 out of 4 authorities returns error
        mock2.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Err(BridgeError::RestAPIError("".into())));
        agg.request_committee_signatures(action.clone()).await.unwrap();

        // 3 out of 4 authorities returns error - good stake below valdiity threshold
        mock1.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Err(BridgeError::RestAPIError("".into())));
    ...
    async fn test_bridge_auth_agg_more_cases() {
        ...
        mock3.add_sui_event_response(sui_tx_digest, sui_tx_event_index, Ok(sign_action_with_key(&action, &secrets[2])));
        let err = agg.request_committee_signatures(action.clone()).await.unwrap_err();
        assert!(matches!(err, BridgeError::AuthoritySignatureAggregationTooManyError(_)));
    }

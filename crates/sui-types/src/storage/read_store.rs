...
pub trait ReadStore: ObjectStore {
    ...
    fn get_checkpoint_contents_by_digest(&self, digest: &CheckpointContentsDigest)
    ...
    fn multi_get_transactions(&self, tx_digests: &[TransactionDigest]) -> Result<Vec<Option<Arc<VerifiedTransaction>>>> {
        tx_digests.iter().map(|digest| self.get_transaction(digest)).collect::<Result<Vec<_>, _>>()
    }
    ...
    fn multi_get_transaction_effects(
        ...
        tx_digests.iter().map(|digest| self.get_transaction_effects(digest)).collect::<Result<Vec<_>, _>>()
    }
    ...
    fn multi_get_events(&self, event_digests: &[TransactionEventsDigest]) -> Result<Vec<Option<TransactionEvents>>> {
        event_digests.iter().map(|digest| self.get_events(digest)).collect::<Result<Vec<_>, _>>()
    }

...
impl TryFrom<RawPasskeyAuthenticator> for PasskeyAuthenticator {
    ...
    fn try_from(raw: RawPasskeyAuthenticator) -> Result<Self, Self::Error> {
        ...
        let intent = Intent::from_bytes(&parsed_challenge[..INTENT_PREFIX_LENGTH])
            .map_err(|_| SuiError::InvalidSignature { error: "Invalid intent from challenge".to_string() })?;

        let digest = parsed_challenge[INTENT_PREFIX_LENGTH..]
            .try_into()
            .map_err(|_| SuiError::InvalidSignature { error: "Invalid digest from challenge".to_string() })?;
...
pub fn to_signing_digest<T: Serialize>(intent_msg: &IntentMessage<T>) -> [u8; DefaultHash::OUTPUT_SIZE] {
    let mut hasher = DefaultHash::default();
    bcs::serialize_into(&mut hasher, &intent_msg.value).expect("Message serialization should not fail");

...
mod test {
    ...
    use sui_benchmark::{
        ...
        workloads::{adversarial::AdversarialPayloadCfg, workload_configuration::WorkloadConfiguration},
        ...
        let _guard = ProtocolConfig::apply_overrides_for_testing(move |_, mut config| {
            ...
            match mode {
                ...
                PerObjectCongestionControlMode::TotalGasBudgetWithCap => {
                    ...
                    config.set_max_accumulated_txn_cost_per_object_in_narwhal_commit_for_testing(total_gas_limit);
                    config.set_max_accumulated_txn_cost_per_object_in_mysticeti_commit_for_testing(total_gas_limit);
                    config.set_gas_budget_based_txn_cost_cap_factor_for_testing(total_gas_limit);
    ...
    async fn build_test_cluster(default_num_validators: usize, default_epoch_duration_ms: u64) -> Arc<TestCluster> {
    ...
    async fn test_simulated_load(test_cluster: Arc<TestCluster>, test_duration_secs: u64) {
        test_simulated_load_with_test_config(test_cluster, test_duration_secs, SimulatedLoadConfig::default()).await;
    }

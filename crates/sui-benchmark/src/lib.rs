...
use sui_core::{
    ...
    quorum_driver::{QuorumDriver, QuorumDriverHandler, QuorumDriverHandlerBuilder, QuorumDriverMetrics},
};
...
impl ExecutionEffects {
    pub fn mutated(&self) -> Vec<(ObjectRef, Owner)> {
        match self {
            ...
            ExecutionEffects::SuiTransactionBlockEffects(sui_tx_effects) => {
                sui_tx_effects.mutated().iter().map(|refe| (refe.reference.to_object_ref(), refe.owner)).collect()
            }
        }
    }
    ...
    pub fn created(&self) -> Vec<(ObjectRef, Owner)> {
        match self {
            ExecutionEffects::CertifiedTransactionEffects(certified_effects, ..) => certified_effects.data().created(),
            ExecutionEffects::SuiTransactionBlockEffects(sui_tx_effects) => {
                sui_tx_effects.created().iter().map(|refe| (refe.reference.to_object_ref(), refe.owner)).collect()
            }
        }
    }
    ...
    pub fn quorum_sig(&self) -> Option<&AuthorityStrongQuorumSignInfo> {
        match self {
            ExecutionEffects::CertifiedTransactionEffects(certified_effects, ..) => Some(certified_effects.auth_sig()),
    ...
    pub fn gas_cost_summary(&self) -> GasCostSummary {
        match self {
            crate::ExecutionEffects::CertifiedTransactionEffects(a, _) => a.data().gas_cost_summary().clone(),
...
impl LocalValidatorAggregatorProxy {
    ...
    async fn new_impl(
        ...
        let qd_handler = (if let Some(reconfig_fullnode_rpc_url) = reconfig_fullnode_rpc_url {
            ...
            let new_agg = observer.get_committee(Arc::new(aggregator)).await.expect("Failed to get latest committee");
            let qd_handler_builder = QuorumDriverHandlerBuilder::new(new_agg, quorum_driver_metrics);
            qd_handler_builder.with_reconfig_observer(Arc::new(EmbeddedReconfigObserver::new()))
        })
        .start();

        let qd = qd_handler.clone_quorum_driver();
        Self { _qd_handler: qd_handler, qd, clients, committee, requests: Mutex::new(JoinSet::new()) }
    }
}
...
impl ValidatorProxy for LocalValidatorAggregatorProxy {
        ...
        while retry_cnt < 3 {
            ...
            match ticket.await {
                ...
                Err(err) => {
                    ...
                    error!(
                        ?tx_digest,
                        retry_cnt, "Transaction failed with err: {:?}. Sleeping for {:?} ...", err, delay,
                    );
                    retry_cnt += 1;
                    sleep(delay).await;
                }
    ...
    async fn execute_bench_transaction(&self, tx: Transaction) -> anyhow::Result<ExecutionEffects> {
        ...
        for (name, client) in self.clients.iter() {
            let fut = client.handle_transaction(tx.clone(), None).map(|r| (r, *name));
            futures.push(fut);
        }
        auth_agg.metrics.inflight_transaction_requests.add(futures.len() as i64);
                ...
                Ok(response) => match response.status {
                    ...
                    TransactionStatus::Executed(cert, _effects, _) => {
                        ...
                        if let Some(cert) = cert {
                            certificate = Some(CertifiedTransaction::new_from_data_and_sig(tx.data().clone(), cert));
                        }
                    }
                },
        ...
        let certified_transaction: CertifiedTransaction = match certificate {
            Some(x) => x,
            None => {
                let signatures: BTreeMap<_, _> = votes.into_iter().map(|a| (a.authority, a.signature)).collect();
        ...
        auth_agg.metrics.inflight_transaction_requests.sub(futures.len() as i64);
        drop(tx_guard);
        ...
        for (name, client) in self.clients.iter() {
            ...
            let name = *name;
            futures.push(async move { client.handle_certificate_v2(certificate, None).map(move |r| (r, name)).await });
        }
        auth_agg.metrics.inflight_certificate_requests.add(futures.len() as i64);
        ...
        const WAIT_TIMEOUT: Duration = Duration::from_secs(10);
        {
            ...
            requests.spawn(async move {
                ...
                .await;
                auth_agg.metrics.inflight_certificate_requests.sub(futures.len() as i64);
            });
        }

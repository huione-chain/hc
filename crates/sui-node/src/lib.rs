...
use sui_core::{
    ...
    consensus_manager::ConsensusClient,
...
use narwhal_network::metrics::{MetricsMakeCallbackHandler, NetworkConnectionMetrics, NetworkMetrics};
...
impl SuiNode {
    ...
    pub async fn start_async(
        ...
        let cache_traits = build_execution_cache(&epoch_start_configuration, &prometheus_registry, &store);
        ...
        let (p2p_network, discovery_handle, state_sync_handle, randomness_handle) = Self::create_p2p_network(
            &config,
        ...
        let network_connection_metrics = NetworkConnectionMetrics::new("sui", &registry_service.default_registry());

        let authority_names_to_peer_ids = ArcSwap::from_pointee(authority_names_to_peer_ids);

        let (_connection_monitor_handle, connection_statuses) = narwhal_network::connectivity::ConnectionMonitor::spawn(
        ...
        let connection_monitor_status = ConnectionMonitorStatus { connection_statuses, authority_names_to_peer_ids };
    ...
    fn create_p2p_network(
        ...
        prometheus_registry: &Registry,
    ) -> Result<(Network, discovery::Handle, state_sync::Handle, randomness::Handle)> {
        ...
        Ok((p2p_network, discovery_handle, state_sync_handle, randomness_handle))
    }
    ...
    async fn start_grpc_validator_service(
        ...
        let server = server_builder.bind(config.network_address()).await.map_err(|err| anyhow!(err.to_string()))?;
...
pub async fn build_http_server(
    ...
    if config.enable_experimental_rest_api {
        let mut rest_service =
            sui_rest_api::RestService::new(Arc::new(RestReadStore::new(state.clone(), store)), software_version);

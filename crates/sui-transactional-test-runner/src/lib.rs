...
use sui_core::authority::{authority_test_utils::send_and_confirm_transaction_with_execution_error, AuthorityState};
...
impl ReadStore for ValidatorWithFullnode {
    ...
    fn get_latest_checkpoint(&self) -> sui_types::storage::error::Result<VerifiedCheckpoint> {
        ...
        self.get_checkpoint_by_sequence_number(sequence_number).map(|c| c.unwrap())
    }

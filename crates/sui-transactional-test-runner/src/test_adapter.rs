...
use move_command_line_common::{address::ParsedAddress, files::verify_and_create_named_address_mapping};
use move_compiler::{
...
use sui_graphql_rpc::test_infra::cluster::{serve_executor, ExecutorCluster, SnapshotLagConfig};
...
async fn init_sim_executor(
    ...
    let cluster =
        serve_executor(Arc::new(read_replica), Some(snapshot_config), epochs_to_keep, data_ingestion_path).await;

    ...
    Req: Send + 'static,
{
    ...
    fn call(&mut self, req: Req) -> Self::Future {
        ...
        let future = Box::pin(async move {
            ...
            match &resp {
                Result::Ok(resp) if !resp.status().is_success() => metrics.failed(None, Some(resp.status())),
...
impl MetricsGuard {
    fn new(metrics: Arc<RequestMetrics>) -> Self {
        ...
        MetricsGuard { timer: Some(metrics.process_latency.start_timer()), metrics }
    }
    ...
    fn failed(mut self, error: Option<&BoxError>, status: Option<StatusCode>) {
        let elapsed = self.timer.take().unwrap().stop_and_record();
        let code = status.map(|c| c.as_str().to_string()).unwrap_or_else(|| "no_code".to_string());
        self.metrics.total_requests_failed.inc();

        if let Some(err) = error {
            error!("Request failed in {:.2}s, error {:?}, code {}", elapsed, err, code);
        } else {

...
use sui_bridge::{
    ...
    types::{BridgeCommitteeValiditySignInfo, CertifiedBridgeAction, VerifiedCertifiedBridgeAction},
    utils::{publish_and_register_coins_return_add_coins_on_sui_action, wait_for_server_to_be_up},
};
use sui_config::{
    genesis::Genesis,
...
use sui_types::{
    ...
    transaction::{CertifiedTransaction, ObjectArg, Transaction, TransactionData, TransactionDataAPI, TransactionKind},
...
impl TestCluster {
    ...
    pub async fn trigger_reconfiguration_if_not_yet_and_assert_bridge_committee_initialized(&self) {
        let mut bridge = get_bridge(self.fullnode_handle.sui_node.state().get_object_store()).unwrap();
        if !bridge.committee().members.contents.is_empty() {
            assert_eq!(self.swarm.active_validators().count(), bridge.committee().members.contents.len());
            return;
        }
        // wait for next epoch
    ...
    pub async fn get_mut_bridge_arg(&self) -> Option<ObjectArg> {
        get_bridge_obj_initial_shared_version(self.fullnode_handle.sui_node.state().get_object_store())
            .unwrap()
        ...
        timeout(
            ...
            self.fullnode_handle.sui_node.with_async(|node| async move {
                ...
                while let Some(tx) = txns.next().await {
                    ...
                    let tx = state.get_transaction_cache_reader().get_transaction_block(&digest).unwrap().unwrap();
...
impl TestClusterBuilder {
    ...
    pub async fn build(mut self) -> TestCluster {
        ...
        TestCluster { swarm, wallet, fullnode_handle, bridge_authority_keys: None, bridge_server_ports: None }
    }
    ...
    pub async fn build_with_bridge(
        ...
        let gas_objects_for_authority_keys = bridge_authority_keys
            ...
            .collect::<Vec<_>>();
        let mut test_cluster = self.with_objects(gas_objects_for_authority_keys).build().await;
        ...
        assert_eq!(bridge_authority_keys.len(), test_cluster.swarm.active_validators().count());

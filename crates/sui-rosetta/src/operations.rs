...
impl Operations {
    ...
    fn process_balance_change(
        ...
        *balances.entry((gas_owner, HC.clone())).or_default() -= gas_used;
...
impl TryFrom<SuiTransactionBlockData> for Operations {
    ...
    fn try_from(data: SuiTransactionBlockData) -> Result<Self, Self::Error> {
        ...
        Ok(Self::new(Self::from_transaction(data.transaction().clone(), sender, None)?))
    }
}
impl Operations {
    pub async fn try_from_response(
        ...
        let staking_balance = if principal_amounts != 0 {
            *accounted_balances.entry((sender, HC.clone())).or_default() -= principal_amounts;
            *accounted_balances.entry((sender, HC.clone())).or_default() -= reward_amounts;
            vec![

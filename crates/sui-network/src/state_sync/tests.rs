...
async fn sync_with_checkpoints_watermark() {
    ...
    timeout(Duration::from_secs(1), async {
        for (checkpoint, contents) in ordered_checkpoints[2..].iter().zip(contents.clone().into_iter().skip(2)) {
            ...
            store_1.get_full_checkpoint_contents(&content_digest).unwrap().unwrap();
            assert_eq!(store_2.get_full_checkpoint_contents(&content_digest).unwrap(), None);
        }
    })
    .await
    .unwrap();
    ...
    timeout(Duration::from_secs(1), async {
        ...
        store_3.get_full_checkpoint_contents(&content_digest).unwrap().unwrap();
    })
    .await
    .unwrap();
    ...
    timeout(Duration::from_secs(6), async {
        for (checkpoint, contents) in ordered_checkpoints[2..].iter().zip(contents.clone().into_iter().skip(2)) {
            ...
            let content_digest = contents.into_checkpoint_contents_digest();
            store_2.get_full_checkpoint_contents(&content_digest).unwrap().unwrap();
            store_3.get_full_checkpoint_contents(&content_digest).unwrap().unwrap();
        }
    })
    .await
    ...
    timeout(Duration::from_secs(3), async {
        for (checkpoint, contents) in ordered_checkpoints[1..].iter().zip(contents.clone().into_iter().skip(1)) {
            ...
            let content_digest = contents.into_checkpoint_contents_digest();
            store_4.get_full_checkpoint_contents(&content_digest).unwrap().unwrap();
        }
    })
    .await
    .unwrap();

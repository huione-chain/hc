    ...
    S: WriteStore + Send + Sync + 'static,
{
    ...
    async fn get_checkpoint_summary(
        ...
        let checkpoint = match request.inner() {
            GetCheckpointSummaryRequest::Latest => self.store.get_highest_synced_checkpoint().map(Some),
    ...
    async fn get_checkpoint_contents(
        ...
        let contents =
            self.store.get_full_checkpoint_contents(request.inner()).map_err(|e| Status::internal(e.to_string()))?;

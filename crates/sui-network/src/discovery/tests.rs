...
async fn get_known_peers() -> Result<()> {
    ...
    let response = server.get_known_peers(Request::new(())).await.unwrap().into_inner();
...
async fn make_connection_to_seed_peer() -> Result<()> {
    let config = P2pConfig::default()
        .set_discovery_config(DiscoveryConfig { enable_node_info_signatures: Some(true), ..DiscoveryConfig::default() });

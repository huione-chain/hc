...
impl ObjectStore for InMemoryObjectStore {
    ...
    fn get_object_by_key(
        ...
        Ok(self.get_object(object_id).unwrap().and_then(|o| if o.version() == version { Some(o.clone()) } else { None }))
    }
}
...
impl ParentSync for InMemoryObjectStore {
    fn get_latest_parent_entry_ref_deprecated(&self, _object_id: ObjectID) -> SuiResult<Option<ObjectRef>> {
        unreachable!()
    }
}

impl GetSharedLocks for InMemoryObjectStore {
    fn get_shared_locks(&self, _key: &TransactionKey) -> Result<Vec<(ObjectID, SequenceNumber)>, SuiError> {
        unreachable!()
    }
}

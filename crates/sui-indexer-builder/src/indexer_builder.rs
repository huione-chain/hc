...
impl<P, D, M> Indexer<P, D, M> {
        ...
        T: Send,
    {
        ...
        let largest_checkpoint = self.storage.get_largest_backfill_task_target_checkpoint(&self.name).await?;
...
pub trait Datasource<T: Send>: Sync + Send {
        ...
        P: Persistent<R>,
    {
        ...
        let _live_tasks_tracker = if is_live_task {
            Some(LiveTasksTracker::new(self.get_inflight_live_tasks_metrics().clone(), &task_name))
        } else {
            None
        };
        let join_handle = self.start_data_retrieval(task.clone(), data_sender).await?;
        let processed_checkpoints_metrics =
            self.get_tasks_processed_checkpoints_metric().with_label_values(&[task_name_prefix, task_type_label]);
        // track remaining checkpoints per task, except for live task
        let remaining_checkpoints_metric = if !is_live_task {
            let remaining = self.get_tasks_remaining_checkpoints_metric().with_label_values(&[task_name_prefix]);

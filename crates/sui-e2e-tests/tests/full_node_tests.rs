...
async fn test_full_node_follows_txes() -> Result<(), anyhow::Error> {
    ...
    fullnode.state().get_transaction_cache_reader().notify_read_executed_effects(&[digest]).await.unwrap();
...
async fn test_full_node_shared_objects() -> Result<(), anyhow::Error> {
    ...
    handle.sui_node.state().get_transaction_cache_reader().notify_read_executed_effects(&[digest]).await.unwrap();
...
async fn do_test_full_node_sync_flood() {
    ...
    let digests: Vec<_> = future::join_all(futures)
        ...
        .collect();
    fullnode.state().get_transaction_cache_reader().notify_read_executed_effects(&digests).await.unwrap();
}
...
async fn test_full_node_bootstrap_from_snapshot() -> Result<(), anyhow::Error> {
    ...
    node.state().get_transaction_cache_reader().notify_read_executed_effects(&[digest]).await.unwrap();
    ...
    node.state().get_transaction_cache_reader().notify_read_executed_effects(&[digest_after_restore]).await.unwrap();
    Ok(())
}
        ...
        validator
            ...
            .with_async(|node| async {
                ...
                state
                    ...
                    .prune_objects_and_compact_for_testing(state.get_checkpoint_store(), state.rest_index.as_deref())
                    .await;
                // Make sure the old version of the object is already pruned.
                assert!(state.database_for_testing().get_object_by_key(&gas_object.0, gas_object.1).unwrap().is_none());

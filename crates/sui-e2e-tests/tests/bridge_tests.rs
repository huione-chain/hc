...
use sui_bridge::{crypto::BridgeAuthorityKeyPair, BRIDGE_ENABLE_PROTOCOL_VERSION};
...
async fn test_committee_registration() {
    ...
    let bridge = get_bridge(test_cluster.fullnode_handle.sui_node.state().get_object_store()).unwrap();

    // Member should be empty before end of epoch
    assert!(bridge.committee().members.contents.is_empty());
    assert_eq!(test_cluster.swarm.active_validators().count(), bridge.committee().member_registrations.contents.len());

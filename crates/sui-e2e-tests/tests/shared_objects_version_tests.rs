...
impl TestEnvironment {
    ...
    async fn create_counter(&self) -> (ObjectRef, Owner) {
        ...
        *fx.created().iter().find(|(_, owner)| matches!(owner, Owner::AddressOwner(_))).expect("Owned object created")
    }
    ...
    async fn create_shared_counter(&self) -> (ObjectRef, Owner) {
        ...
        *fx.created().iter().find(|(_, owner)| owner.is_shared()).expect("Shared object created")
    }
    ...
    async fn share_counter(&self, counter: ObjectRef) -> Result<(ObjectRef, Owner), ExecutionFailureStatus> {
        ...
        Ok(*fx.mutated().iter().find(|(obj, _)| obj.0 == counter.0).expect("Counter mutated"))
    }
    ...
    async fn increment_owned_counter(&self, counter: ObjectRef) -> (ObjectRef, Owner) {
        ...
        *fx.mutated().iter().find(|(obj, _)| obj.0 == counter.0).expect("Counter modified")
    }
    ...
    async fn increment_shared_counter(
        ...
        Ok(*fx.mutated().iter().find(|(obj, _)| obj.0 == counter).expect("Counter modified"))
    }
}

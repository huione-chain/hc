...
impl StressTestRunner {
    ...
    pub fn display_effects(&self, effects: &TransactionEffects) {
        ...
        for (obj_ref, _) in effects.created() {
            let object_opt = state.get_object_store().get_object_by_key(&obj_ref.0, obj_ref.1).unwrap();
        ...
        for (obj_ref, _) in effects.mutated() {
            let object = state.get_object_store().get_object_by_key(&obj_ref.0, obj_ref.1).unwrap().unwrap();
            let struct_tag = object.struct_tag().unwrap();
            let total_sui = object.get_total_sui(layout_resolver.as_mut()).unwrap() - object.storage_rebate;
            println!(">> {struct_tag} TOTAL_SUI: {total_sui}");
        }
        ...
        for kind in effects.input_shared_objects() {
            ...
            let object = state.get_object_store().get_object_by_key(&obj_id, version).unwrap().unwrap();
            let struct_tag = object.struct_tag().unwrap();
            let total_sui = object.get_total_sui(layout_resolver.as_mut()).unwrap() - object.storage_rebate;
            println!(">> {struct_tag} TOTAL_SUI: {total_sui}");
        }
    ...
    async fn get_from_effects(&self, effects: &[(ObjectRef, Owner)], name: &str) -> Option<Object> {
        ...
        let found: Vec<_> = effects
            ...
            .filter_map(|(obj_ref, _)| {
                let object = db.get_object_by_key(&obj_ref.0, obj_ref.1).unwrap().unwrap();
                let struct_tag = object.struct_tag().unwrap();

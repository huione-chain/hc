...
{
    let committee =
        store.get_committee(checkpoint.epoch()).expect("store operation should not fail").unwrap_or_else(|| {
            panic!(
    ...
    S: WriteStore + Clone,
{
    ...
    futures::stream::iter(range_clone.into_iter().tuple_windows())
        .map(|(a, b)| {
            ...
            let committee =
                store.get_committee(next.epoch()).expect("store operation should not fail").unwrap_or_else(|| {
                    panic!(
                        "BUG: should have committee for epoch {} before we try to verify checkpoint {}",
                        next.epoch(),

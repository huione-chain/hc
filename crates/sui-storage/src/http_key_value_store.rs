...
pub fn key_to_path_elements(key: &Key) -> SuiResult<(String, &'static str)> {
    match key {
        ...
        Key::CheckpointContents(seq) => Ok((encoded_tagged_key(&TaggedKey::CheckpointSequenceNumber(*seq)), "cc")),
        Key::CheckpointSummary(seq) => Ok((encoded_tagged_key(&TaggedKey::CheckpointSequenceNumber(*seq)), "cs")),
...
impl HttpKVStore {
    ...
    async fn fetch(&self, key: Key) -> SuiResult<Option<Bytes>> {
        ...
        if let Some(res) = self.cache.get(&url) {
            ...
            self.metrics.key_value_store_num_fetches_success.with_label_values(&["http_cache", "url"]).inc();
            return Ok(Some(res));
        }

        self.metrics.key_value_store_num_fetches_not_found.with_label_values(&["http_cache", "url"]).inc();
...
impl TransactionKeyValueStoreTrait for HttpKVStore {
    ...
    async fn multi_get_checkpoints(
        ...
        let keys = checkpoint_summaries
            ...
            .chain(checkpoint_contents_by_digest.iter().map(|cp| Key::CheckpointContentsByDigest(*cp)))
            .collect::<Vec<_>>();
        ...
        let input_slices = [summaries_len, contents_len, summaries_by_digest_len, contents_by_digest_len];
        ...
        let contents_by_digest_results = result_slices[3]
            ...
            .map(|maybe_bytes| {
                maybe_bytes
                    .and_then(|(bytes, digest)| deser_check_digest(digest, bytes, |c: &CheckpointContents| *c.digest()))
            })
            .collect::<Vec<_>>();

        Ok((summaries_results, contents_results, summaries_by_digest_results, contents_by_digest_results))
    }
    ...
    async fn get_object(&self, object_id: ObjectID, version: SequenceNumber) -> SuiResult<Option<Object>> {
        ...
        self.fetch(key).await.map(|maybe| maybe.and_then(|bytes| deser::<_, Object>(&key, bytes.as_ref())))
    }

...
use sui_bridge_indexer::eth_bridge_indexer::{EthFinalizedSyncDatasource, EthSubscriptionDatasource};
...
use sui_bridge_indexer::{
    ...
    eth_bridge_indexer::EthDataMapper,
...
use sui_bridge_watchdog::{
    ...
    eth_vault_balance::EthVaultBalance,
...
async fn main() -> Result<()> {
    ...
    if Some(true) == config.disable_eth {
        ...
        let bridge_addresses: Vec<EthAddress> =
            vec![bridge_address, bridge_addresses.0, bridge_addresses.1, bridge_addresses.2, bridge_addresses.3];
        ...
        let eth_subscription_indexer = IndexerBuilder::new(
            ...
            EthDataMapper { metrics: indexer_meterics.clone() },
            datastore.clone(),
        )
    ...
    let indexer = IndexerBuilder::new(
        ...
        SuiBridgeDataMapper { metrics: indexer_meterics.clone() },
...
async fn start_watchdog(
    ...
    BridgeWatchDog::new(vec![Arc::new(eth_vault_balance), Arc::new(eth_bridge_status), Arc::new(sui_bridge_status)])
        .run()
        .await;
    Ok(())
}

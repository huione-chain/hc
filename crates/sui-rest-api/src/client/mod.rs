...
impl Client {
    pub fn new<S: AsRef<str>>(base_url: S) -> Self {
        Self { inner: sdk::Client::new(base_url.as_ref()).unwrap() }
    }
    ...
    pub async fn get_full_checkpoint(
        ...
        let url = self.inner.url().join(&format!("checkpoints/{checkpoint_sequence_number}/full"))?;

        let response =
            self.inner.client().get(url).header(reqwest::header::ACCEPT, crate::APPLICATION_BCS).send().await?;
...
pub enum EffectsFinality {
    Certified { signature: AuthorityStrongQuorumSignInfo },
    Checkpointed { checkpoint: CheckpointSequenceNumber },
}

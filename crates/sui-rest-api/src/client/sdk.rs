...
use crate::{
    ...
    transactions::{ListTransactionsQueryParameters, TransactionExecutionResponse, TransactionResponse},
    types::{
        X_SUI_CHAIN,
        X_SUI_CHAIN_ID,
...
impl Client {
    ...
    pub async fn node_info(&self) -> Result<Response<NodeInfo>> {
        ...
        let response = self.inner.get(url).header(reqwest::header::ACCEPT, crate::APPLICATION_JSON).send().await?;
    ...
    pub async fn get_object(&self, object_id: ObjectId) -> Result<Response<Object>> {
        ...
        let response = self.inner.get(url).header(reqwest::header::ACCEPT, crate::APPLICATION_BCS).send().await?;
    ...
    pub async fn get_latest_checkpoint(&self) -> Result<Response<SignedCheckpointSummary>> {
        let parameters = ListCheckpointsQueryParameters { limit: Some(1), start: None, direction: None };
    ...
    pub async fn list_checkpoints(
        ...
        let response =
            self.inner.get(url).query(parameters).header(reqwest::header::ACCEPT, crate::APPLICATION_BCS).send().await?;
    ...
    async fn json<T: serde::de::DeserializeOwned>(&self, response: reqwest::Response) -> Result<Response<T>> {
    ...
    pub(super) async fn bcs<T: serde::de::DeserializeOwned>(&self, response: reqwest::Response) -> Result<Response<T>> {
        let (response, parts) = self.check_response(response).await?;

...
use sui_sdk2::types::{CheckpointData, CheckpointDigest, CheckpointSequenceNumber, SignedCheckpointSummary};
...
impl ApiEndpoint<RestService> for GetCheckpointFull {
    ...
    fn operation(&self, generator: &mut schemars::gen::SchemaGenerator) -> openapiv3::v3_1::Operation {
        OperationBuilder::new()
            ...
            .response(200, ResponseBuilder::new().json_content::<CheckpointData>(generator).bcs_content().build())
    ...
    let verified_summary = match checkpoint_id {
        CheckpointId::SequenceNumber(s) => {
            ...
            if s < oldest_checkpoint {
                return Err(crate::RestError::new(axum::http::StatusCode::GONE, "Old checkpoints have been pruned"));
            }
...
impl ApiEndpoint<RestService> for GetCheckpoint {
    ...
    fn operation(&self, generator: &mut schemars::gen::SchemaGenerator) -> openapiv3::v3_1::Operation {
        OperationBuilder::new()
            ...
            .response(
                200,
                ResponseBuilder::new().json_content::<SignedCheckpointSummary>(generator).bcs_content().build(),
            )
            .response(404, ResponseBuilder::new().build())
            .response(410, ResponseBuilder::new().build())
            .build()
...
async fn list_checkpoints(
    ...
    let checkpoints = state
        ...
        .map(|result| {
            result
                .map_err(Into::into)
                .and_then(|(checkpoint, _contents)| SignedCheckpointSummary::try_from(checkpoint).map_err(Into::into))
        })
...
impl ListCheckpointsQueryParameters {
    pub fn limit(&self) -> usize {
        self.limit.map(|l| (l as usize).clamp(1, crate::MAX_PAGE_SIZE)).unwrap_or(crate::DEFAULT_PAGE_SIZE)
    }

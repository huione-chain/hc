...
impl ApiEndpoint<RestService> for GetLatestCommittee {
    ...
    fn operation(&self, generator: &mut schemars::gen::SchemaGenerator) -> openapiv3::v3_1::Operation {
        OperationBuilder::new()
            ...
            .response(200, ResponseBuilder::new().json_content::<ValidatorCommittee>(generator).bcs_content().build())
            .build()
    }
...
async fn get_latest_committee(
    ...
    let committee = state.get_committee(current_epoch)?.ok_or_else(|| CommitteeNotFoundError::new(current_epoch))?;
...
async fn get_committee(
    ...
    let committee = state.get_committee(epoch)?.ok_or_else(|| CommitteeNotFoundError::new(epoch))?;

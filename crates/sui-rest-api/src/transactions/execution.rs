...
impl ApiEndpoint<RestService> for ExecuteTransaction {
    ...
    fn operation(&self, generator: &mut schemars::gen::SchemaGenerator) -> openapiv3::v3_1::Operation {
        ...
        OperationBuilder::new()
            ...
            .response(
                200,
                ResponseBuilder::new().json_content::<TransactionExecutionResponse>(generator).bcs_content().build(),
            )
            .build()
    }
...
async fn execute_transaction(
    ...
    let sui_types::quorum_driver_types::ExecuteTransactionResponseV3 {
        ...
        output_objects,
        auxiliary_data: _,
    } = executor.execute_transaction(request, client_address.map(|a| a.0)).await?;
    ...
    let events = if parameters.events { events.map(TryInto::try_into).transpose()? } else { None };
    ...
    let input_objects = if parameters.input_objects { input_objects } else { None };

    let output_objects = if parameters.output_objects { output_objects } else { None };

    let response =
        TransactionExecutionResponse { effects, finality, events, balance_changes, input_objects, output_objects };
        ...
        S: serde::Serializer,
    {
        if serializer.is_human_readable() {
            let readable = match self.clone() {
                EffectsFinality::Certified { signature } => ReadableEffectsFinality::Certified { signature },
                EffectsFinality::Checkpointed { checkpoint } => ReadableEffectsFinality::Checkpointed { checkpoint },
            };
            readable.serialize(serializer)
        } else {
            let binary = match self.clone() {
                EffectsFinality::Certified { signature } => BinaryEffectsFinality::Certified { signature },
                EffectsFinality::Checkpointed { checkpoint } => BinaryEffectsFinality::Checkpointed { checkpoint },
            };
        ...
        D: serde::Deserializer<'de>,
    {
        if deserializer.is_human_readable() {
            ReadableEffectsFinality::deserialize(deserializer).map(|readable| match readable {
                ReadableEffectsFinality::Certified { signature } => EffectsFinality::Certified { signature },
                ReadableEffectsFinality::Checkpointed { checkpoint } => EffectsFinality::Checkpointed { checkpoint },
            })
        } else {
            BinaryEffectsFinality::deserialize(deserializer).map(|binary| match binary {
                BinaryEffectsFinality::Certified { signature } => EffectsFinality::Certified { signature },
                BinaryEffectsFinality::Checkpointed { checkpoint } => EffectsFinality::Checkpointed { checkpoint },
            })
        }
    }
}
...
fn derive_balance_changes(
    ...
    balances
        ...
        .filter_map(|((address, coin_type), amount)| {
            ...
            Some(BalanceChange { address: *address, coin_type: coin_type.to_owned(), amount })
        })
        .collect()
}

...
impl<T, V: store::SimulatorStore> ReadStore for Simulacrum<T, V> {
    ...
    fn get_checkpoint_by_sequence_number(
        ...
        Ok(self.store().get_checkpoint_by_sequence_number(sequence_number))
    }
...
impl<T: Send + Sync, V: store::SimulatorStore + Send + Sync> RestStateReader for Simulacrum<T, V> {
    ...
    ) -> sui_types::storage::error::Result<Option<sui_types::messages_checkpoint::CheckpointSequenceNumber>> {
        todo!()
    }

...
async fn make_clients(
    ...
    for validator in active_validators {
        ...
        let channel = net_config.connect_lazy(&net_addr).map_err(|err| anyhow!(err.to_string()))?;
...
pub async fn dump_checkpoints_from_archive(
    ...
    for key in store.inner().checkpoints().values().sorted_by(|a, b| a.sequence_number().cmp(&b.sequence_number)) {
        let mut content = serde_json::to_string(
            &store.get_full_checkpoint_contents_by_sequence_number(key.sequence_number)?.unwrap(),
        )?;

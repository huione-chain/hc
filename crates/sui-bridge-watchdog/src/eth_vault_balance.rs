...
use std::sync::Arc;
use sui_bridge::{abi::EthERC20, metered_eth_provider::MeteredEthHttpProvier};
...
impl EthVaultBalance {
    pub fn new(
        ...
        Self { coin_contract, vault_address, ten_zeros, metric }
    }
}
...
impl Observable for EthVaultBalance {
    ...
    async fn observe_and_report(&self) {
        match self.coin_contract.balance_of(self.vault_address).call().await {
            Ok(balance) => {

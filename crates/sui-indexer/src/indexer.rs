...
use crate::{
    ...
    handlers::{
        ...
        objects_snapshot_processor::start_objects_snapshot_processor,
        pruner::Pruner,
    },
...
impl Indexer {
    ...
    pub async fn start_writer_with_config(
        ...
        let (object_snapshot_worker, object_snapshot_watermark) =
            start_objects_snapshot_processor(store.clone(), metrics.clone(), snapshot_config, cancel.clone()).await?;

        if let Some(epochs_to_keep) = pruning_options.epochs_to_keep {
            info!("Starting indexer pruner with epochs to keep: {}", epochs_to_keep);

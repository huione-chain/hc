...
impl PgIndexerStore {
    ...
    async fn get_checkpoint_range_for_epoch(&self, epoch: u64) -> Result<(u64, Option<u64>), IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    async fn get_transaction_range_for_checkpoint(&self, checkpoint: u64) -> Result<(u64, u64), IndexerError> {
        use diesel_async::RunQueryDsl;
    ...
    async fn persist_objects_snapshot_chunk(
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                for objects_snapshot_chunk in objects_snapshot.chunks(PG_COMMIT_CHUNK_SIZE_INTRA_DB_TX) {
    ...
    async fn persist_object_version_chunk(&self, object_versions: Vec<StoredObjectVersion>) -> Result<(), IndexerError> {
        use diesel_async::RunQueryDsl;
                ...
                if let Some(last_epoch) = &epoch.last_epoch {
                    ...
                    diesel::insert_into(epochs::table)
                        ...
                        .set((
                            ...
                            epochs::storage_fund_reinvestment.eq(excluded(epochs::storage_fund_reinvestment)),
                            ...
                            epochs::leftover_storage_fund_inflow.eq(excluded(epochs::leftover_storage_fund_inflow)),
    ...
    async fn prune_tx_indices_table(&self, min_tx: u64, max_tx: u64) -> Result<(), IndexerError> {
        ...
        transaction_with_retry(&self.pool, PG_DB_COMMIT_SLEEP_DURATION, |conn| {
            async {
                ...
                diesel::delete(tx_senders::table.filter(tx_senders::tx_sequence_number.between(min_tx, max_tx)))
                    .execute(conn)
                    .await?;

                diesel::delete(tx_recipients::table.filter(tx_recipients::tx_sequence_number.between(min_tx, max_tx)))
                    .execute(conn)
                    .await?;
    ...
    async fn get_network_total_transactions_by_end_of_epoch(&self, epoch: u64) -> Result<u64, IndexerError> {
        use diesel_async::RunQueryDsl;
...
impl IndexerStore for PgIndexerStore {
    ...
    async fn persist_objects(&self, object_changes: Vec<TransactionObjectChangesToCommit>) -> Result<(), IndexerError> {
        ...
        let mutation_futures =
            object_mutation_chunks.into_iter().map(|c| self.persist_object_mutation_chunk(c)).collect::<Vec<_>>();
        futures::future::join_all(mutation_futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
            IndexerError::PostgresWriteError(format!("Failed to persist all object mutation chunks: {:?}", e))
        })?;
        let deletion_futures =
            object_deletion_chunks.into_iter().map(|c| self.persist_object_deletion_chunk(c)).collect::<Vec<_>>();
        futures::future::join_all(deletion_futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
            IndexerError::PostgresWriteError(format!("Failed to persist all object deletion chunks: {:?}", e))
        })?;

        let elapsed = guard.stop_and_record();
        info!(elapsed, "Persisted objects with {} mutations and {} deletions ", mutation_len, deletion_len,);
        Ok(())
    }
    ...
    async fn persist_objects_snapshot(
        ...
        let objects_snapshot = indexed_mutations
            ...
            .chain(indexed_deletions.into_iter().map(StoredObjectSnapshot::from))
            .collect::<Vec<_>>();
        let len = objects_snapshot.len();
        let chunks = chunk!(objects_snapshot, self.config.parallel_objects_chunk_size);
        let futures = chunks.into_iter().map(|c| self.persist_objects_snapshot_chunk(c)).collect::<Vec<_>>();

        futures::future::join_all(futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
            IndexerError::PostgresWriteError(format!("Failed to persist all objects snapshot chunks: {:?}", e))
        })?;
        let elapsed = guard.stop_and_record();
    ...
    async fn persist_object_versions(&self, object_versions: Vec<StoredObjectVersion>) -> Result<(), IndexerError> {
        ...
        let futures = chunks.into_iter().map(|c| self.persist_object_version_chunk(c)).collect::<Vec<_>>();

        futures::future::join_all(futures).await.into_iter().collect::<Result<Vec<_>, _>>().map_err(|e| {
            IndexerError::PostgresWriteError(format!("Failed to persist all object version chunks: {:?}", e))
        })?;

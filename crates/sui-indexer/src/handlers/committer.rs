    ...
    S: IndexerStore + Clone + Sync + Send + 'static,
{
    ...
    let raw_checkpoints_batch = checkpoint_batch.iter().map(|c| c.into()).collect::<Vec<_>>();
    ...
    info!(
        elapsed,
        "Checkpoint {}-{} committed with {} transactions.", first_checkpoint_seq, last_checkpoint_seq, tx_count,
    );
    metrics.latest_tx_checkpoint_sequence_number.set(last_checkpoint_seq as i64);

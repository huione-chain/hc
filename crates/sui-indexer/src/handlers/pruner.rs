...
impl Pruner {
    pub fn new(store: PgIndexerStore, epochs_to_keep: u64, metrics: IndexerMetrics) -> Result<Self, IndexerError> {
        let partition_manager = PgPartitionManager::new(store.pool())?;
        Ok(Self { store, partition_manager, epochs_to_keep, metrics })
    }
    ...
    pub async fn start(&self, cancel: CancellationToken) -> IndexerResult<()> {
        ...
        while !cancel.is_cancelled() {
            ...
            for (table_name, (min_partition, max_partition)) in &table_partitions {
                ...
                let epochs_to_keep =
                    if table_name == "objects_history" { OBJECTS_HISTORY_EPOCHS_TO_KEEP } else { self.epochs_to_keep };

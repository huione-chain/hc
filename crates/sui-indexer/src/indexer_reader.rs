    ...
    pub async fn query_transaction_blocks(
        ...
        let (table_name, main_where_clause) = match filter {
            ...
            Some(TransactionFilter::FromAddress(from_address)) => {
                ...
                ("tx_senders".to_owned(), format!("sender = '\\x{from_address}'::bytea"))
            }
...
impl sui_types::storage::ObjectStore for ConnectionAsObjectStore {
    fn get_object(
        ...
        self.get_object(object_id, None).map_err(sui_types::storage::error::Error::custom)
    }
    ...
    fn get_object_by_key(
        ...
        self.get_object(object_id, Some(version)).map_err(sui_types::storage::error::Error::custom)
    }
}

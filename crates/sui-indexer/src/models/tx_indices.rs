...
impl TxIndex {
    pub fn split(
        ...
        let tx_sender = StoredTxSenders { tx_sequence_number, sender: self.sender.to_vec() };

        let tx_recipients = self
            .recipients
            .iter()
            .map(|s| StoredTxRecipients { tx_sequence_number, recipient: s.to_vec(), sender: self.sender.to_vec() })
            .collect();

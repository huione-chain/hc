...
impl ReadApi {
    ...
    async fn get_checkpoint_internal(&self, id: CheckpointId) -> Result<Checkpoint, Error> {
        Ok(match id {
            CheckpointId::SequenceNumber(seq) => {
                ...
                let content =
                    self.transaction_kv_store.get_checkpoint_contents_by_digest(verified_summary.content_digest).await?;
    ...
    pub async fn get_checkpoints_internal(
        ...
        let checkpoint_contents_digest: Vec<CheckpointContentsDigest> =
            checkpoint_summaries_and_signatures.iter().map(|summary| summary.0.content_digest).collect();
        let checkpoint_contents =
            transaction_kv_store.multi_get_checkpoints_contents_by_digest(checkpoint_contents_digest.as_slice()).await?;
    ...
    async fn multi_get_transaction_blocks_internal(
        ...
        if opts.show_events {
            ...
            let events_digests_list =
                events_digests_list.into_iter().filter(|d| d != &empty_events_digest).collect::<Vec<_>>();
            ...
            let mut events_digest_to_events = if events_digests_list.is_empty() {
                ...
                let events = retry(backoff, || async {
                    match self.transaction_kv_store.multi_get_events(&events_digests_list).await {
                ...
                let events_map = events_digests_list.into_iter().zip(events).collect::<HashMap<_, _>>();
    ...
    async fn get_events(&self, transaction_digest: TransactionDigest) -> RpcResult<Vec<SuiEvent>> {
        with_tracing!(async move {
            ...
            spawn_monitored_task!(async move {
                ...
                let effect = transaction_kv_store.get_fx_by_tx_digest(transaction_digest).await.map_err(Error::from)?;
                let events = if let Some(event_digest) = effect.events_digest() {
                    transaction_kv_store
                        ...
                        .map_err(|e| {
                            error!(
                                "Failed to get transaction events for event digest {event_digest:?} with error: {e:?}"
                            );
                        ...
                        .map(|(seq, e)| {
                            ...
                            SuiEvent::try_from(e, *effect.transaction_digest(), seq as u64, None, layout)
                        })
                        .collect::<Result<Vec<_>, _>>()

    ...
    match param {
        ...
        SignatureToken::Vector(inner) => {
            ...
            match inner_layout_opt {
                Some(inner_layout) => (is_primitive, Some(MoveTypeLayout::Vector(Box::new(inner_layout)))),
        ...
        SignatureToken::DatatypeInstantiation(struct_inst) => {
            ...
            if resolved_struct == RESOLVED_STD_OPTION && targs.len() == 1 {
                ...
                let layout = inner_layout.map(|inner_layout| MoveTypeLayout::Vector(Box::new(inner_layout)));
                (is_primitive, layout)
            } else {
                (false, None)
            }
        ...
        SignatureToken::TypeParameter(idx) => {
            (type_args.get(*idx as usize).map(is_primitive_type_tag).unwrap_or(false), None)
        }

        SignatureToken::Signer | SignatureToken::Reference(_) | SignatureToken::MutableReference(_) => (false, None),
    }
}
    ...
    if is_primitive {
        match layout_opt {
            ...
            None => {
                ...
                bail!("Could not serialize argument of type {:?} at {}", param, idx);
            }
        }
    }

...
impl ObjectImpl<'_> {
    ...
    pub(crate) async fn owner(&self, ctx: &Context<'_>) -> Option<ObjectOwner> {
        ...
        match native.owner {
            ...
            O::ObjectOwner(address) => {
                let parent = Object::query(ctx, address.into(), Object::latest_at(self.0.checkpoint_viewed_at))
                    .await
                    .ok()
                    .flatten();
    ...
    pub(crate) async fn previous_transaction_block(&self, ctx: &Context<'_>) -> Result<Option<TransactionBlock>> {
        ...
        TransactionBlock::query(ctx, digest.into(), self.0.checkpoint_viewed_at).await.extend()
    }
...
impl Object {
    ...
    pub(crate) fn new_serialized(
        ...
        if let Some(bytes) = serialized {
            Self { address: object_id, version, kind: ObjectKind::Serialized(bytes), checkpoint_viewed_at, root_version }
        } else {
            Self {
                address: object_id,
                version,

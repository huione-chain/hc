...
impl TransactionBlock {
    ...
    async fn bcs(&self) -> Option<Base64> {
        match &self.inner {
            TransactionBlockInner::Stored { stored_tx, .. } => Some(Base64::from(&stored_tx.raw_transaction)),
            TransactionBlockInner::Executed { tx_data, .. } => bcs::to_bytes(&tx_data).ok().map(Base64::from),

...
impl Address {
    ...
    async fn transaction_blocks(
        ...
        let Some(filter) = filter.unwrap_or_default().intersect(match relation {
            ...
            Some(R::Sign) | Some(R::Sent) | None => {
                TransactionBlockFilter { sent_address: Some(self.address), ..Default::default() }
            }

            Some(R::Recv) => TransactionBlockFilter { recv_address: Some(self.address), ..Default::default() },

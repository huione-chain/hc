...
impl CheckpointStore {
    ...
    pub async fn reexecute_local_checkpoints(&self, state: &AuthorityState, epoch_store: &AuthorityPerEpochStore) {
        ...
        for seq in highest_executed + 1..=*highest_built.sequence_number() {
            ...
            let txns =
                cache.multi_get_transaction_blocks(&tx_digests).expect("multi_get_transaction_blocks should not fail");
...
impl CheckpointBuilder {
    ...
    fn expensive_consensus_commit_prologue_invariants_check(
        ...
        let root_txs = self.state.get_transaction_cache_reader().multi_get_transaction_blocks(root_digests).unwrap();
        let ccps = root_txs
            .iter()
            .filter_map(|tx| {
                if let Some(tx) = tx {
...
impl CheckpointAggregator {
    fn new(
        ...
        let current = None;
        Self { tables, epoch_store, notify, exit, current, output, state, metrics }
    }
    ...
    async fn run(mut self) {
        ...
        loop {
            ...
            match select(self.exit.changed().boxed(), timeout(Duration::from_secs(1), self.notify.notified()).boxed())
                .await
            {
                Either::Left(_) => {
...
mod tests {
    ...
    impl TransactionCacheRead for HashMap<TransactionDigest, TransactionEffects> {
        fn notify_read_executed_effects(
            ...
            std::future::ready(Ok(digests.iter().map(|d| self.get(d).expect("effects not found").clone()).collect()))
                .boxed()
        }
        ...
        fn notify_read_executed_effects_digests(
            ...
            std::future::ready(Ok(digests
                .iter()
                .map(|d| self.get(d).map(|fx| fx.digest()).expect("effects not found"))
                .collect()))
            .boxed()
        }
        ...
        fn multi_get_effects(&self, _: &[TransactionEffectsDigest]) -> SuiResult<Vec<Option<TransactionEffects>>> {
            unimplemented!()
        }

        fn multi_get_events(&self, _: &[TransactionEventsDigest]) -> SuiResult<Vec<Option<TransactionEvents>>> {
            unimplemented!()
        }
    }

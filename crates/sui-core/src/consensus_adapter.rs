...
impl SubmitToConsensus for TransactionsClient<sui_network::tonic::transport::Channel> {
    async fn submit_to_consensus(
        ...
        let transactions_bytes = transactions
            .iter()
            .map(|t| {
                let serialized = bcs::to_bytes(t).expect("Serializing consensus transaction cannot fail");
                Bytes::from(serialized)
            })
            .collect::<Vec<_>>();
        self.clone()
            .submit_transaction(TransactionProto { transactions: transactions_bytes })
            .await
    ...
    async fn submit_and_wait_inner(
        ...
        if let Some(processed_waiter) = processed_waiter {
            ...
            let submit_inner = async {
                ...
                while let Err(e) = self.consensus_client.submit_to_consensus(&transactions[..], epoch_store).await {
        ...
        let is_user_tx =
            is_soft_bundle || matches!(transactions[0].kind, ConsensusTransactionKind::CertifiedTransaction(_));
    ...
    async fn await_consensus_or_checkpoint(
        ...
        for transaction_key in transaction_keys {
            let transaction_digests =
                if let SequencedConsensusTransactionKey::External(ConsensusTransactionKey::Certificate(digest)) =
                    transaction_key
                {
                    vec![digest]
                } else {

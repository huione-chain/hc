...
impl AuthorityPerEpochStore {
    ...
    /// assignment table.
    pub(crate) fn get_input_object_keys(&self, key: &TransactionKey, objects: &[InputObjectKind]) -> BTreeSet<InputKey> {
    ...
    pub async fn notify_read_checkpoint_state_digests(
        ...
        self.checkpoint_state_notify_read
            .read(&checkpoints, |checkpoints| -> SuiResult<_> {
                Ok(self.tables()?.state_hash_by_checkpoint.multi_get(checkpoints)?)
            })
            .await
    }
    ...
    pub fn insert_pending_consensus_transactions(
        ...
        for transaction in transactions {
            if let ConsensusTransactionKind::CertifiedTransaction(cert) = &transaction.kind {
                ...
                self.pending_consensus_certificates.write().insert(*cert.digest());
            }
        }
        Ok(())
    }
    ...
    pub(crate) async fn process_consensus_transactions_and_commit_boundary<'a, C: CheckpointServiceNotify>(
        ...
        let (transactions_to_schedule, notifications, lock, final_round, consensus_commit_prologue_root) = self
            ...
            .await?;
        self.finish_consensus_certificate_process_with_batch(&mut output, &transactions_to_schedule)?;
    ...
    pub(crate) async fn process_consensus_transactions<C: CheckpointServiceNotify>(
        ...
        let mut shared_object_congestion_tracker = SharedObjectCongestionTracker::new(
            ...
            self.protocol_config().gas_budget_based_txn_cost_cap_factor_as_option(),
        );
        let mut shared_object_using_randomness_congestion_tracker = SharedObjectCongestionTracker::new(
            self.protocol_config().per_object_congestion_control_mode(),
            self.protocol_config().gas_budget_based_txn_cost_cap_factor_as_option(),
        );
    ...
    async fn process_consensus_transaction<C: CheckpointServiceNotify>(
        ...
        match &transaction {
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                ...
                if dkg_failed && self.randomness_state_enabled() && certificate.transaction_data().uses_randomness() {
                    debug!(
    ...
    pub fn process_pending_checkpoint(
        ...
        for (position_in_commit, (summary, transactions)) in content_info.into_iter().enumerate() {
            ...
            batch.insert_batch(&self.tables()?.builder_checkpoint_summary_v2, [(&sequence_number, summary)])?;
            batch.insert_batch(
...
impl GetSharedLocks for AuthorityPerEpochStore {
    fn get_shared_locks(&self, key: &TransactionKey) -> Result<Vec<(ObjectID, SequenceNumber)>, SuiError> {
        Ok(self.tables()?.assigned_shared_object_versions_v2.get(key)?.unwrap_or_default())
    }
}

...
impl VersionedProcessedMessage {
    ...
    pub fn merge(
        ...
        let (conf, msgs) = party.merge_v1(&messages.into_iter().map(|vm| vm.unwrap_v1()).collect::<Vec<_>>())?;
...
impl VersionedUsedProcessedMessages {
    fn complete_dkg<'a, Iter: Iterator<Item = &'a VersionedDkgConfirmation>>(
        ...
        party.complete_v1(msg, &confirmations.map(|vm| vm.unwrap_v1()).cloned().collect::<Vec<_>>(), rng)
    }
}
...
impl RandomnessManager {
    ...
    fn randomness_dkg_info_from_committee(
        committee: &Committee,
    ) -> Vec<(u16, AuthorityName, fastcrypto_tbls::ecies::PublicKey<bls12381::G2Element>, StakeUnit)> {
        committee
            .members()
            .map(|(name, stake)| {
                ...
                (index, *name, fastcrypto_tbls::ecies::PublicKey::from(pk), *stake)
            })
            .collect()
    }
}

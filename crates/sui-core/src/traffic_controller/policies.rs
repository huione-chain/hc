...
impl TrafficSketch {
    ...
    fn update_highest_rates(&mut self, key: &SketchKey, rate: f64) {
        match key.1 {
            ClientType::Direct => {
                Self::update_highest_rate(&mut self.highest_rates.direct, key.0, rate, self.highest_rates.capacity);
            }
            ClientType::ThroughFullnode => {
                Self::update_highest_rate(&mut self.highest_rates.proxied, key.0, rate, self.highest_rates.capacity);
            }
        }
    }
...
impl TrafficTally {
    pub fn new(
        ...
        Self { direct, through_fullnode, error_weight, spam_weight, timestamp: SystemTime::now() }
    }
}
...
impl FreqThresholdPolicy {
    pub fn new(
        ...
        Self { config, sketch, client_threshold, proxied_client_threshold }
    }

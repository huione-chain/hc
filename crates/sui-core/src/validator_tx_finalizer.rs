...
mod tests {
    ...
    async fn create_tx(
        ...
        let gas_object_ref = state.get_object(&gas_object_id).await.unwrap().unwrap().compute_object_reference();
        let tx_data =
    ...
    fn check_quorum_execution(
        ...
        let executed_weight: StakeUnit = clients
            ...
            .filter_map(|(name, client)| {
                client.authority.is_tx_already_executed(tx_digest).unwrap().then_some(auth_agg.committee.weight(name))
            })
            .sum();

...
impl ReadStore for RocksDbStore {
    fn get_checkpoint_by_digest(&self, digest: &CheckpointDigest) -> Result<Option<VerifiedCheckpoint>, StorageError> {
        self.checkpoint_store.get_checkpoint_by_digest(digest).map_err(Into::into)
    }
    ...
    fn get_checkpoint_by_sequence_number(
        ...
        self.checkpoint_store.get_checkpoint_by_sequence_number(sequence_number).map_err(Into::into)
    }
    ...
    fn get_full_checkpoint_contents_by_sequence_number(
        ...
        self.checkpoint_store.get_full_checkpoint_contents_by_sequence_number(sequence_number).map_err(Into::into)
    }
            ...
            .map(|contents| {
                ...
                for tx in contents.iter() {
                    if let (Some(t), Some(e)) = (
                        ...
                        return Result::<Option<FullCheckpointContents>, sui_types::storage::error::Error>::Ok(None);
                    }
                }
                Ok(Some(FullCheckpointContents::from_contents_and_execution_data(contents, transactions.into_iter())))
            })
            .transpose()
    ...
    fn get_committee(&self, epoch: EpochId) -> Result<Option<Arc<Committee>>, sui_types::storage::error::Error> {
    ...
    fn get_transaction(&self, digest: &TransactionDigest) -> Result<Option<Arc<VerifiedTransaction>>, StorageError> {
        self.cache_traits.transaction_cache_reader.get_transaction_block(digest).map_err(StorageError::custom)
    }

    fn get_transaction_effects(&self, digest: &TransactionDigest) -> Result<Option<TransactionEffects>, StorageError> {
        self.cache_traits.transaction_cache_reader.get_executed_effects(digest).map_err(StorageError::custom)
    }

    fn get_events(&self, digest: &TransactionEventsDigest) -> Result<Option<TransactionEvents>, StorageError> {
        self.cache_traits.transaction_cache_reader.get_events(digest).map_err(StorageError::custom)
    }
    ...
    fn get_checkpoint_contents_by_digest(
        ...
        self.checkpoint_store.get_checkpoint_contents(digest).map_err(sui_types::storage::error::Error::custom)
    }
    ...
    fn get_checkpoint_contents_by_sequence_number(
        ...
        match self.get_checkpoint_by_sequence_number(sequence_number) {
            Ok(Some(checkpoint)) => self.get_checkpoint_contents_by_digest(&checkpoint.content_digest),
            Ok(None) => Ok(None),
            Err(e) => Err(e),
        }
    }
...
impl ReadStore for RestReadStore {
    fn get_committee(&self, epoch: EpochId) -> sui_types::storage::error::Result<Option<Arc<Committee>>> {
...
impl RestStateReader for RestReadStore {
    ...
    fn get_lowest_available_checkpoint_objects(&self) -> sui_types::storage::error::Result<CheckpointSequenceNumber> {
        let highest_pruned_cp =
            self.state.get_object_cache_reader().get_highest_pruned_checkpoint().map_err(StorageError::custom)?;

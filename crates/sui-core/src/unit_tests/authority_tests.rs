...
async fn construct_shared_object_transaction_with_sequence_number(
    ...
    let (shared_object_id, shared_object) = {
        ...
        let mut shared_object = authority.get_object(&shared_object_id).await.unwrap().unwrap();
...
async fn test_dry_run_transaction_block() {
    ...
    let initial_shared_object_version = validator.get_object(&shared_object_id).await.unwrap().unwrap().version();
    ...
    let gas_object_version = fullnode.get_object(&gas_object_id).await.unwrap().unwrap().version();
    assert_eq!(gas_object_version, OBJECT_START_VERSION);
    let shared_object_version = fullnode.get_object(&shared_object_id).await.unwrap().unwrap().version();
...
async fn test_dev_inspect_object_by_bytes() {
    ...
    let created_object = validator.get_object(&created_object_id).await.unwrap().unwrap();
    ...
    // compare the bytes
    let updated_object = validator.get_object(&created_object_id).await.unwrap().unwrap();
...
async fn test_handle_transfer_transaction_bad_signature() {
    ...
    let object = authority_state.get_object(&object_id).await.unwrap().unwrap();
    ...
    let client = NetworkAuthorityClient::connect(server_handle.address()).await.unwrap();
...
async fn test_transfer_package() {
    ...
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
    let epoch_store = authority_state.load_epoch_store_one_call_per_task();
    let gas_object = authority_state.get_object(&object_id).await.unwrap().unwrap();
...
async fn test_immutable_gas() {
    ...
    let mut_object = authority_state.get_object(&mut_object_id).await.unwrap().unwrap();
    let transfer_transaction = init_transfer_transaction(
        &authority_state,
        sender,
        &sender_key,
...
async fn test_publish_dependent_module_ok() {
    ...
    assert!(authority.get_object(&dependent_module_id).await.unwrap().is_none());
...
async fn test_publish_non_existing_dependent_module() {
    ...
    assert_eq!(authority.get_object(&gas_payment_object_id).await.unwrap().unwrap().version(), gas_payment_object_ref.1);
}
...
async fn test_handle_move_transaction() {
    ...
    let created_obj = authority_state.get_object(&created_object_id).await.unwrap().unwrap();
...
async fn test_handle_confirmation_transaction_ok() {
    ...
    let old_account = authority_state.get_object(&object_id).await.unwrap().unwrap();
    ...
    let new_account = authority_state.get_object(&object_id).await.unwrap().unwrap();
...
async fn test_move_call_mutable_object_not_mutated() {
    ...
    let gas_version = authority_state.get_object(&gas_object_id).await.unwrap().unwrap().version();
    ...
    assert_eq!(authority_state.get_object(&new_object_id1).await.unwrap().unwrap().version(), next_object_version);
    assert_eq!(authority_state.get_object(&new_object_id2).await.unwrap().unwrap().version(), next_object_version);
}
...
async fn test_move_call_insufficient_gas() {
    ...
    let certified_transfer_transaction = init_certified_transfer_transaction(
        ...
        &sender_key,
        recipient,
        authority_state.get_object(&object_id).await.unwrap().unwrap().compute_object_reference(),
        authority_state.get_object(&gas_object_id1).await.unwrap().unwrap().compute_object_reference(),
        &authority_state,
    );
    let effects = authority_state
        .execute_certificate(&certified_transfer_transaction, &authority_state.epoch_store_for_testing())
    ...
    let obj_ref = authority_state.get_object(&object_id).await.unwrap().unwrap().compute_object_reference();

    let gas_ref = authority_state.get_object(&gas_object_id2).await.unwrap().unwrap().compute_object_reference();
    ...
    let obj = authority_state.get_object(&object_id).await.unwrap().unwrap();
...
async fn test_get_latest_parent_entry_genesis() {
    ...
    assert!(authority_state.get_object_or_tombstone(ObjectID::ZERO).await.unwrap().is_none());
}
...
async fn test_get_latest_parent_entry() {
    ...
    let obj_ref = authority_state.get_object_or_tombstone(new_object_id1).await.unwrap().unwrap();
    ...
    assert!(authority_state.get_object_or_tombstone(unknown_object_id).await.unwrap().is_none());

    // Check gas object is returned.
    let obj_ref = authority_state.get_object_or_tombstone(gas_object_id).await.unwrap().unwrap();
...
async fn test_account_state_ok() {
    ...
    authority_state.get_object(&object_id).await.unwrap().unwrap();
}
...
async fn test_account_state_unknown_account() {
    ...
    assert!(authority_state.get_object(&unknown_address).await.unwrap().is_none());
}
...
async fn test_genesis_sui_system_state_object() {
    ...
    let authority_state = TestAuthorityBuilder::new().build().await;
    let wrapper = authority_state.get_object(&SUI_SYSTEM_STATE_OBJECT_ID).await.unwrap().unwrap();
...
async fn test_transfer_sui_no_amount() {
    ...
    let new_balance =
        sui_types::gas::get_gas_balance(&authority_state.get_object(&gas_object_id).await.unwrap().unwrap()).unwrap();
...
async fn test_transfer_sui_with_amount() {
    ...
    let new_gas = authority_state.get_object(&effects.created()[0].0 .0).await.unwrap().unwrap();
...
async fn test_store_revert_transfer_sui() {
    ...
    assert_eq!(cache.get_object(&gas_object_id).unwrap().unwrap().owner, Owner::AddressOwner(sender),);
    assert_eq!(cache.get_latest_object_ref_or_tombstone(gas_object_id).unwrap().unwrap(), gas_object_ref);
...
async fn test_dynamic_field_struct_name_parsing() {
    ...
    assert_eq!(parse_type_tag("0x0::object_basics::Name").unwrap(), fields[0].name.type_)
}
...
async fn test_store_revert_remove_ofield() {
    ...
    reconfig_api.revert_state_update(&remove_ofield_digest).unwrap();
    reconfig_api.clear_state_end_of_epoch(&authority_state.execution_lock_for_reconfiguration().await);
...
async fn test_shared_object_transaction_ok() {
    ...
    let shared_object_version = authority.get_object(&shared_object_id).await.unwrap().unwrap().version();
...
async fn test_consensus_message_processed() {
    ...
    for _ in 0..50 {
        ...
        let effects2 = if send_first && rng.gen_bool(0.5) {
            ...
            authority2.get_transaction_cache_reader().get_executed_effects(transaction_digest).unwrap().unwrap()
        };
...
async fn test_gas_smashing() {
    ...
    async fn run_and_check(reference_gas_used: u64, coin_num: u64, budget: u64, success: bool) -> u64 {
        ...
        let balance =
            sui_types::gas::get_gas_balance(&state.get_object(&gas_coin_ids[0]).await.unwrap().unwrap()).unwrap();

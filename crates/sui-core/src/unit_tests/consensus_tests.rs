...
use sui_types::{
    ...
    multiaddr::Multiaddr,
    object::Object,
    transaction::{CallArg, CertifiedTransaction, ObjectArg, TransactionData, TEST_ONLY_GAS_UNIT_FOR_OBJECT_BASICS},
...
pub async fn test_certificates(authority: &AuthorityState, shared_object: Object) -> Vec<CertifiedTransaction> {
    ...
    for gas_object in test_gas_objects() {
        ...
        let gas_object = authority.get_object(&gas_object.id()).await.unwrap().unwrap();
...
pub fn make_consensus_adapter_for_test(
    ...
    Arc::new(ConsensusAdapter::new(
        Arc::new(SubmitDirectly { state: state.clone(), process_via_checkpoint, execute }),
        state.name,

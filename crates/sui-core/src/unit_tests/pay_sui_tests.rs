...
async fn test_pay_sui_success_one_input_coin() -> anyhow::Result<()> {
    ...
    let created_obj1 = res.authority_state.get_object(&created_obj_id1).await.unwrap().unwrap();
    let created_obj2 = res.authority_state.get_object(&created_obj_id2).await.unwrap().unwrap();
    let created_obj3 = res.authority_state.get_object(&created_obj_id3).await.unwrap().unwrap();

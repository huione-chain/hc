    ...
    F: FnOnce(&GasCostSummary, u64, u64) -> SuiResult,
{
    ...
    for coin_id in &gas_coin_ids {
        let coin_ref = authority_state.get_object(coin_id).await.unwrap().unwrap().compute_object_reference();
...
async fn touch_gas_coins(
    ...
    let gas_object_ref = authority_state.get_object(&gas_object_id).await.unwrap().unwrap().compute_object_reference();
    let rgp = authority_state.reference_gas_price_for_testing().unwrap();
...
async fn test_native_transfer_insufficient_gas_execution() {
    ...
    let gas_object = result.authority_state.get_object(&result.gas_object_id).await.unwrap().unwrap();

...
async fn do_transaction_test_impl(
    ...
    let gas_object1 = authority_state.get_object(&gas_object_id1).await.unwrap().unwrap();
    let gas_object2 = authority_state.get_object(&gas_object_id2).await.unwrap().unwrap();
...
async fn test_handle_soft_bundle_certificates() {
    ...
    let shared_object = {
        ...
        authority.get_object(&shared_object_id).await.unwrap().unwrap()
    };
    ...
    for i in 0..4 {
        let cert = {
            let gas_object_ref =
                authority.get_object(&gas_object_ids[i]).await.unwrap().unwrap().compute_object_reference();
...
async fn test_handle_soft_bundle_certificates_errors() {
    ...
    // The bundle should be rejected.
    {
        ...
        for i in 0..5 {
            let owned_object_ref =
                authority.get_object(&owned_objects[i].id()).await.unwrap().unwrap().compute_object_reference();
            let gas_object_ref =
                authority.get_object(&gas_objects[i].id()).await.unwrap().unwrap().compute_object_reference();
            let data = TransactionData::new_transfer(
                senders[i + 1].0,
                owned_object_ref,
                senders[i].0,
    ...
    {
        let owned_object_ref =
            authority.get_object(&owned_objects[5].id()).await.unwrap().unwrap().compute_object_reference();
        let gas_object_ref =
            authority.get_object(&gas_objects[5].id()).await.unwrap().unwrap().compute_object_reference();
        let data = TransactionData::new_transfer(
            senders[6].0,
            owned_object_ref,
            senders[5].0,
    ...
    // The bundle should be rejected.
    {
        let cert0 = {
            let gas_object_ref =
                authority.get_object(&gas_objects[6].id()).await.unwrap().unwrap().compute_object_reference();
            let data = TransactionData::new_move_call(
                senders[6].0,
                package.0,
                ident_str!("object_basics").to_owned(),
        ...
        let cert1 = {
            let gas_object_ref =
                authority.get_object(&gas_objects[7].id()).await.unwrap().unwrap().compute_object_reference();
            let data = TransactionData::new_move_call(
                senders[7].0,
                package.0,
                ident_str!("object_basics").to_owned(),
    ...
    // The bundle should be rejected.
    {
        let cert0 = {
            let gas_object_ref =
                authority.get_object(&gas_objects[8].id()).await.unwrap().unwrap().compute_object_reference();
            let data = TransactionData::new_move_call(
                senders[8].0,
                package.0,
                ident_str!("object_basics").to_owned(),
        ...
        let cert1 = {
            let gas_object_ref =
                authority.get_object(&gas_objects[9].id()).await.unwrap().unwrap().compute_object_reference();
            let data = TransactionData::new_move_call(
                senders[9].0,
                package.0,
                ident_str!("object_basics").to_owned(),

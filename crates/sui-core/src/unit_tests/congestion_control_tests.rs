...
impl TestSetup {
    ...
    async fn create_genesis_objects_for_new_authority_state(&self, objects: &[ObjectID]) -> Vec<Object> {
        ...
        genesis_objects.push(TestSetup::convert_to_genesis_obj(
            self.setup_authority_state.get_object(&self.package.0).await.unwrap().unwrap(),
        ));
        genesis_objects.push(TestSetup::convert_to_genesis_obj(
            self.setup_authority_state.get_object(&self.gas_object_id).await.unwrap().unwrap(),
        ));

        for obj in objects {
            genesis_objects.push(TestSetup::convert_to_genesis_obj(
                self.setup_authority_state.get_object(obj).await.unwrap().unwrap(),
            ));
        }
        genesis_objects
    }
...
async fn test_congestion_control_execution_cancellation() {
    ...
    register_fail_point_arg("initial_congestion_tracker", move || {
        Some(SharedObjectCongestionTracker::new_with_initial_value_for_test(
    ...
    let (congested_tx, effects) = update_objects(
        ...
        &authority_state.get_object(&owned_object.0).await.unwrap().unwrap().compute_object_reference(),
    )
    .await;

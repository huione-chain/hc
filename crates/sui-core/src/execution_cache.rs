...
pub fn choose_execution_cache(config: &ExecutionCacheConfig) -> ExecutionCacheConfigType {
    ...
    if std::env::var(DISABLE_WRITEBACK_CACHE_ENV_VAR).is_ok() || matches!(config, ExecutionCacheConfig::PassthroughCache)
    {
...
pub fn build_execution_cache(
    ...
    ExecutionCacheTraitPointers::new(ProxyCache::new(epoch_start_config, store.clone(), execution_cache_metrics).into())
}
...
pub fn build_execution_cache_from_env(
    ...
    if std::env::var(DISABLE_WRITEBACK_CACHE_ENV_VAR).is_ok() {
        ...
        ExecutionCacheTraitPointers::new(WritebackCache::new(store.clone(), execution_cache_metrics).into())
    }
}
...
pub trait ObjectCacheRead: Send + Sync {
    ...
    fn multi_input_objects_available(
        ...
        let unversioned_results = keys_without_version.into_iter().map(|(idx, key)| {
            (idx, match self.get_latest_object_ref_or_tombstone(key.id()).expect("read cannot fail") {
                None => false,
    ...
    fn have_deleted_owned_object_at_version_or_after(
        ...
        match self.get_latest_marker(object_id, epoch_id)? {
            Some((marker_version, MarkerValue::OwnedDeleted)) if marker_version >= version => Ok(true),
            _ => Ok(false),
        }
    }
...
pub trait TransactionCacheRead: Send + Sync {
    ...
    fn get_transaction_block(&self, digest: &TransactionDigest) -> SuiResult<Option<Arc<VerifiedTransaction>>> {
        self.multi_get_transaction_blocks(&[*digest])
            .map(|mut blocks| blocks.pop().expect("multi-get must return correct number of items"))
    }
    ...
    fn is_tx_already_executed(&self, digest: &TransactionDigest) -> SuiResult<bool> {
        self.multi_get_executed_effects_digests(&[*digest])
            .map(|mut digests| digests.pop().expect("multi-get must return correct number of items").is_some())
    }
    ...
    fn get_executed_effects(&self, digest: &TransactionDigest) -> SuiResult<Option<TransactionEffects>> {
        self.multi_get_executed_effects(&[*digest])
            .map(|mut effects| effects.pop().expect("multi-get must return correct number of items"))
    }
    ...
    fn get_events(&self, digest: &TransactionEventsDigest) -> SuiResult<Option<TransactionEvents>> {
        self.multi_get_events(&[*digest])
            .map(|mut events| events.pop().expect("multi-get must return correct number of items"))
    }
    ...
    fn notify_read_executed_effects<'a>(
        ...
        async move {
            ...
            self.multi_get_effects(&digests)
                .map(|effects| effects.into_iter().map(|e| e.expect("digests must exist")).collect())
        }
        .boxed()
    }
}
...
pub trait StateSyncAPI: Send + Sync {
    ...
    fn multi_insert_transaction_and_effects(&self, transactions_and_effects: &[VerifiedExecutionData]) -> SuiResult;
}
    ...
    ($implementor: ident) => {
        impl ObjectStore for $implementor {
            ...
            fn get_object_by_key(
                ...
                ObjectCacheRead::get_object_by_key(self, object_id, version).map_err(StorageError::custom)
            }
        }
    ...
    ($implementor: ident) => {
        impl CheckpointCache for $implementor {
            ...
            fn deprecated_multi_get_transaction_checkpoint(
                ...
                self.store.deprecated_multi_get_transaction_checkpoint(digests)
            }
            ...
            fn deprecated_insert_finalized_transactions(
                ...
                self.store.deprecated_insert_finalized_transactions(digests, epoch, sequence)
            }
        }
        ...
        impl ExecutionCacheReconfigAPI for $implementor {
            ...
            fn reconfigure_cache<'a>(&'a self, _: &'a EpochStartConfiguration) -> BoxFuture<'a, ()> {
                ...
                panic!("reconfigure_cache should not be called on a {}", stringify!($implementor));
            }
        }
        ...
        impl StateSyncAPI for $implementor {
            fn insert_transaction_and_effects(
                ...
                Ok(self.store.insert_transaction_and_effects(transaction, transaction_effects)?)
            }
            ...
            fn multi_insert_transaction_and_effects(
                ...
                Ok(self.store.multi_insert_transaction_and_effects(transactions_and_effects.iter())?)
            }
        }

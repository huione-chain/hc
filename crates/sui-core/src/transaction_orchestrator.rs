    ...
    OnsiteReconfigObserver: ReconfigObserver<A>,
{
    pub fn new(
        ...
        let quorum_driver_handler = Arc::new(
            QuorumDriverHandlerBuilder::new(validators, Arc::new(QuorumDriverMetrics::new(prometheus_registry)))
    ...
    A: AuthorityAPI + Send + Sync + 'static + Clone,
{
    ...
    async fn wait_for_finalized_tx_executed_locally_with_timeout(
        ...
        .await
        {
            ...
            Ok(Err(err)) => {
                debug!(?tx_digest, "Waiting for finalized tx to be executed locally failed with error: {:?}", err);
                metrics.local_execution_failure.inc();
                Err(SuiError::TransactionOrchestratorLocalExecutionError { error: err.to_string() })
            }
            Ok(Ok(_)) => {

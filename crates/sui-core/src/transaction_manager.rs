...
impl TransactionManager {
    ...
    fn enqueue_impl(
        ...
        let certs: Vec<_> = certs
            ...
            .filter(|(cert, _)| {
                ...
                if self
                    ...
                    .unwrap_or_else(|err| panic!("Failed to check if tx is already executed: {:?}", err))
                {
                    self.metrics
        ...
        let certs: Vec<_> = certs
            ...
            .map(|(cert, fx_digest)| {
                ...
                let mut input_object_keys = epoch_store.get_input_object_keys(&cert.key(), &input_object_kinds);
    ...
    pub(crate) fn check_execution_overload(
        ...
        for (object_id, queue_len, txn_age) in self.objects_queue_len_and_age(
            tx_data.transaction_data().input_objects()?.into_iter().map(|r| r.object_id()).collect(),
        ) {

...
impl CachedCommittedData {
    fn new() -> Self {
        let object_cache = MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
        let object_by_id_cache = MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
        let marker_cache = MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
        let transactions = MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
        let transaction_effects = MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
        let transaction_events = MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
        let executed_effects_digests =
            MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
        let transaction_objects = MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
...
impl WritebackCache {
    pub fn new(store: Arc<AuthorityStore>, metrics: Arc<ExecutionCacheMetrics>) -> Self {
        let packages = MokaCache::builder().max_capacity(MAX_CACHE_SIZE).max_capacity(MAX_CACHE_SIZE).build();
        Self {
    ...
    async fn write_object_entry(&self, object_id: &ObjectID, version: SequenceNumber, object: ObjectEntry) {
        ...
        self.cached
            .object_by_id_cache
            .insert(*object_id, Arc::new(Mutex::new(LatestObjectCacheEntry::Object(version, object.clone()))));
    ...
    fn get_object_impl(&self, request_type: &'static str, id: &ObjectID) -> SuiResult<Option<Object>> {
    ...
    async fn write_transaction_outputs(&self, epoch_id: EpochId, tx_outputs: Arc<TransactionOutputs>) -> SuiResult {
    ...
    #[instrument(level = "debug", skip_all)]
    async fn commit_transaction_outputs(&self, epoch: EpochId, digests: &[TransactionDigest]) -> SuiResult {
        ...
        self.store.write_transaction_outputs(epoch, &all_outputs).await?;
...
impl ObjectCacheRead for WritebackCache {
    ...
    fn get_object_by_key(&self, object_id: &ObjectID, version: SequenceNumber) -> SuiResult<Option<Object>> {
        match self.get_object_by_key_cache_only(object_id, version) {
            ...
            CacheResult::NegativeHit => Ok(None),
            CacheResult::Miss => Ok(self.record_db_get("object_by_version").get_object_by_key(object_id, version)?),
        }
    }
    ...
    fn object_exists_by_key(&self, object_id: &ObjectID, version: SequenceNumber) -> SuiResult<bool> {
        match self.get_object_by_key_cache_only(object_id, version) {
            ...
            CacheResult::Miss => self.record_db_get("object_by_version").object_exists_by_key(object_id, version),
        }
    }
    ...
    fn multi_object_exists_by_key(&self, object_keys: &[ObjectKey]) -> SuiResult<Vec<bool>> {
        do_fallback_lookup(
            ...
            |remaining| {
                self.record_db_multi_get("object_by_version", remaining.len()).multi_object_exists_by_key(remaining)
            },
        )
    }
    ...
    fn get_latest_object_ref_or_tombstone(&self, object_id: ObjectID) -> SuiResult<Option<ObjectRef>> {
        match self.get_object_entry_by_id_cache_only("latest_objref_or_tombstone", &object_id) {
            ...
            CacheResult::NegativeHit => Ok(None),
            CacheResult::Miss => {
                self.record_db_get("latest_objref_or_tombstone").get_latest_object_ref_or_tombstone(object_id)
            }
        }
    }
    ...
    fn get_latest_object_or_tombstone(
        ...
        match self.get_object_entry_by_id_cache_only("latest_object_or_tombstone", &object_id) {
            ...
            CacheResult::NegativeHit => Ok(None),
            CacheResult::Miss => {
                self.record_db_get("latest_object_or_tombstone").get_latest_object_or_tombstone(object_id)
            }
        }
    }
    ...
    fn get_marker_value(
        ...
        match self.get_marker_value_cache_only(object_id, version, epoch_id) {
            ...
            CacheResult::NegativeHit => Ok(None),
            CacheResult::Miss => self.record_db_get("marker_by_version").get_marker_value(object_id, &version, epoch_id),
        }
    }
    ...
    fn get_latest_marker(
        ...
        match self.get_latest_marker_value_cache_only(object_id, epoch_id) {
            ...
            CacheResult::Miss => self.record_db_get("marker_latest").get_latest_marker(object_id, epoch_id),
        }
    }
...
impl TransactionCacheRead for WritebackCache {
    ...
    fn multi_get_effects(&self, digests: &[TransactionEffectsDigest]) -> SuiResult<Vec<Option<TransactionEffects>>> {
        do_fallback_lookup(
            ...
            |remaining| {
                self.record_db_multi_get("transaction_effects", remaining.len()).multi_get_effects(remaining.iter())
            },
        )
    }
...
impl ExecutionCacheWrite for WritebackCache {
    fn acquire_transaction_locks<'a>(
        ...
        self.object_locks.acquire_transaction_locks(self, epoch_store, owned_input_objects, transaction).boxed()
    }

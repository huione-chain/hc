...
impl ObjectLocks {
    ...
    pub(crate) async fn acquire_transaction_locks(
        ...
        epoch_store.tables()?.write_transaction_locks(transaction, locks_to_write.iter().cloned())?;
...
mod tests {
    ...
    async fn test_transaction_locks_are_exclusive() {
        ...
        Scenario::iterate(|mut s| async move {
            ...
            s.cache.acquire_transaction_locks(&s.epoch_store, &[new1, new2], tx2.clone()).await.unwrap_err();

            // new3 is lockable, but new2 is not, so this should fail
            s.cache.acquire_transaction_locks(&s.epoch_store, &[new3, new2], tx2.clone()).await.unwrap_err();

            // new3 is unlocked
            s.cache.acquire_transaction_locks(&s.epoch_store, &[new3], tx2).await.expect("new3 should be unlocked");
        })
        .await;
    }
    ...
    async fn test_transaction_locks_are_durable() {
        ...
        Scenario::iterate(|mut s| async move {
            ...
            s.cache.acquire_transaction_locks(&s.epoch_store, &[new1, old2], tx.clone()).await.unwrap_err();
    ...
    async fn test_acquire_transaction_locks_revert() {
        ...
        Scenario::iterate(|mut s| async move {
            ...
            // fails because we are referring to an old object
            s.cache.acquire_transaction_locks(&s.epoch_store, &[new1, old2], tx).await.unwrap_err();
    ...
    async fn test_acquire_transaction_locks_is_sync() {
        ...
        Scenario::iterate(|mut s| async move {
            ...
            s.cache.acquire_transaction_locks(&s.epoch_store, &objects, tx2).now_or_never().unwrap().unwrap();
        })
        .await;
    }
}

...
impl Scenario {
    ...
    pub fn reset_cache(&mut self) {
        self.cache = Arc::new(WritebackCache::new(self.store.clone(), self.cache.metrics.clone()));
    ...
    pub fn assert_live(&self, short_ids: &[u32]) {
        for short_id in short_ids {
            ...
            assert_eq!(self.cache().get_object_by_key(id, version).unwrap().unwrap(), *expected);
            assert_eq!(self.cache().get_object(&expected.id()).unwrap().unwrap(), *expected);
    ...
    pub fn assert_received(&self, short_ids: &[u32]) {
        for short_id in short_ids {
            let id = self.id_map.get(short_id).expect("no such object");
            let object = self.objects.get(id).expect("no such object");
            assert_eq!(self.cache().get_object_by_key(id, object.version()).unwrap().unwrap(), *object);
            assert!(self.cache().have_received_object_at_version(id, object.version(), 1).unwrap());
        }
    }
    ...
    pub fn assert_not_exists(&self, short_ids: &[u32]) {
        for short_id in short_ids {
            ...
            assert!(self.cache().get_object(id).unwrap().is_none(), "object exists in cache");
        }
    }
...
async fn test_committed() {
    ...
    Scenario::iterate(|mut s| async move {
        ...
        s.assert_live(&[1, 2]);
        s.assert_dirty(&[1, 2]);
        s.cache().commit_transaction_outputs(1, &[tx]).await.expect("commit failed");
...
async fn test_extra_outputs() {
    ...
    Scenario::iterate(|mut s| async move {
        ...
        let fx = s.cache.get_executed_effects(&tx).unwrap().unwrap();
        let events_digest = fx.events_digest().unwrap();
        assert!(s.cache.get_events(events_digest).unwrap().is_none(), "empty events should be none");
    ...
    Scenario::iterate(|mut s| async move {
        let check_all_versions = |s: &Scenario| {
            for i in 1u64..=3 {
                ...
                assert_eq!(s.cache().find_object_lt_or_eq_version(s.obj_id(1), v).unwrap().unwrap().version(), v);
            }
        };
...
async fn test_lt_or_eq_caching() {
    ...
    Scenario::iterate(|mut s| async move {
        ...
        let check_version = |lookup_version: u64, expected_version: u64| {
            ...
            assert_eq!(
                s.cache().find_object_lt_or_eq_version(s.obj_id(1), lookup_version).unwrap().unwrap().version(),
                expected_version
            );
        };
        ...
        assert!(s.cache().find_object_lt_or_eq_version(s.obj_id(1), 0.into()).unwrap().is_none());
...
async fn test_lt_or_eq_with_cached_tombstone() {
    ...
    Scenario::iterate(|mut s| async move {
        ...
        let check_version = |lookup_version: u64, expected_version: Option<u64>| {
            let lookup_version = SequenceNumber::from_u64(lookup_version);
            assert_eq!(
                s.cache().find_object_lt_or_eq_version(s.obj_id(1), lookup_version).unwrap().map(|v| v.version()),
...
async fn test_write_transaction_outputs_is_sync() {
    ...
    Scenario::iterate(|mut s| async move {
        ...
        s.cache.write_transaction_outputs(1, outputs).now_or_never().unwrap().unwrap();
    })
    .await;
}
...
async fn test_revert_state_update_mutated() {
    ...
    Scenario::iterate(|mut s| async move {
        let v1 = {
            ...
            let tx = s.do_tx().await;
            s.commit(tx).await.unwrap();
            s.cache().get_object(&s.obj_id(1)).unwrap().unwrap().version()
        };
        ...
        let version_after_revert = s.cache().get_object(&s.obj_id(1)).unwrap().unwrap().version();
    ...
    let t2 = {
        ...
        tokio::task::spawn(async move {
            for (_, _, parent_ref, child_id) in txns {
                ...
                loop {
                    let parent = cache.get_object_by_key(&parent_ref.0, parent_ref.1).unwrap();
                    if parent.is_none() {
                        tokio::task::yield_now().await;
                        continue;
                    }
    ...
    let t1 = {
        ...
        tokio::task::spawn(async move {
            ...
            for (tx1, _, a_ref, b_ref) in txns {
                results.push(cache.acquire_transaction_locks(&epoch_store, &[a_ref, b_ref], tx1).await);
                barrier.wait().await;
            }
            results
        })
    ...
    let t2 = {
        ...
        tokio::task::spawn(async move {
            ...
            for (_, tx2, a_ref, b_ref) in txns {
                results.push(cache.acquire_transaction_locks(&epoch_store, &[a_ref, b_ref], tx2).await);
                barrier.wait().await;
            }
            results
        })
        ...
        std::thread::spawn(move || {
            while start.elapsed() < Duration::from_secs(2) {
                let Some(latest_version) =
                    cache.cached.object_by_id_cache.get(&object_id).and_then(|e| e.lock().version())
                else {
                    continue;
                };

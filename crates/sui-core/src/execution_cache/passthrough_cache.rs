...
impl TransactionCacheRead for PassthroughCache {
    fn multi_get_transaction_blocks(
        ...
        Ok(self.store.multi_get_transaction_blocks(digests)?.into_iter().map(|o| o.map(Arc::new)).collect())
    }
...
impl ExecutionCacheWrite for PassthroughCache {
    ...
    fn write_transaction_outputs<'a>(
        ...
        async move {
            ...
            //    tx effects exist.
            self.store.check_owned_objects_are_live(&tx_outputs.locks_to_delete)?;

            self.store.write_transaction_outputs(epoch_id, &[tx_outputs]).await?;
    ...
    fn acquire_transaction_locks<'a>(
        ...
        self.store.acquire_transaction_locks(epoch_store, owned_input_objects, transaction).boxed()
    }
}

...
use crate::{
    authority::{authority_per_epoch_store::AuthorityPerEpochStore, authority_store_tables::LiveObject, AuthorityStore},
...
impl IndexStoreTables {
    fn open<P: Into<PathBuf>>(path: P) -> Self {
        IndexStoreTables::open_tables_read_write(path.into(), MetricConf::new("rest-index"), None, None)
    }
        ...
        std::thread::scope(|s| -> Result<(), StorageError> {
            ...
            for index in 0u8..(1 << BITS) {
                ...
                threads.push(s.spawn(move || {
                    this.live_object_set_index_task(index, BITS, authority_store, coin_index, epoch_store, package_store)
                }));
            }
    ...
    fn live_object_set_index_task(
        ...
        let mut resolver = epoch_store.executor().type_layout_resolver(Box::new(package_store));
            ...
            .filter_map(LiveObject::to_normal)
        {
            ...
            match object.owner {
                ...
                Owner::ObjectOwner(parent) => {
                    if let Some(field_info) = try_create_dynamic_field_info(&object, resolver.as_mut())? {
        ...
        let tables = {
            ...
            if tables.needs_to_do_initialization() {
                let mut tables = if tables.needs_to_delete_old_db() {
                    drop(tables);
                    typed_store::rocks::safe_drop_db(path.clone()).expect("unable to destroy old rest-index db");

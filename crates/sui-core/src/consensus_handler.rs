...
impl ConsensusHandlerInitializer {
    ...
    pub fn new_for_testing(state: Arc<AuthorityState>, checkpoint_service: Arc<CheckpointService>) -> Self {
        Self {
            state: state.clone(),
            checkpoint_service,
...
impl<C> ConsensusHandler<C> {
    pub fn new(
        ...
        let transaction_scheduler = AsyncTransactionScheduler::start(transaction_manager, epoch_store.clone());
        Self {
            epoch_store,
...
impl<C: CheckpointServiceNotify + Send + Sync> ConsensusHandler<C> {
    ...
    async fn handle_consensus_output(&mut self, consensus_output: impl ConsensusOutputAPI) {
        ...
        assert!(self.epoch_store.protocol_config().consensus_order_end_of_epoch_last());
...

        {
            ...
            for (authority_index, authority_transactions) in consensus_output.transactions() {
                ...
                for (transaction, serialized_len) in authority_transactions {
                    ...
                    if matches!(&transaction.kind, ConsensusTransactionKind::CertifiedTransaction(_)) {
                        self.last_consensus_stats.stats.inc_num_user_transactions(authority_index as usize);
                    }
                    if let ConsensusTransactionKind::RandomnessStateUpdate(randomness_round, _) = &transaction.kind {
        ...
        self.throughput_calculator.add_transactions(timestamp, transactions_to_schedule.len() as u64);
        ...
        self.transaction_scheduler.schedule(transactions_to_schedule).await;
    }
}
...
impl AsyncTransactionScheduler {
    pub fn start(transaction_manager: Arc<TransactionManager>, epoch_store: Arc<AuthorityPerEpochStore>) -> Self {
    ...
    pub fn new(
        ...
        let handle = spawn_monitored_task!(async move {
            ...
            while let Some(consensus_output) = receiver.recv().await {
                ...
                consensus_handler.handle_consensus_output(consensus_output).await;
    ...
    pub async fn test_consensus_handler() {
        ...
        for (i, transaction) in transactions.iter().enumerate() {
            let transaction_bytes: Vec<u8> =
                bcs::to_bytes(&ConsensusTransaction::new_certificate_message(&state.name, transaction.clone())).unwrap();
        ...
        consensus_handler.handle_consensus_output(committed_sub_dag.clone()).await;

...
impl SuiTxValidator {
    pub fn new(
        ...
        Self { epoch_store, checkpoint_service, _transaction_manager: transaction_manager, metrics }
    }

    fn validate_transactions(&self, txs: Vec<ConsensusTransactionKind>) -> Result<(), eyre::Report> {
        ...
        for ckpt in ckpt_messages {
            self.checkpoint_service.notify_checkpoint_signature(&self.epoch_store, &ckpt)?;
        }
...
fn tx_from_bytes(tx: &[u8]) -> Result<ConsensusTransaction, eyre::Report> {
    bcs::from_bytes::<ConsensusTransaction>(tx).wrap_err("Malformed transaction (failed to deserialize)")
}
...
impl TransactionVerifier for SuiTxValidator {
    fn verify_batch(&self, batch: &[&[u8]]) -> Result<(), ValidationError> {
        ...
        let txs = batch
            .iter()
            .map(|tx| {
                tx_from_bytes(tx).map(|tx| tx.kind).map_err(|e| ValidationError::InvalidTransaction(e.to_string()))
            })

    ...
    pub fn compute_source_coverage(&self, file_path: &Path) -> SourceCoverage {
        ...
        for (_, fn_cov) in self.uncovered_locations.iter() {
            for span in merge_spans(fn_cov.clone()).into_iter() {
                ...
                let segments = uncovered_segments.entry(start_line).or_insert_with(Vec::new);
                if start_line == end_line {
                    ...
                    segments.push(AbstractSegment::BoundedLeft { start: start_loc.column.0 });
                    ...
                    last_segment.push(AbstractSegment::BoundedRight { end: end_loc.column.0 });
                }
            }
        }

...
fn move_check_testsuite(path: &Path) -> datatest_stable::Result<()> {
    ...
    let config =
        PackageConfig { flavor, edition, is_dependency: false, warning_filter: WarningFilters::new_for_source() };
...
fn testsuite(path: &Path, mut config: PackageConfig, lint: bool) -> datatest_stable::Result<()> {
    ...
    if path.with_extension(TEST_EXT).exists() {
        ...
        config.warning_filter.union(&WarningFilters::unused_warnings_filter_for_test());

...
const FREEZE_FUNCTIONS: &[(&str, &str, &str)] =
    &[(SUI_PKG_NAME, TRANSFER_MOD_NAME, PUBLIC_FREEZE_FUN), (SUI_PKG_NAME, TRANSFER_MOD_NAME, FREEZE_FUN)];
...
impl TypingVisitorConstructor for FreezeWrappedVisitor {
    ...
    fn context<'a>(env: &'a mut CompilationEnv, program: &T::Program) -> Self::Context<'a> {
        Context { env, program_info: program.info.clone(), wrapping_fields: WrappingFields::new() }
    }
}
        ...
        if let E::ModuleCall(fun) = &exp.exp.value {
                ...
                .any(|(addr, module, fname)| fun.module.value.is(*addr, *module) && &fun.name.value().as_str() == fname)
            {
                ...
                if let Some(wrapping_field_info) = self.find_wrapping_field_loc(mident, sname) {
                    add_diag(self.env, fun.arguments.exp.loc, sname.value(), wrapping_field_info);
                }
            }
        }
...
impl<'a> Context<'a> {
    ...
    fn find_wrapping_field_loc(&mut self, mident: E::ModuleIdent, sname: P::DatatypeName) -> Option<WrappingFieldInfo> {
...
fn add_diag(env: &mut CompilationEnv, freeze_arg_loc: Loc, frozen_struct_name: Symbol, info: WrappingFieldInfo) {

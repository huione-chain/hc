...
const COLLECTION_TYPES: &[(&str, &str, &str)] = &[
    ...
    (SUI_PKG_NAME, OBJECT_TABLE_MOD_NAME, OBJECT_TABLE_STRUCT_NAME),
    (SUI_PKG_NAME, LINKED_TABLE_MOD_NAME, LINKED_TABLE_STRUCT_NAME),
...
impl TypingVisitorContext for Context<'_> {
    fn visit_exp_custom(&mut self, exp: &T::Exp) -> bool {
        ...
        if let E::BinopExp(_, op, t, _) = &exp.exp.value {
            ...
            if let Some((caddr, cmodule, cname)) = COLLECTION_TYPES
                .iter()
                .find(|(caddr, cmodule, cname)| mident.value.is(*caddr, *cmodule) && sname.value().as_str() == *cname)
            {
                let msg =
                    format!("Comparing collections of type '{caddr}::{cmodule}::{cname}' may yield unexpected result.");
                let note_msg =

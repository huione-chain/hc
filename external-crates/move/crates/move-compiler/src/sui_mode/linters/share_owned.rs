...
const SHARE_FUNCTIONS: &[(&str, &str, &str)] =
    &[(SUI_PKG_NAME, TRANSFER_MOD_NAME, PUBLIC_SHARE_FUN), (SUI_PKG_NAME, TRANSFER_MOD_NAME, SHARE_FUN)];
...
impl<'a> ShareOwnedVerifierAI<'a> {
    ...
    fn can_hold_obj_base(&self, sp!(_, bt_): &BaseType) -> bool {
        match bt_ {
            ...
            BaseType_::Apply(_, sp!(_, tn), _) if tn.is(SUI_ADDR_NAME, TX_CONTEXT_MODULE_NAME, TX_CONTEXT_TYPE_NAME) => {
                false
            }

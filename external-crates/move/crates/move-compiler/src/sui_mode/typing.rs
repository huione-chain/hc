...
fn struct_def(context: &mut Context, name: DatatypeName, sdef: &N::StructDefinition) {
    ...
    if let Some(loc) = invalid_first_field {
        ...
        context.env.add_diag(invalid_object_id_field_diag(key_loc, loc, name));
        return;
    };

    let (_, id_field_type) = fields.get_(&ID_FIELD_NAME).unwrap();
    let id_field_loc = fields.get_loc_(&ID_FIELD_NAME).unwrap();
    if !id_field_type.value.is(SUI_ADDR_NAME, OBJECT_MODULE_NAME, UID_TYPE_NAME) {
...
fn init_visibility(context: &mut Context, name: FunctionName, visibility: Visibility, entry: Option<Loc>) {
    match visibility {
        Visibility::Public(loc) | Visibility::Friend(loc) | Visibility::Package(loc) => context.env.add_diag(diag!(
            INIT_FUN_DIAG,
...
fn init_signature(context: &mut Context, name: FunctionName, signature: &FunctionSignature) {
    ...
    if tx_ctx_kind == TxContextKind::None {
        ...
        context.env.add_diag(diag!(INIT_FUN_DIAG, (name.loc(), "Invalid 'init' function declaration"), (last_loc, msg),))
    }
...
fn is_mut_random(param_ty: &Type) -> bool {
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n_), _) => n_.is(SUI_ADDR_NAME, RANDOMNESS_MODULE_NAME, RANDOMNESS_STATE_TYPE_NAME),
        Type_::Unit | Type_::Param(_) | Type_::Var(_) | Type_::Anything | Type_::UnresolvedError | Type_::Fun(_, _) => {
            false
        }
    }
...
fn is_entry_receiving_ty(param_ty: &Type) -> bool {
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n), targs) if n.is(SUI_ADDR_NAME, TRANSFER_MODULE_NAME, RECEIVING_TYPE_NAME) => {
...
fn is_entry_primitive_ty(param_ty: &Type) -> bool {
    ...
    match &param_ty.value {
        ...
        Type_::Apply(_, sp!(_, n), targs) if n.is(STD_ADDR_NAME, OPTION_MODULE_NAME, OPTION_TYPE_NAME) => {
            debug_assert!(targs.len() == 1);
            is_entry_primitive_ty(&targs[0])
        }
...
fn entry_return(
    ...
    match return_type_ {
        ...
        Type_::Ref(_, _) => {
            ...
            context.env.add_diag(diag!(ENTRY_FUN_SIGNATURE_DIAG, (entry_loc, fmsg), (*tloc, tmsg)))
        }
        Type_::Param(tp) => {
...
fn exp(context: &mut Context, e: &T::Exp) {
    match &e.exp.value {
        T::UnannotatedExp_::ModuleCall(mcall) => {
            ...
            if module.value.is(SUI_ADDR_NAME, EVENT_MODULE_NAME) && name.value() == EVENT_FUNCTION_NAME {
...
fn otw_special_cases(context: &Context) -> bool {
    BRIDGE_SUPPORTED_ASSET.iter().any(|token| context.current_module().value.is(BRIDGE_ADDR_NAME, token))
        || context.current_module().value.is(SUI_ADDR_NAME, SUI_MODULE_NAME)
}
...
fn check_event_emit(context: &mut Context, loc: Loc, mcall: &ModuleCall) {
    ...
    if !is_defined_in_current_module {
        ...
        context.env.add_diag(diag!(EVENT_EMIT_CALL_DIAG, (loc, msg), (first_ty.loc, ty_msg)));
    }
}
...
fn check_private_transfer(context: &mut Context, loc: Loc, mcall: &ModuleCall) {
    let ModuleCall { module, name, type_arguments, .. } = mcall;
    let current_module = context.current_module();
    if current_module.value.is(SUI_ADDR_NAME, TRANSFER_FUNCTION_NAME) {

...
impl<'env> Context<'env> {
    fn new(compilation_env: &'env mut CompilationEnv, prog: &G::Program) -> Self {
        ...
        Self { env: compilation_env, constants }
    }
...
fn construct_module_test_plan(
    ...
    let tests: BTreeMap<_, _> = module
        ...
        .filter_map(|(loc, fn_name, func)| {
            build_test_info(context, loc, fn_name, func).map(|test_case| (fn_name.to_string(), test_case))
        })
        .collect();
...
fn build_test_info<'func>(
    ...
    let (test_attribute, is_random_test) = match (test_attribute_opt, random_test_attribute_opt) {
        (None, None) => {
            ...
            if let Some(abort_attribute) = abort_attribute_opt {
                ...
                context
                    .env
                    .add_diag(diag!(Attributes::InvalidUsage, (fn_loc, fn_msg), (abort_attribute.loc, abort_msg),))
            }
            return None;
        }
    ...
    if is_random_test && arguments.is_empty() {
        ...
        context.env.add_diag(diag!(Attributes::InvalidTest, (test_attribute.loc, msg), (fn_loc, IN_THIS_TEST_MSG),));
        return None;
    }
    ...
    match expected_attr {
        ...
        EA::Parameterized(sp!(_, nm), attrs) => {
            ...
            if expected_failure_kind_vec.len() != 1 {
                ...
                context.env.add_diag(diag!(Attributes::InvalidValue, (*aloc, invalid_attr_msg)));
                return None;
            }
            ...
            for (_, (loc, _)) in attrs {
                ...
                context.env.add_diag(diag!(UnusedItem::Attribute, (loc, msg)));
            }
...
fn convert_constant_value_u64_constant_or_value(
    ...
    match constant {
        ...
        (cloc, None, _) => {
            ...
            context.env.add_diag(diag!(Attributes::InvalidValue, (vloc, INVALID_VALUE), (*cloc, msg),));
            None
        }
...
fn check_location<T>(context: &mut Context, loc: Loc, attr: &str, location: Option<T>) -> Option<T> {
    if location.is_none() {
        ...
        context.env.add_diag(diag!(Attributes::InvalidUsage, (loc, msg)));
    }
    location
}

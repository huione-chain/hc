...
impl Compiler {
    ...
    pub fn run<const TARGET: Pass>(
        ...
        let mut compilation_env = CompilationEnv::new(flags, visitors, save_hooks, package_configs, default_config);
        ...
        let (source_text, pprog, comments) = parse_program(&mut compilation_env, maps, targets, deps)?;
...
impl<const P: Pass> SteppedCompiler<P> {
    fn run_impl<const TARGET: Pass>(self) -> Result<SteppedCompiler<TARGET>, (Pass, Diagnostics)> {
        ...
        let Self { mut compilation_env, pre_compiled_lib, program } = self;
        let new_prog = run(&mut compilation_env, pre_compiled_lib.clone(), program.unwrap(), TARGET)?;

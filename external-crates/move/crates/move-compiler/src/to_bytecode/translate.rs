...
pub fn program(
    ...
    let G::Program { modules: gmodules, info: _ } = prog;
...
fn module(
    ...
    let (mut module, source_map) = match move_ir_to_bytecode::compiler::compile_module(ir_module, deps) {
        Ok(res) => res,
        Err(e) => {
            compilation_env.add_diag(diag!(Bug::BytecodeGeneration, (ident_loc, format!("IR ERROR: {}", e))));
            return None;
        }
    };
...
fn function(
    ...
    let ir_function = IR::Function_ { visibility: v, is_entry: entry.is_some(), signature, body };

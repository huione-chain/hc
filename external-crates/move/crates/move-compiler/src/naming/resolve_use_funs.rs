...
impl<'env, 'info> Context<'env, 'info> {
    fn new(env: &'env mut CompilationEnv, info: &'info NamingProgramInfo, current_module: ModuleIdent) -> Self {
        Self { env, info, current_module }
    }
}
...
fn module(env: &mut CompilationEnv, info: &mut NamingProgramInfo, mident: ModuleIdent, mdef: &mut N::ModuleDefinition) {
...
fn function(context: &mut Context, function: &mut N::Function) {
    context.env.add_warning_filter_scope(function.warning_filter.clone());
    ...
    for (tn, methods) in &mut *resolved {
        *methods = std::mem::take(methods).filter_map(|method, mut nuf| {
            ...
            if is_valid {
                if let Some(public_loc) = nuf.is_public {
                    let defining_module = match &tn.value {
                        N::TypeName_::Multiple(_) => {
                            context.env.add_diag(ice!((tn.loc, "ICE tuple type should not be reachable from use fun")));
                            return None;
                        }
                ...
                context.env.add_diag(diag!(Declarations::InvalidUseFun, (loc, msg), (first_ty_loc, first_tn_msg),));
                None
            }
        });
    }
    ...
    for (method, implicit) in std::mem::take(implicit_candidates) {
        ...
        let Some((target_f, tn)) = is_valid_method(context, &target_m, target_f) else {
            if matches!(ekind, E::ImplicitUseFunKind::UseAlias { used: false }) {
                ...
                context.env.add_diag(diag!(UnusedItem::Alias, (method.loc, msg),))
            }
            continue;
        };
...
fn exp_dotted(context: &mut Context, sp!(_, ed_): &mut N::ExpDotted) {
    match ed_ {
        ...
        N::ExpDotted_::Dot(ed, _) | N::ExpDotted_::DotAutocomplete(_, ed) => exp_dotted(context, ed),

...
use crate::{
    ...
    shared::{ide::EllipsisMatchEntries, program_info::NamingProgramInfo, unique_map::UniqueMap, *},
...
impl<'env> Context<'env> {
    ...
    fn resolve_type_inner(&mut self, sp!(nloc, ma_): E::ModuleAccess, error_kind: ErrorKind) -> ResolvedType {
        ...
        match ma_ {
            EN::Name(sp!(_, n)) if n == symbol!("_") => {
                ...
                self.env.check_feature(current_package, FeatureGate::TypeHoles, nloc);
                ResolvedType::Hole
            }
    ...
    fn resolve_use_fun_function(&mut self, sp!(mloc, ma_): E::ModuleAccess) -> ResolvedUseFunFunction {
        ...
        match ma_ {
            ...
            EA::Variant(_, _) => {
                self.env.add_diag(ice!((mloc, "Tried to resolve variant '{}' as a function in current scope"),));
                ResolvedUseFunFunction::Unbound
            }
        }
    }
    ...
    fn resolve_datatype_constructor(&mut self, ma: E::ModuleAccess, verb: &str) -> Option<ResolvedConstructor> {
        ...
        match self.resolve_type_for_constructor(ma) {
            ...
            rt @ (ResolvedType::BuiltinType(_) | ResolvedType::TParam(_, _) | ResolvedType::Hole) => {
                ...
                self.env.add_diag(diag!(NameResolution::NamePositionMismatch, (ma.loc, msg), (rtloc, rtmsg)));
                None
            }
            ResolvedType::ModuleType(module_type) => {
    ...
    fn resolve_term(&mut self, sp!(mloc, ma_): E::ModuleAccess) -> ResolvedTerm {
        match ma_ {
            ...
            ma_ @ E::ModuleAccess_::Variant(_, _) => {
                self.env.check_feature(self.current_package, FeatureGate::Enums, mloc);
    ...
    fn resolve_pattern_term(&mut self, sp!(mloc, ma_): E::ModuleAccess) -> ResolvedPatternTerm {
        match ma_ {
            E::ModuleAccess_::Name(name) if !is_constant_name(&name.value) => {
                self.env.add_diag(ice!((mloc, "This should have become a binder")));
    ...
    fn current_loop(&mut self, loc: Loc, usage: NominalBlockUsage) -> Option<BlockLabel> {
        let Some((_name, label, name_type)) = self.nominal_blocks.iter().rev().find(|(_, _, name_type)| {
            ...
            self.env.add_diag(diag!(TypeSafety::InvalidLoopControl, (loc, msg)));
            return None;
        };
    ...
    fn resolve_nominal_label(&mut self, usage: NominalBlockUsage, label: P::BlockLabel) -> Option<BlockLabel> {
        ...
        if let Some((label, block_type)) = label_opt {
            ...
            self.env.add_diag(diag!(NameResolution::UnboundLabel, (loc, msg)));
            None
        }
    }
...
fn module(context: &mut Context, ident: ModuleIdent, mdef: E::ModuleDefinition) -> N::ModuleDefinition {
...
fn use_funs(context: &mut Context, eufs: E::UseFuns) -> N::UseFuns {
    ...
    for (tn, method, nuf) in resolved_vec {
        ...
        if let Err((_, prev)) = methods.add(method, nuf) {
            ...
            context.env.add_diag(diag!(Declarations::DuplicateItem, (nuf_loc, msg), (prev, "Previously declared here"),))
        }
    }
    N::UseFuns {
...
fn explicit_use_fun(context: &mut Context, e: E::ExplicitUseFun) -> Option<(N::TypeName, Name, N::UseFun)> {
    ...
    let m_f_opt = match context.resolve_use_fun_function(function) {
        ...
        ResolvedUseFunFunction::Builtin(_) => {
            ...
            context.env.add_diag(diag!(Declarations::InvalidUseFun, (loc, msg)));
            None
        }
        ResolvedUseFunFunction::Unbound => {
            assert!(context.env.has_errors());
    ...
    let tn_opt = match tn_opt {
        ...
        ResolvedType::Hole => {
            ...
            context.env.add_diag(diag!(Declarations::InvalidUseFun, (loc, msg), (ty_loc, tmsg)));
            None
        }
        ResolvedType::TParam(tloc, tp) => {
            ...
            context.env.add_diag(diag!(Declarations::InvalidUseFun, (loc, msg,), (tloc, tmsg)));
            None
        }
    };
    let tn_ = tn_opt?;
...
fn use_fun_module_defines(
    ...
    match rtype {
        ResolvedType::ModuleType(mtype) => {
            if specified.as_ref().is_some_and(|mident| mident == &mtype.mident()) {
                Ok(())
            } else {
        ...
        ResolvedType::TParam(_, _) | ResolvedType::Hole | ResolvedType::Unbound => {
            context.env.add_diag(ice!((*use_fun_loc, "Tried to validate use fun for invalid type")));
            Ok(())
        }
    }
}
    ...
    if !matches!(body.value, N::FunctionBody_::Native) {
        for tparam in &signature.type_parameters {
            if !context.used_fun_tparams.contains(&tparam.id) {
                ...
                context.env.add_diag(diag!(UnusedItem::FunTypeParam, (loc, msg)))
            }
        }
    }
    ...
    let ne_ = match e_ {
        ...
        EE::MethodCall(edot, n, is_macro, tys_opt, rhs) => match dotted(context, *edot) {
            ...
            Some(d) => {
                ...
                if is_macro.is_some() {
                    context.env.check_feature(context.current_package, FeatureGate::MacroFuns, eloc);
                }
        ...
        EL::Var(mut_, sp!(_, E::ModuleAccess_::Name(n)), None) => {
            ...
            if v.is_underscore() {
                ...
                if let Err((var, prev_loc)) = seen_locals.add(n, ()) {
                    ...
                    context.env.add_diag(diag!(Declarations::DuplicateItem, primary, secondary));
                }
...
fn check_mut_underscore(context: &mut Context, mut_: Option<Mutability>) {
    ...
    context.env.add_diag(diag!(NameResolution::InvalidMut, (loc, msg)));
}
    ...
    match context.resolve_call_subject(fun_name) {
        ResolvedCallSubject::Function(mf) => {
            ...
            if let Some(mloc) = is_macro {
                context.env.check_feature(context.current_package, FeatureGate::MacroFuns, mloc);
            }
            ...
            let builtin_ = match &bf.fun.value {
                B::Freeze(_) => {
                    ...
                    match tyargs_opt.as_deref() {
                        ...
                        Some(_tys) => {
                            context.env.add_diag(ice!((call_loc, "Builtin tyarg arity failure")));
                ...
                B::Assert(_) => {
                    ...
                    if args.value.len() == 1 && is_macro.is_some() {
                        context.env.check_feature(context.current_package, FeatureGate::CleverAssertions, subject_loc);
        ...
        ResolvedCallSubject::Constructor(_) => {
            context.env.check_feature(context.current_package, FeatureGate::PositionalFields, call_loc);
        ...
        ResolvedCallSubject::Var(var) => {
            context.env.check_feature(context.current_package, FeatureGate::Lambda, call_loc);
            ...
            if !var.value.is_syntax_identifier() {
                ...
                context.env.add_diag(diag!(TypeSafety::InvalidCallTarget, (var.loc, msg)));
                N::Exp_::UnresolvedError
            } else {
...
fn check_is_not_macro(context: &mut Context, is_macro: Option<Loc>, name: &str) {
    if let Some(mloc) = is_macro {
        ...
        context.env.add_diag(diag!(TypeSafety::InvalidCallTarget, (mloc, msg)));
    }
}
...
fn report_invalid_macro(context: &mut Context, is_macro: Option<Loc>, kind: &str) {
    if let Some(mloc) = is_macro {
        ...
        context.env.add_diag(diag!(NameResolution::PositionalCallMismatch, (mloc, msg)));
    }
}
...
fn exp_types_opt_with_arity_check(
    ...
    if args_len != arity {
        ...
        context.env.add_diag(diag!(diag_code, (msg_loc, msg), (tyarg_error_loc, targs_msg)));
    }
...
fn report_unused_local(context: &mut Context, sp!(loc, unused_): &N::Var) {
    ...
    context.env.add_diag(diag!(UnusedItem::Variable, (*loc, msg)));
}

...
pub(super) fn resolve_syntax_attributes(
    ...
    if !context.env.check_feature(context.current_package, FeatureGate::SyntaxMethods, attr_loc) {
        return None;
    }
    ...
    if let Some(macro_loc) = function.macro_ {
        ...
        context.env.add_diag(diag!(Declarations::InvalidSyntaxMethod, (attr_loc, msg), (macro_loc, fn_msg)));
        return None;
    }
...
fn prev_syntax_defn_error(
    ...
    context.env.add_diag(diag!(Declarations::InvalidAttribute, (sloc, msg), (prev.loc, prev_msg)));
}
                    ...
                    Attribute_::Name(name) => {
                        if let Some(kind) = attr_param_from_str(*argloc, name.value.as_str()) {
                            if let Some(prev_kind) = kinds.replace(kind) {
                                ...
                                env.add_diag(diag!(Declarations::InvalidAttribute, (loc, msg), (prev_kind.loc, prev)));
                            }
                        } else {
    ...
    let sk = match kind {
        SyntaxMethodPrekind_::Index => {
            if valid_imm_ref(subject_type) {
                ...
                context.env.add_diag(diag!(Declarations::InvalidAttribute, (sloc, msg), (subject_type.loc, ty_msg)));
                return None;
            }
        }
        SyntaxMethodPrekind_::For => {
            let msg = "'for' syntax attributes are not currently supported";
            context.env.add_diag(diag!(Declarations::InvalidAttribute, (sloc, msg),));
            return None;
        }
...
fn determine_subject_type_name(
    ...
    match ty_ {
        ...
        N::Type_::Param(param) => {
            ...
            context.env.add_diag(diag!(Declarations::InvalidSyntaxMethod, (*ann_loc, msg), (*loc, tmsg)));
            None
        }
...
fn valid_index_return_type(context: &mut Context, kind_loc: &Loc, sp!(tloc, type_): &N::Type) -> bool {
    match type_ {
        ...
        N::Type_::Unit => {
            ...
            context.env.add_diag(diag!(Declarations::InvalidSyntaxMethod, (*kind_loc, msg), (*tloc, tmsg)));
            false
        }
        N::Type_::Fun(_, _) => {
            let msg =
...
fn get_first_type(context: &mut Context, attr_loc: &Loc, fn_signature: &N::FunctionSignature) -> Option<N::Type> {
    if let Some((_, _, ty)) = fn_signature.parameters.first() {
        ...
        context.env.add_diag(diag!(Declarations::InvalidAttribute, (*attr_loc, msg)));
        None
    }
}

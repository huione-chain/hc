...
impl<'env, 'lexer, 'input> Context<'env, 'lexer, 'input> {
    fn new(env: &'env mut CompilationEnv, tokens: &'lexer mut Lexer<'input>, package_name: Option<Symbol>) -> Self {
        let stop_set = TokenSet::from([Tok::EOF]);
        Self { current_package: package_name, env, tokens, stop_set }
    }
...
fn parse_name_access_chain_<'a, F: Fn() -> &'a str>(
    ...
    if let Some(loc) = &is_macro {
        if !macros_allowed {
            ...
            context.env.add_diag(diag!(Syntax::InvalidName, (*loc, msg)));
            is_macro = None;
        }
    }
    ...
    while context.tokens.peek() == Tok::ColonColon {
        ...
        path.push_path_entry(name, tys, is_macro).into_iter().for_each(|diag| context.env.add_diag(diag));
    }
...
fn parse_bind(context: &mut Context) -> Result<Bind, Box<Diagnostic>> {
    ...
    let args = if context.tokens.peek() == Tok::LParen {
        ...
        context.env.check_feature(context.current_package, FeatureGate::PositionalFields, current_loc);
            ...
            Tok::Period => {
                ...
                match context.tokens.peek() {
                    Tok::NumValue | Tok::NumTypedValue
                        if context.env.check_feature(context.current_package, FeatureGate::PositionalFields, loc) =>
                    {
...
fn parse_type_(context: &mut Context, whitespace_sensitive_ty_args: bool) -> Result<Type, Box<Diagnostic>> {
    ...
    let t = match context.tokens.peek() {
        ...
        _ => {
            if context.at_stop_set() {
                context.env.add_diag(*unexpected_token_error(context.tokens, "a type name"));
...
fn parse_enum_variant_fields(context: &mut Context) -> Result<VariantFields, Box<Diagnostic>> {
    match context.tokens.peek() {
        Tok::LParen => {
            ...
            let loc = current_token_loc(context.tokens);
            context.env.check_feature(current_package, FeatureGate::PositionalFields, loc);
...
fn check_enum_visibility(visibility: Option<Visibility>, context: &mut Context) {
    ...
    match &visibility {
        Some(Visibility::Public(loc)) => {
            context.env.check_feature(current_package, FeatureGate::Enums, *loc);
        }
        vis => {
...
fn parse_postfix_ability_declarations(
    ...
    if postfix_ability_declaration {
        context.env.check_feature(context.current_package, FeatureGate::PostFixAbilities, has_location);
...
fn check_struct_visibility(visibility: Option<Visibility>, context: &mut Context) {
    ...
    if let Some(Visibility::Public(loc)) = &visibility {
        context.env.check_feature(current_package, FeatureGate::StructTypeVisibility, *loc);
    }
    ...
    let modules = match context.tokens.peek() {
        Tok::LBrace => {
            if in_migration_mode {
                ...
                context.env.add_diag(diag!(Migration::AddressRemove, (loc, "address decl")));
            }
            context.tokens.advance()?;
            let mut modules = vec![];
            loop {
            ...
            if in_migration_mode {
                let loc = context.tokens.current_token_loc();
                context.env.add_diag(diag!(Migration::AddressRemove, (loc, "close lbrace")));
            }
...
fn parse_friend_decl(attributes: Vec<Attributes>, context: &mut Context) -> Result<FriendDecl, Box<Diagnostic>> {
    ...
    if friend.value.is_macro().is_some() || friend.value.has_tyargs() {
        context.env.add_diag(diag!(Syntax::InvalidName, (friend.loc, "Invalid 'friend' name")))
    }
    consume_token(context.tokens, Tok::Semicolon)?;
    let loc = make_loc(context.tokens.file_hash(), start_loc, context.tokens.previous_end_loc());
...
fn parse_module(
    ...
    match context.tokens.peek() {
        ...
        Tok::Semicolon => {
            context.env.check_feature(context.current_package, FeatureGate::ModuleLabel, name.loc());

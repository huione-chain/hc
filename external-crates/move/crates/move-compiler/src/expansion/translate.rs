...
impl<'env, 'map> Context<'env, 'map> {
    fn new(
        ...
        Context { defn_context, address: None, all_filter_alls, path_expander: None }
    }
...
fn top_level_address_(
    ...
    match ln_ {
        ...
        P::LeadingNameAccess_::GlobalAddress(name) => {
            context.env.add_diag(ice!((loc, "Found an address in top-level address position that uses a global name")));
        ...
        P::LeadingNameAccess_::Name(name) => match named_address_mapping.get(&name.value).copied() {
            ...
            None => {
                if name_res.is_ok() {
                    context.env.add_diag(address_without_value_error(suggest_declaration, loc, &name));
                }
                Address::NamedUnassigned(name)
            }
        },
...
fn duplicate_module(
    ...
    context.env().add_diag(diag!(Declarations::DuplicateItem, (mident.loc, dup_msg), (old_loc, prev_msg),))
}
...
fn set_module_address(context: &mut Context, module_name: &ModuleName, address: Option<Spanned<Address>>) {
    context.address = Some(match address {
        ...
        None => {
            ...
            context.env().add_diag(diag!(Declarations::InvalidModule, (loc, msg)));
...
fn module_(
    ...
    if name.value().starts_with('_') {
        ...
        context.env().add_diag(diag!(Declarations::InvalidName, (name.loc(), msg)));
    }
        ...
        for (_, _, friend_decl) in friends {
            ...
            let diag = if edition == Edition::E2024_MIGRATION {
                for aloc in &friend_decl.attr_locs {
                    context.env().add_diag(diag!(Migration::RemoveFriend, (*aloc, friend_msg)));
                }
    ...
    for (_, _, function) in functions {
        match function.visibility {
            ...
            E::Visibility::Package(loc) => {
                context.env().check_feature(package_name, FeatureGate::PublicPackage, loc);
...
fn known_attributes(
    ...
    UniqueMap::maybe_from_iter(attributes.into_iter().filter_map(|(n, attr)| match n {
        sp!(loc, E::AttributeName_::Unknown(n)) => {
            ...
            context.env().add_diag(diag!(Declarations::UnknownAttribute, (loc, msg)));
            None
        }
...
fn gate_known_attribute(context: &mut Context, loc: Loc, known: &KnownAttribute) {
    match known {
        ...
        KnownAttribute::Error(_) => {
            ...
            context.env().check_feature(pkg, FeatureGate::CleverAssertions, loc);
        }
    }
}
        ...
        let name_ = match known_attributes::KnownAttribute::resolve(sym) {
            ...
            Some(known) => {
                ...
                if is_nested {
                    ...
                    context.env().add_diag(diag!(Declarations::InvalidAttribute, (nloc, msg)));
                    continue;
                }
                ...
                if !expected_positions.contains(&attr_position) {
                    ...
                    context.env().add_diag(diag!(Declarations::InvalidAttribute, (nloc, msg), (nloc, expected_msg)));
                    continue;
                }
        ...
        for (inner_attr_loc, _, inner_attr) in inners.into_iter().flatten() {
            let (prefix, names) = match &inner_attr.value {
                ...
                E::Attribute_::Assigned(n, _) => {
                    ...
                    context.env().add_diag(diag!(Declarations::InvalidAttribute, (inner_attr_loc, msg)));
                    (None, vec![*n])
                }
            };
    ...
    for (diag_attr, prefix, names) in prefixed_filters {
        for sp!(nloc, n_) in names {
            ...
            if filters.is_empty() {
                ...
                context.env().add_diag(diag!(Attributes::ValueWarning, (nloc, msg)));
                continue;
            };
            for f in filters {
...
fn get_allow_attribute_inners<'a>(
    ...
    match &allow_attr.value {
        ...
        _ => {
            ...
            context.env().add_diag(diag!(Attributes::ValueWarning, (allow_attr.loc, msg)));
            None
        }
    }
}
    ...
    inners
        ...
        .map(|(_, inner_attr)| match inner_attr {
            ...
            sp!(loc, E::Attribute_::Assigned(n, _) | E::Attribute_::Parameterized(n, _)) => {
                ...
                context.env().add_diag(diag!(Attributes::ValueWarning, (*loc, msg)));
                *n
            }
        })
        .collect()
        ...
        P::Use::Fun { visibility, function, ty, method } => {
            ...
            let is_public = match visibility {
                ...
                P::Visibility::Friend(vis_loc) | P::Visibility::Package(vis_loc) => {
                    ...
                    context.env().add_diag(diag!(Declarations::InvalidUseFun, (loc, msg), (vis_loc, vis_msg)));
                    None
                }
            };
...
fn module_use(
    ...
    macro_rules! add_module_alias {
        ($ident:expr, $alias:expr) => {{
            if let Err(()) =
                check_restricted_name_all_cases(&mut context.defn_context.env, NameCase::ModuleAlias, &$alias)
            {
                return;
            }
...
fn explicit_use_fun(context: &mut Context, pexplicit: ParserExplicitUseFun) -> Option<E::ExplicitUseFun> {
    ...
    ice_assert!(context.env(), tyargs.is_none(), loc, "'use fun' with tyargs");
    ice_assert!(context.env(), is_macro.is_none(), loc, "Found a 'use fun' as a macro");
...
fn enum_variants(
    ...
    if pvariants.is_empty() {
        context.env().add_diag(diag!(Declarations::InvalidEnum, (eloc, "An 'enum' must define at least one variant")))
    }
...
fn function_(
    ...
    if let (Some(entry_loc), Some(macro_loc)) = (entry, macro_) {
        ...
        context.env().add_diag(diag!(Declarations::InvalidFunction, (entry_loc, e_msg), (macro_loc, m_msg),));
    }
    if let (Some(macro_loc), sp!(native_loc, P::FunctionBody_::Native)) = (macro_, &pbody) {
        ...
        let m_msg = format!("Function declared as '{MACRO_MODIFIER}' here");
        context.env().add_diag(diag!(Declarations::InvalidFunction, (*native_loc, n_msg), (macro_loc, m_msg),));
    }
    if let Some(macro_loc) = macro_ {
        let current_package = context.current_package();
        context.env().check_feature(current_package, FeatureGate::MacroFuns, macro_loc);
    }
...
fn exp(context: &mut Context, pe: Box<P::Exp>) -> Box<E::Exp> {
    ...
    let e_ = match pe_ {
        ...
        PE::Name(pn) if pn.value.has_tyargs() => {
            ...
            context.env().add_diag(diag!(NameResolution::NamePositionMismatch, (loc, msg)));
            EE::UnresolvedError
        }
        PE::Name(pn) => {
            bind_access_result!(
        ...
        pdotted_ @ (PE::Dot(_, _) | PE::DotUnresolved(_, _)) => match exp_dotted(context, Box::new(sp(loc, pdotted_))) {
        ...
        PE::DotCall(pdotted, n, is_macro, ptys_opt, sp!(rloc, prs)) => match exp_dotted(context, pdotted) {
            Some(edotted) => {
                let pkg = context.current_package();
                context.env().check_feature(pkg, FeatureGate::DotCall, loc);
                let tys_opt = optional_types(context, ptys_opt);
...
fn exp_cast(context: &mut Context, in_parens: bool, plhs: Box<P::Exp>, pty: P::Type) -> E::Exp_ {
    ...
    if !in_parens {
        let current_package = context.current_package();
        let loc = plhs.loc;
        let supports_feature = context.env().check_feature(current_package, FeatureGate::NoParensCast, loc);
        if supports_feature && ambiguous_cast(&plhs) {
            let msg = "Potentially ambiguous 'as'. Add parentheses to disambiguate";
            context.env().add_diag(diag!(Syntax::AmbiguousCast, (loc, msg)));
        }
    }
    ...
    let e_ = match e_ {
        ...
        _ => {
            let msg = "Invalid label. Labels can only be used on 'while', 'loop', or block '{{}}' \
                 expressions";
            context.env().add_diag(diag!(Syntax::InvalidLabel, (loc, msg)));
    ...
    match &e.value {
        E::ExpDotted_::Exp(inner) => {
            if !matches!(&inner.value, E::Exp_::Name(_, _)) {
                ...
                context.env().add_diag(diag!(Syntax::InvalidMoveOrCopy, (cloc, cmsg), (inner.loc, emsg)));
                return None;
            }
        }
        E::ExpDotted_::Dot(_, _) | E::ExpDotted_::DotUnresolved(_, _) | E::ExpDotted_::Index(_, _) => {
            let current_package = context.current_package();
            context.env().check_feature(current_package, FeatureGate::Move2024Paths, cloc);
        }
    }
    Some(match case {
...
fn exp_dotted(context: &mut Context, pdotted: Box<P::Exp>) -> Option<Box<E::ExpDotted>> {
    ...
    let edotted_ = match pdotted_ {
        ...
        PE::Index(plhs, sp!(argloc, args)) => {
            let cur_pkg = context.current_package();
            context.env().check_feature(cur_pkg, FeatureGate::Move2024Paths, loc);
            context.env().check_feature(cur_pkg, FeatureGate::SyntaxMethods, loc);
            let lhs = exp_dotted(context, plhs)?;
...
fn match_pattern(context: &mut Context, sp!(loc, pat_): P::MatchPattern) -> E::MatchPattern {
    ...
    fn resolve_and_validate_name(
        ...
        if let Some(loc) = is_macro {
            context.env().add_diag(diag!(Syntax::InvalidMacro, (loc, "Macros are not allowed in patterns.")));
        }
        Some((name, ptys_opt))
    }
...
pub(super) fn value(context: &mut DefnContext, sp!(loc, pvalue_): P::Value) -> Option<E::Value> {
    ...
    let value_ = match pvalue_ {
        ...
        PV::Num(s) => match parse_u256(&s) {
            ...
            Err(_) => {
                context.env.add_diag(num_too_big_error(loc, "the largest possible integer type, 'u256'"));
                return None;
            }
        },
        PV::Bool(b) => EV::Bool(b),
    ...
    match e_ {
        PE::Name(name) => match context.name_access_chain_to_module_access(Access::Term, name.clone()) {
            ...
            Some(access_result!(sp!(loc, M::Variant(_, _)), _tys_opt, _is_macro)) => {
                let cur_pkg = context.current_package();
                if context.env().check_feature(cur_pkg, FeatureGate::Enums, loc) {
        ...
        PE::Pack(pn, pfields) => {
            ...
            ice_assert!(context.env(), is_macro.is_none(), loc, "Marked a bind as a macro");
            let tys_opt = optional_sp_types(context, ptys_opt);
        ...
        PE::Call(pn, sp!(_, exprs)) => {
            let pkg = context.current_package();
            context.env().check_feature(pkg, FeatureGate::PositionalFields, loc);
            let access_result!(name, ptys_opt, is_macro) =
                context.name_access_chain_to_module_access(Access::ApplyNamed, pn)?;
            ice_assert!(context.env(), is_macro.is_none(), loc, "Marked a bind as a macro");
            let tys_opt = optional_sp_types(context, ptys_opt);

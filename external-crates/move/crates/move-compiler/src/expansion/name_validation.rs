...
pub fn check_valid_address_name(env: &mut CompilationEnv, sp!(_, ln_): &P::LeadingNameAccess) -> Result<(), ()> {
    use P::LeadingNameAccess_ as LN;
    match ln_ {
        LN::AnonymousAddress(_) => Ok(()),
        LN::GlobalAddress(n) | LN::Name(n) => check_restricted_name_all_cases(env, NameCase::Address, n),
    }
}
...
#[allow(clippy::result_unit_err)]
pub fn check_valid_function_parameter_name(env: &mut CompilationEnv, is_macro: Option<Loc>, v: &Var) {
...
pub fn check_valid_module_member_name(env: &mut CompilationEnv, member: ModuleMemberKind, name: Name) -> Option<Name> {
...
pub fn check_valid_module_member_alias(env: &mut CompilationEnv, member: ModuleMemberKind, alias: Name) -> Option<Name> {
    match check_valid_module_member_name_impl(env, member, &alias, NameCase::ModuleMemberAlias(member)) {
        Err(()) => None,
        Ok(()) => Some(alias),
    }
}
...
fn check_valid_module_member_name_impl(
    ...
    check_restricted_names(env, case, n, crate::naming::ast::BuiltinFunction_::all_names())?;
    check_restricted_names(env, case, n, crate::naming::ast::BuiltinTypeName_::all_names())?;
...
#[allow(clippy::result_unit_err)]
pub fn check_valid_type_parameter_name(env: &mut CompilationEnv, is_macro: Option<Loc>, n: &Name) -> Result<(), ()> {
    // TODO move these names to a more central place?
    if n.value == symbol!("_") {
    ...
    // TODO move these names to a more central place?
    check_restricted_names(env, NameCase::TypeParameter, n, crate::naming::ast::BuiltinFunction_::all_names())?;
    check_restricted_names(env, NameCase::TypeParameter, n, crate::naming::ast::BuiltinTypeName_::all_names())?;
...
pub fn check_restricted_name_all_cases(env: &mut CompilationEnv, case: NameCase, n: &Name) -> Result<(), ()> {
    match case {
        NameCase::Constant
        | NameCase::Function
        | NameCase::Struct

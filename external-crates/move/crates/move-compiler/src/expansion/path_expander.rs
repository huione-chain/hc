    ...
    fn resolve_name(&mut self, context: &mut DefnContext, namespace: NameSpace, name: Name) -> AccessChainNameResult {
        ...
        match self.aliases.resolve(namespace, &name) {
            ...
            Some(AliasEntry::TypeParam(_)) => {
                context.env.add_diag(ice!((name.loc, "ICE alias map misresolved name as type param")));
                ...
                for entry in entries {
                    ...
                    match result {
                        ...
                        NR::UnresolvedName(_, _) => {
                            context.env.add_diag(ice!((loc, "ICE access chain expansion failed")));
                            break;
                        }
...
impl PathExpander for Move2024PathExpander {
    ...
    fn name_access_chain_to_attribute_value(
        ...
        Some(sp(loc, match avalue_ {
            ...
            PV::ModuleAccess(access_chain) => {
                ice_assert!(context.env, access_chain.value.tyargs().is_none(), loc, "Found tyargs");
                ice_assert!(context.env, access_chain.value.is_macro().is_none(), loc, "Found macro");
                ...
                let result = match (term_result, module_result) {
                    ...
                    (t_res, m_res) => {
                        ...
                        context.env.add_diag(diag!(Attributes::AmbiguousAttributeValue, (loc, msg)));
                        return None;
                    }
                };
                match result {
        ...
        let (module_access, tyargs, is_macro) = match access {
            Access::ApplyPositional | Access::ApplyNamed | Access::Type => {
                ...
                match resolved_name {
                    ...
                    NR::Address(_, _) => {
                        context.env.add_diag(unexpected_access_error(resolved_name.loc(), resolved_name.name(), access));
                        return None;
                    }
            ...
            Access::Module => {
                context.env.add_diag(ice!((loc, "ICE module access should never resolve to a module member")));
                return None;
            }
        };
    ...
    fn ide_autocomplete_suggestion(&mut self, context: &mut DefnContext, loc: Loc) {
        if context.env.ide_mode() {
            ...
            context.env.add_ide_annotation(loc, IDEAnnotation::PathAutocompleteInfo(Box::new(info)));
        }
    }
}
...
impl PathExpander for LegacyPathExpander {
    ...
    fn name_access_chain_to_module_access(
        ...
        let tn_: ModuleAccessResult = match (access, ptn_) {
            (Access::Pattern, _) => {
                context.env.add_diag(ice!((loc, "Attempted to expand a variant with the legacy path expander")));
                return None;
            }
            ...
            (Access::Module, single_entry!(_name, _tyargs, _is_macro)) => {
                context.env.add_diag(ice!((loc, "ICE path resolution produced an impossible path for a module")));
                return None;
            }
            (_, PN::Path(mut path)) => {
                if access == Access::Type {
        ...
        match pn_ {
            ...
            PN::Path(path) => {
                ...
                match (&path.root.name, &path.entries[..]) {
                    ...
                    // Error cases
                    (_ln, []) => {
                        context.env.add_diag(ice!((loc, "Found path with no path entries")));
                        None
                    }
                    (ln, [n, m, ..]) => {

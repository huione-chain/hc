...
pub enum Exp_ {
    ...
    MethodCall(Box<ExpDotted>, Name, /* is_macro */ Option<Loc>, Option<Vec<Type>>, Spanned<Vec<Exp>>),
...
impl Address {
    ...
    pub fn is(&self, address: impl AsRef<str>) -> bool {
        match self {
            Self::Numerical { name: Some(n), .. } | Self::NamedUnassigned(n) => n.value.as_str() == address.as_ref(),

...
pub fn program(compilation_env: &mut CompilationEnv, modules: &mut UniqueMap<ModuleIdent, T::ModuleDefinition>) {
...
impl<'a, 'env> Context<'a, 'env> {
    fn new(env: &'env mut CompilationEnv, modules: &'a UniqueMap<ModuleIdent, T::ModuleDefinition>) -> Self {
        Context {
            env,
            modules,
...
fn lvalue(context: &mut Context, sp!(loc, lv_): &T::LValue) {
    ...
    match lv_ {
        ...
        L::BorrowUnpackVariant(..) | L::UnpackVariant(..) => {
            context.env.add_diag(ice!((*loc, "variant unpacking shouldn't occur before match expansion")));
        }
    }
}
...
fn exp(context: &mut Context, e: &T::Exp) {
    ...
    match &e.exp.value {
        ...
        E::VariantMatch(..) => {
            context.env.add_diag(ice!((e.exp.loc, "shouldn't find variant match before HLIR lowering")));
        }
        E::While(_, e1, e2) => {
            exp(context, e1);
            exp(context, e2);

    ...
    fn counterexample_rec(
        ...
        let result = if matrix.patterns_empty() {
            ...
            if !context.env.has_errors() {
                context.env.add_diag(ice!((matrix.loc, "Non-empty matrix with non errors but no type")));
            }
            None
        };
...
fn ide_report_missing_arms(context: &mut Context, loc: Loc, matrix: &PatternMatrix) {
    ...
    fn report_bool(context: &mut Context, loc: Loc, matrix: &PatternMatrix) {
        ...
        if !unused.is_empty() {
            ...
            context.env.add_ide_annotation(loc, IDEAnnotation::MissingMatchArms(Box::new(info)));
        }
    }
    ...
    fn report_datatype(
        ...
        if context.modules.is_struct(&mident, &name) {
            ...
            let Some(fields) = context.modules.struct_fields(&mident, &name) else {
                context.env.add_diag(ice!((loc, "Tried to look up fields for this struct and found none")));
                return;
            };
            ...
            for variant in context.modules.enum_variants(&mident, &name).into_iter() {
                ...
                let Some(fields) = context.modules.enum_variant_fields(&mident, &name, &variant) else {
                    context.env.add_diag(ice!((loc, "Tried to look up fields for this enum and found none")));
                    continue;
                };
    ...
    let Some(ty) = matrix.tys.first() else {
        context.env.add_diag(ice!((loc, "Pattern matrix with no types handed to IDE function")));
        return;
    };
    if let Some(sp!(_, BuiltinTypeName_::Bool)) = &ty.value.unfold_to_builtin_type_name() {

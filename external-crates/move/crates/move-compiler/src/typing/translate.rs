...
pub fn program(
    ...
    let N::Program { info, inner: N::Program_ { modules: nmodules } } = prog;
    ...
    let prog = T::Program { modules, info: Arc::new(module_info) };
...
mod check_valid_constant {
    ...
    pub(crate) fn signature<T: ToString, F: FnOnce() -> T>(
        ...
        context.env.add_diag(diag!(code, (sloc, fmsg()), (loc, tmsg)))
    }
    ...
    fn exp_(context: &mut Context, sp!(loc, e_): &T::UnannotatedExp) {
        ...
        let error_case = match e_ {
            ...
            E::VariantMatch(_subject, _, _arms) => {
                context.env.add_diag(ice!((*loc, "shouldn't find variant match before match compilation")));
    ...
    fn sequence_item(context: &mut Context, sp!(loc, item_): &T::SequenceItem) {
        ...
        context.env.add_diag(diag!(TypeSafety::UnsupportedConstant, (*loc, msg),))
    }
}
...
fn struct_def(context: &mut Context, sloc: Loc, s: &mut N::StructDefinition) {
    ...
    context.env.add_warning_filter_scope(s.warning_filter.clone());
...
fn enum_def(context: &mut Context, enum_: &mut N::EnumDefinition) {
    ...
    context.env.add_warning_filter_scope(enum_.warning_filter.clone());
...
fn check_non_phantom_param_usage(context: &mut Context, param: &N::TParam, param_usage: Option<bool>) {
    ...
    match param_usage {
        None => {
            ...
            context.env.add_diag(diag!(UnusedItem::StructTypeParam, (name.loc, msg)))
        }
        Some(false) => {
            let msg = format!(
            ...
            context.env.add_diag(diag!(Declarations::InvalidNonPhantomUse, (name.loc, msg)))
        }
        Some(true) => {}
    }
}
...
fn exp(context: &mut Context, ne: Box<N::Exp>) -> Box<T::Exp> {
    ...
    let (ty, e_) = match ne_ {
        ...
        NE::MethodCall(ndotted, f, Some(macro_call_loc), ty_args_opt, sp!(argloc, nargs_)) => {
            let edotted = process_exp_dotted(context, None, ndotted);
            let ty_call_opt = macro_method_call(context, eloc, edotted, f, macro_call_loc, ty_args_opt, argloc, nargs_);
            match ty_call_opt {
                None => {
                    assert!(context.env.has_errors());
                    (context.error_type(eloc), TE::UnresolvedError)
        ...
        NE::IfElse(nb, nt, nf) => {
            ...
            let ty = join(context, eloc, || "Incompatible branches", et.ty.clone(), ef.ty.clone());
        ...
        NE::Lambda(_) => {
            if context.env.check_feature(context.current_package, FeatureGate::Lambda, eloc) {
                let msg = "Lambdas can only be used directly as arguments to 'macro' functions";
                context.env.add_diag(diag!(TypeSafety::UnexpectedLambda, (eloc, msg)))
            }
            (context.error_type(eloc), TE::UnresolvedError)
        }
...
fn binop(context: &mut Context, el: Box<T::Exp>, bop: BinOp, loc: Loc, er: Box<T::Exp>) -> Box<T::Exp> {
    ...
    let (ty, operand_ty) = match &bop.value {
        ...
        Range | Implies | Iff => {
            context.env.add_diag(ice!((loc, "ICE unexpect specification operator")));
    ...
    match pat_ {
        ...
        P::Struct(m, struct_, tys_opt, fields) => {
            ...
            if !context.is_current_module(&m) {
                ...
                context.env.add_diag(diag!(TypeSafety::Visibility, (loc, msg)));
            }
            let bt = rtype!(bt);
            let pat_ = if field_error {
                TP::ErrorPat
...
fn resolve_field(context: &mut Context, loc: Loc, ty: Type, field: &Field) -> Type {
    ...
    match core::ready_tvars(&context.subst, ty) {
        ...
        sp!(tloc, Anything) => {
            context.env.add_diag(diag!(TypeSafety::UninferredType, (loc, msg()), (tloc, UNINFERRED_MSG),));
        ...
        sp!(_, Apply(_, sp!(_, ModuleType(m, n)), targs)) => {
            ...
            match context.datatype_kind(&m, &n) {
                ...
                DatatypeKind::Enum => {
                    ...
                    context.env.add_diag(diag!(TypeSafety::ExpectedSpecificType, (loc, msg)));
                    context.error_type(loc)
                }
            }
        }
        t => {
            ...
            context.env.add_diag(diag!(TypeSafety::ExpectedSpecificType, (loc, msg()), (t.loc, smsg),));
            context.error_type(loc)
        }
    }
}
...
fn add_struct_field_types<T>(
    ...
    for (_, f_, _) in &fields_ty {
        if fields.get_(f_).is_none() {
            ...
            context.env.add_diag(diag!(TypeSafety::TooFewArguments, (loc, msg)))
        }
    }
    fields.map(|f, (idx, x)| {
    ...
    match ty {
        ...
        sp!(_, T::Apply(_, type_name, _)) => {
            ...
            if index_opt.is_none() {
                context.env.add_diag(diag!(Declarations::MissingSyntaxMethod, (loc, msg()),));
            }
            index_opt
        }
        sp!(_, T::Unit | T::Ref(_, _) | T::Param(_) | T::Fun(_, _)) => {
            ...
            context.env.add_diag(diag!(TypeSafety::ExpectedSpecificType, (loc, msg()), (ty.loc, smsg),));
            None
        }
    }
}
...
fn process_exp_dotted(context: &mut Context, constraint_verb: Option<&str>, ndotted: N::ExpDotted) -> ExpDotted {
    ...
    fn process_index_access(
        ...
        let base_type = match result_type {
            ...
            _ => {
                context.env.add_diag(ice!((dloc, "Index should have failed in naming")));
    ...
    fn process_exp_dotted_inner(
        ...
        match ndot_ {
            ...
            N::ExpDotted_::Dot(ndot, field) => {
                ...
                inner.loc = dloc;
                inner.accessors.push(ExpDottedAccess::Field(field, field_type));
                inner
            }
            ...
            N::ExpDotted_::DotAutocomplete(_loc, ndot) => {
                context.env.add_diag(ice!((dloc, "Found a dot autocomplete where unsupported")));
...
fn resolve_exp_dotted(context: &mut Context, usage: DottedUsage, error_loc: Loc, edotted: ExpDotted) -> Box<T::Exp> {
    use T::UnannotatedExp_ as TE;
        ...
        DottedUsage::Move(loc) => {
            match edotted.base.exp.value {
                ...
                TE::Constant(_, _) if edotted.accessors.is_empty() => {
                    context
                        .env
                        .add_diag(diag!(TypeSafety::InvalidMoveOp, (loc, "Invalid 'move'. Cannot 'move' constants")));
                    make_error(context)
                }
                ...
                _ => {
                    if context.env.check_feature(context.current_package(), FeatureGate::Move2024Paths, loc) {
                        // call for effect
                        ...
                        context.env.add_diag(diag!(TypeSafety::InvalidMoveOp, (loc, msg)));
                        make_error(context)
                    } else {
                        make_error(context)
                    }
            ...
            let copy_exp = if edotted.accessors.is_empty() {
                match edotted.base.exp.value {
                    ...
                    _ => {
                        ...
                        context.env.add_diag(diag!(TypeSafety::InvalidCopyOp, (loc, msg)));
                        make_error(context)
                    }
                }
            } else {
...
fn borrow_exp_dotted(context: &mut Context, error_loc: Loc, mut_: bool, ed: ExpDotted) -> Box<T::Exp> {
    ...
    let mut exp = match base_kind {
        BaseRefKind::Owned => exp_to_borrow_(context, loc, mut_, Box::new(base), base_type, warn_on_constant),
            ...
            ExpDottedAccess::Index { index_loc, syntax_methods, args, base_type: index_base_type } => {
                ...
                let (m, f) = if mut_ {
                    if let Some(index_mut) = index_methods.index_mut {
                        ...
                        context.env.add_diag(diag!(Declarations::MissingSyntaxMethod, (index_loc, msg),));
...
fn exp_dotted_to_owned(context: &mut Context, error_loc: Loc, usage: DottedUsage, ed: ExpDotted) -> Box<T::Exp> {
    ...
    let (access_msg, access_type) = if let Some(accessor) = ed.accessors.last() {
        ...
        context.env.add_diag(ice!((ed.loc, "Attempted to make a dotted path with no dots")));
        return make_error_exp(context, ed.loc);
    };
    let case = match usage {
        DottedUsage::Move(_) => {
            context.env.add_diag(ice!((ed.loc, "Invalid dotted usage 'move' in to_owned")));
            return make_error_exp(context, ed.loc);
        }
        DottedUsage::Borrow(_) => {
            context.env.add_diag(ice!((ed.loc, "Invalid dotted usage 'borrow' in to_owned")));
            return make_error_exp(context, ed.loc);
        }
        DottedUsage::Use => "implicit copy",
        DottedUsage::Copy(loc) => {
            context.env.check_feature(context.current_package(), FeatureGate::Move2024Paths, loc);
            "'copy'"
        }
    };
...
fn warn_on_constant_borrow(context: &mut Context, loc: Loc, e: &T::Exp) {
    ...
    if matches!(&e.exp.value, TE::Constant(_, _)) {
        ...
        context.env.add_diag(diag!(TypeSafety::ImplicitConstantCopy, (loc, msg)))
    }
}
...
fn method_call(
    ...
    let (m, f, fty, usage) = match method_call_resolve(context, call_loc, &edotted, method, ty_args_opt) {
        ...
        ResolvedMethodCall::UnknownName if context.env.ide_mode() => {
            ...
            let dot_output = resolve_exp_dotted(context, DottedUsage::Borrow(false), call_loc, edotted);
    ...
    match &ty.value {
        ...
        t => {
            let msg = match t {
                ...
                Ty::Ref(_, _) | Ty::Var(_) => {
                    context.env.add_diag(ice!((loc, "Typing did not unfold type before resolving type name")));
                    return None;
                }
...
fn check_call_target(
    ...
    context.env.add_diag(diag!(TypeSafety::InvalidCallTarget, (macro_call_loc, call_msg), (decl_loc, decl_msg),));
}
    ...
    let res = match macro_expand::call(context, call_loc, m, f, type_args.clone(), args, return_ty) {
        None => {
            if !(context.env.has_errors() || context.env.ide_mode()) {
                context.env.add_diag(ice!((call_loc, "No macro found, but name resolution passed.")));
            }
...
fn unused_module_members(context: &mut Context, mident: &ModuleIdent_, mdef: &T::ModuleDefinition) {
    ...
    for (loc, name, c) in &mdef.constants {
        context.env.add_warning_filter_scope(c.warning_filter.clone());
        ...
        if members.is_none() || !members.unwrap().contains(name) {
            ...
            context.env.add_diag(diag!(UnusedItem::Constant, (loc, msg)))
        }
    ...
    for (loc, name, fun) in &mdef.functions {
        ...
        context.env.add_warning_filter_scope(fun.warning_filter.clone());
            ...
            && (members.is_none() || !members.unwrap().contains(name))
        {
            ...
            context.env.add_diag(diag!(UnusedItem::Function, (loc, msg)))
        }
        context.env.pop_warning_filter_scope();
    }

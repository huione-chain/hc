...
impl<'env> Context<'env> {
    ...
    pub fn reset_for_module_item(&mut self, loc: Loc) {
        ...
        if !self.ide_info.is_empty() {
            self.env.add_diag(ice!((loc, "IDE info should be cleared after each item")));
    ...
    pub fn declare_local(&mut self, _: Mutability, var: Var, ty: Type) {
        if let Err((_, prev_loc)) = self.locals.add(var, ty) {
            ...
            self.env.add_diag(ice!((var.loc, msg), (prev_loc, "Previously declared here")));
        }
    }
...
pub fn make_constant_type(context: &mut Context, loc: Loc, m: &ModuleIdent, c: &ConstantName) -> Type {
    ...
    if !in_current_module {
        ...
        context.env.add_diag(diag!(TypeSafety::Visibility, (loc, msg), (defined_loc, internal_msg)));
    }
...
pub fn make_method_call_type(
    ...
    let Some((target_m, target_f)) = target_function_opt else {
        ...
        if let Some((m, finfo)) = finfo_opt {
            ...
            context.env.add_diag(diag!(TypeSafety::InvalidMethodCall, (loc, msg), (first_ty_loc, fmsg)));
        } else {
            let msg = format!(
                "Invalid method call. \
                No known method '{method}' on type '{lhs_ty_str}'"
            ...
            context.env.add_diag(diag!(TypeSafety::InvalidMethodCall, (loc, msg), (method.loc, fmsg)));
        }
        return None;
    };
...
fn report_visibility_error_(
    ...
    if let Some(names) = context.expanding_macros_names() {
        ...
        match context.macro_expansion.first() {
            ...
            _ => {
                context.env.add_diag(ice!((call_loc, "Error when dealing with macro visibilities")));
            }
        };
...
pub fn check_call_arity<S: std::fmt::Display, F: Fn() -> S>(
    ...
    context.env.add_diag(diag!(code, (loc, cmsg), (argloc, format!("Found {} argument(s) here", given_len)),));
}
...
fn solve_base_type_constraint(context: &mut Context, loc: Loc, msg: String, ty: &Type) {
    ...
    match unfolded_ {
        ...
        Unit | Ref(_, _) | Apply(_, sp!(_, Multiple(_)), _) => {
            ...
            context.env.add_diag(diag!(TypeSafety::ExpectedBaseType, (loc, msg), (tyloc, tmsg)))
        }
...
fn solve_single_type_constraint(context: &mut Context, loc: Loc, msg: String, ty: &Type) {
    ...
    match unfolded_ {
        ...
        Unit | Apply(_, sp!(_, Multiple(_)), _) => {
            ...
            context.env.add_diag(diag!(TypeSafety::ExpectedSingleType, (loc, msg), (tyloc, tmsg)))
        }
...
fn join_impl(mut subst: Subst, case: TypingCase, lhs: &Type, rhs: &Type) -> Result<(Subst, Type), TypingError> {
    use TypeName_::*;
    use Type_::*;
    use TypingCase::*;
    match (lhs, rhs) {

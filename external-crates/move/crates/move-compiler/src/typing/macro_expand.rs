...
pub(crate) fn call(
    ...
    let reloc_clever_errors = match &context.macro_expansion[0] {
        ...
        core::MacroExpansion::Argument { .. } => {
            context.env.add_diag(ice!((call_loc, "ICE top level macro scope should never be an argument")));
            call_loc
        }
    };
...
fn bind_lambda(
    ...
    match arg.value {
        ...
        _ => {
            ...
            context.env.add_diag(diag!(TypeSafety::CannotExpandMacro, (arg.loc, msg)));
            None
        }
    }
}
...
fn recolor_exp_dotted(ctx: &mut Recolor, sp!(_, ed_): &mut N::ExpDotted) {
    match ed_ {
        ...
        N::ExpDotted_::Dot(ed, _) | N::ExpDotted_::DotAutocomplete(_, ed) => recolor_exp_dotted(ctx, ed),
...
fn report_unused_argument(context: &mut core::Context, loc: EvalStrategy<Loc, Loc>) {
    ...
    context.env.add_diag(diag!(UnusedItem::DeadCode, (loc, msg)));
}
    ...
    match e_ {
        ...
        N::Exp_::VarCall(sp!(_, v_), sp!(argloc, es)) if context.lambdas.contains_key(v_) => {
            ...
            if context.core.env.ide_mode() {
                context.core.env.add_ide_annotation(*eloc, IDEAnnotation::ExpandedLambda);
            }
            *e_ = block;
        }

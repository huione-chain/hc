...
fn validate_index_syntax_methods(context: &mut Context, index: &SyntaxMethod, index_mut: &SyntaxMethod) -> bool {
    ...
    if let Some((ndx, (subject_ref_type, subject_mut_ref_type))) = param_tys.next() {
        if let Ok((subst_, _)) = core::subtype(subst.clone(), subject_mut_ref_type, subject_ref_type) {
            subst = subst_;
        } else {
    ...
    // under the reference.
    if core::subtype(subst.clone(), &mut_finfo.signature.return_type, &index_ty.return_).is_err() {

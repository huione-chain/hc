    ...
    pub fn new(env: &mut CompilationEnv, info: &NamingProgramInfo) -> Self {
        ...
        for (mident, module_info) in info.modules.key_cloned_iter() {
            if let Some(deprecation) =
                deprecations(env, AttributePosition::Module, &module_info.attributes, mident.loc, mident)
            {
            ...
            for (name, constant) in module_info.constants.key_cloned_iter() {
                if let Some(deprecation) =
                    deprecations(env, AttributePosition::Constant, &constant.attributes, name.0.loc, mident)
                {
            ...
            for (name, function) in module_info.functions.key_cloned_iter() {
                if let Some(deprecation) =
                    deprecations(env, AttributePosition::Function, &function.attributes, name.0.loc, mident)
                {
                    deprecated_members.insert((mident, Some(name.0)), deprecation);
                }
            }
            ...
            for (name, datatype) in module_info.structs.key_cloned_iter() {
                if let Some(deprecation) =
                    deprecations(env, AttributePosition::Struct, &datatype.attributes, name.0.loc, mident)
                {
                    deprecated_members.insert((mident, Some(name.0)), deprecation);
                }
            }
            ...
            for (name, datatype) in module_info.enums.key_cloned_iter() {
                if let Some(deprecation) =
                    deprecations(env, AttributePosition::Enum, &datatype.attributes, name.0.loc, mident)
                {
                    deprecated_members.insert((mident, Some(name.0)), deprecation);
                }
            }
...
impl Deprecation {
    ...
    pub fn emit_deprecation_warning(&self, env: &mut CompilationEnv, member_name: Name, method_opt: Option<Name>) {

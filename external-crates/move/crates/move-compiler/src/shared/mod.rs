...
pub struct CompilationEnv {
    ...
    prim_definers: BTreeMap<crate::naming::ast::BuiltinTypeName_, crate::expansion::ast::ModuleIdent>,
...
impl CompilationEnv {
    ...
    pub fn add_diag(&mut self, mut diag: Diagnostic) {
        if diag.info().severity() <= Severity::NonblockingError
            && self.diags.any_syntax_error_with_primary_loc(diag.primary_loc())
        {
    ...
    pub fn check_diags_at_or_above_severity(&mut self, threshold: Severity) -> Result<(), Diagnostics> {
    ...
    fn is_filtered(&self, diag: &Diagnostic) -> bool {
        self.warning_filter.iter().rev().any(|filter| filter.is_filtered(diag))
    }

    fn filter_for_dependency(&self) -> bool {
        self.warning_filter.iter().rev().any(|filter| filter.for_dependency())
    }
    ...
    pub fn check_feature(&mut self, package: Option<Symbol>, feature: FeatureGate, loc: Loc) -> bool {
    ...
    pub fn feature_edition_error_msg(&mut self, feature: FeatureGate, package: Option<Symbol>) -> Option<String> {
    ...
    pub fn set_primitive_type_definers(&mut self, m: BTreeMap<N::BuiltinTypeName_, E::ModuleIdent>) {

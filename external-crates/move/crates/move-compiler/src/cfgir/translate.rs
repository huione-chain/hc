...
pub fn program(
    ...
    let H::Program { modules: hmodules, info } = prog;

    let mut context = Context::new(compilation_env, &info);

    let modules = modules(&mut context, hmodules);

    let mut program = G::Program { modules, info: info.clone() };
...
fn constant_(
    ...
    assert!(num_previous_errors == context.env.count_diags(), "{}", ICE_MSG);
    cfgir::optimize(context.env, context.current_package, &fake_signature, &locals, constant_values, &mut cfg);

    if blocks.len() != 1 {
        context.env.add_diag(diag!(BytecodeGeneration::UnfoldableConstant, (full_loc, CANNOT_FOLD)));
        return None;
    }
    ...
    for sp!(cloc, cmd_) in &optimized_block {
        let e = match cmd_ {
            ...
            _ => {
                context.env.add_diag(diag!(BytecodeGeneration::UnfoldableConstant, (*cloc, CANNOT_FOLD)));
                continue;
            }
        };
...
fn check_constant_value(context: &mut Context, e: &H::Exp) {
    ...
    match &e.exp.value {
        E::Value(_) => (),
        _ => context.env.add_diag(diag!(BytecodeGeneration::UnfoldableConstant, (e.exp.loc, CANNOT_FOLD))),
    }
}
...
fn function(context: &mut Context, module: ModuleIdent, name: FunctionName, f: H::Function) -> G::Function {
    let H::Function { warning_filter, index, attributes, visibility, compiled_visibility, entry, signature, body } = f;
    context.env.add_warning_filter_scope(warning_filter.clone());
    let body = function_body(context, module, name, &attributes, entry, visibility, &signature, body);
    context.env.pop_warning_filter_scope();
    G::Function { warning_filter, index, attributes, visibility, compiled_visibility, entry, signature, body }
}
    ...
    let b_ = match tb_ {
        ...
        HB::Defined { locals, body } => {
            ...
            if !context.env.has_errors() {
                cfgir::optimize(context.env, context.current_package, signature, &locals, &UniqueMap::new(), &mut cfg);
            }
...
impl CFGIRVisitorConstructor for AbsintVisitor {
    ...
    fn context<'a>(env: &'a mut CompilationEnv, program: &G::Program) -> Self::Context<'a> {
        AbsintVisitorContext { env, info: program.info.clone(), current_package: None }
    }
}

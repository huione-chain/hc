...
pub fn last_usage(compilation_env: &mut CompilationEnv, context: &super::CFGContext, cfg: &mut MutForwardCFG) {
...
mod last_usage {
    ...
    impl<'a, 'b> Context<'a, 'b> {
        fn new(env: &'a mut CompilationEnv, next_live: &'b BTreeSet<Var>, dropped_live: BTreeSet<Var>) -> Self {
            Context { env, next_live, dropped_live }
        }
    }
    ...
    pub fn block(
        ...
        for idx in 0..len {
            ...
            command(&mut Context::new(compilation_env, next_data, dropped_live), cmd)
        }
    }
                    ...
                    match display_var(v.value()) {
                        ...
                        DisplayVar::Orig(vstr) | DisplayVar::MatchTmp(vstr) => {
                            if !v.starts_with_underscore() {
                                ...
                                context.env.add_diag(diag!(UnusedItem::Assignment, (l.loc, msg)));
                            }

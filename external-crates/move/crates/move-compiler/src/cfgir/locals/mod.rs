...
impl<'a> LocalsSafety<'a> {
    fn new(
        ...
        let unused_mut = local_types
            ...
            .collect();
        Self { env, info, package, local_types, signature, unused_mut }
    }
}
...
pub fn verify(
    ...
    let mut locals_safety = LocalsSafety::new(compilation_env, context.info, context.package, locals, signature);
...
fn unused_let_muts<T>(env: &mut CompilationEnv, locals: &UniqueMap<Var, T>, unused_mut_locals: BTreeMap<Var, Loc>) {
    for (v, mut_loc) in unused_mut_locals {
        if !v.starts_with_underscore() {
            ...
            env.add_diag(diag!(UnusedItem::MutModifier, (decl_loc, decl_msg), (mut_loc, mut_msg)))
        }
    }
}

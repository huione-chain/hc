...
impl<'env> Context<'env> {
    ...
    fn maybe_report_value_error(&mut self, error: &mut ControlFlow) -> bool {
        ...
        match error {
            CF::Divergent { loc, set: _, reported } if !*reported => {
                *reported = true;
                self.env.add_diag(diag!(UnusedItem::DeadCode, (*loc, VALUE_UNREACHABLE_MSG)));
                true
            }
    ...
    fn maybe_report_tail_error(&mut self, error: &mut ControlFlow) -> bool {
        ...
        match error {
            CF::Divergent { loc, set: _, reported } if !*reported => {
                *reported = true;
                self.env.add_diag(diag!(UnusedItem::DeadCode, (*loc, DIVERGENT_MSG)));
                true
            }
            CF::Divergent { .. } | CF::None | CF::Possible => false,
        }
...
fn constant(context: &mut Context, _name: &Symbol, cdef: &T::Constant) {
    context.env.add_warning_filter_scope(cdef.warning_filter.clone());
...
fn tail(context: &mut Context, e: &T::Exp) -> ControlFlow {
    ...
    match e_ {
        ...
        E::VariantMatch(..) => {
            context.env.add_diag(ice!((*eloc, "Found variant match in detect_dead_code")));
            CF::None
        }
    ...
    if stmt_flow.is_some() {
        ...
        match last_exp {
            ...
            Some(sp!(loc, _)) => {
                context
                    .env
                    .add_diag(ice!((*loc, "ICE last sequence item should have been an exp in dead code analysis")));
                ControlFlow::None
            }
        }
    }
...
fn value(context: &mut Context, e: &T::Exp) -> ControlFlow {
    ...
    match e_ {
        ...
        E::IfElse(test, conseq, alt) => {
            do_if(context, (eloc, test, conseq, alt), /* tail_pos */ false, value, |context, flow| {
            ...
            for item in items {
                match item {
                    ...
                    T::ExpListItem::Splat(_, _, _) => {
                        context.env.add_diag(ice!((*eloc, "ICE splat exp unsupported by dead code analysis")));
                    }
                }
            }
            flow
...
fn statement(context: &mut Context, e: &T::Exp) -> ControlFlow {
    ...
    match e_ {
        ...
        E::IfElse(test, conseq, alt) => {
            do_if(context, (eloc, test, conseq, alt), /* tail_pos */ false, statement, |_, _| false)
        }
        E::Match(subject, arms) => {
        ...
        // -----------------------------------------------------------------------------------------
        E::Use(_) => {
            context.env.add_diag(ice!((*eloc, "ICE found unexpanded use")));
            CF::None
        }
    }
}

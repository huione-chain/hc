...
pub fn program(
    ...
    let T::Program { modules: tmodules, info } = prog;
...
fn base_type(context: &mut Context, sp!(loc, nb_): N::Type) -> H::BaseType {
    ...
    let b_ = match nb_ {
        NT::Var(_) => {
            context
                .env
                .add_diag(ice!((loc, format!("ICE type inf. var not expanded: {}", debug_display_verbose!(nb_)))));
            return error_base_type(loc);
        }
        NT::Apply(None, _, _) => {
            context.env.add_diag(ice!((loc, format!("ICE kind not expanded: {}", debug_display_verbose!(nb_)))));
            return error_base_type(loc);
        }
        ...
        NT::Ref(_, _) | NT::Unit | NT::Fun(_, _) => {
            context
                .env
                .add_diag(ice!((loc, format!("ICE base type constraint failed: {}", debug_display_verbose!(nb_)))));
            return error_base_type(loc);
        }
    };
    sp(loc, b_)
...
fn type_(context: &mut Context, sp!(loc, ty_): N::Type) -> H::Type {
    ...
    let t_ = match ty_ {
        NT::Unit => HT::Unit,
        NT::Apply(None, _, _) => {
            context.env.add_diag(ice!((loc, format!("ICE kind not expanded: {}", debug_display_verbose!(ty_)))));
...
fn tail(context: &mut Context, block: &mut Block, expected_type: Option<&H::Type>, e: T::Exp) -> Option<H::Exp> {
    ...
    match e_ {
        ...
        E::Return(_) | E::Abort(_) | E::Give(_, _) | E::Continue(_) | E::Assign(_, _, _) | E::Mutate(_, _) => {
            context.env.add_diag(ice!((eloc, "ICE statement mishandled in HLIR lowering")));
            None
        }
...
fn tail_block(
    ...
    match last_exp {
        ...
        Some(sp!(loc, _)) => {
            context.env.add_diag(ice!((loc, "ICE statement mishandled in HLIR lowering")));
            None
        }
    }
}
    ...
    let preresult: H::Exp = match e_ {
        ...
        E::Builtin(bt, arguments) if matches!(&*bt, sp!(_, T::BuiltinFunction_::Assert(None))) => {
            ...
            let [cond_item, code_item]: [TI; 2] = match arguments.exp.value {
                ...
                _ => {
                    context.env.add_diag(ice!((eloc, "ICE type checking assert failed")));
...
fn value_block(
    ...
    match last_exp {
        ...
        Some(sp!(loc, _)) => {
            context.env.add_diag(ice!((loc, "ICE last sequence item should be an exp")));
            error_exp(loc)
        }
        None => {
            context.env.add_diag(ice!((seq_loc, "ICE empty sequence in value position")));
            error_exp(seq_loc)
        }
    }
}
...
fn process_value(context: &mut Context, sp!(loc, ev_): E::Value) -> H::Value {
    ...
    let v_ = match ev_ {
        EV::InferredNum(_) => {
            context.env.add_diag(ice!((loc, "ICE not expanded to value")));
            HV::U64(0)
        }
...
fn gen_unused_warnings(
    ...
    for (_, sname, sdef) in structs {
        context.env.add_warning_filter_scope(sdef.warning_filter.clone());
        ...
        if let H::StructFields::Defined(fields) = &sdef.fields {
            for (f, _) in fields {
                ...
                if !context.used_fields.get(sname).is_some_and(|names| names.contains(&f.value())) {
                    ...
                    context.env.add_diag(diag!(UnusedItem::StructField, (f.loc(), msg)));
                }
            }
        }

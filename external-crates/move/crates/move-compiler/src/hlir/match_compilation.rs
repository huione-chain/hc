...
pub(super) fn compile_match(
    ...
    let mut work_queue: Vec<(usize, Fringe, PatternMatrix)> = vec![(0, VecDeque::from([match_subject]), pattern_matrix)];
    ...
    while let Some((cur_id, init_fringe, matrix)) = work_queue.pop() {
        ...
        let redefined: Option<WorkResult> = match compile_match_head(context, init_fringe.clone(), matrix) {
            ...
            MatchStep::LiteralSwitch { subject, subject_binders, fringe, arms, default } => {
                ...
                let result =
                    WorkResult::LiteralSwitch { subject, subject_binders, arms: answer_map, default: default_work_id };
                compilation_results.insert(cur_id, result)
            }
            MatchStep::StructUnpack { subject, subject_binders, tyargs, unpack } => {
                ...
                compilation_results.insert(cur_id, WorkResult::StructUnpack { subject, subject_binders, tyargs, unpack })
            }

            MatchStep::VariantSwitch { subject, subject_binders, tyargs, arms, default: (dfringe, dmatrix) } => {
                let mut answer_map = BTreeMap::new();
                ...
                compilation_results.insert(cur_id, WorkResult::VariantSwitch {
                    subject,
                    subject_binders,
                    tyargs,
                    arms: answer_map,
        ...
        ice_assert!(context.env, redefined.is_none(), loc, "Match work queue went awry");
    }
...
fn compile_match_head(context: &mut Context, mut fringe: VecDeque<FringeEntry>, mut matrix: PatternMatrix) -> MatchStep {
    debug_print!(
    ...
    if matrix.is_empty() {
        ...
        let subject = fringe.pop_front().expect("ICE empty fringe in match compilation");
        ...
        for lit in lits {
            ...
            ice_assert!(context.env, arms.insert(lit, inner_matrix).is_none(), lit_loc, "Specialization failed");
        }
        ...
        if context.info.is_struct(&mident, &datatype_name) {
            ...
            for (ctor, (ploc, arg_types)) in ctors {
                ...
                ice_assert!(
                    context.env,
                    arms.insert(ctor, (fringe_binders, inner_fringe, inner_matrix)).is_none(),
                    ploc,
            ...
            MatchStep::VariantSwitch { subject, subject_binders, tyargs, arms, default: (fringe, default_matrix) }
        }
    }
}
...
#[growing_stack]
fn resolve_result(context: &mut ResolutionContext, init_subject: &FringeEntry, result: WorkResult) -> T::Exp {
    match result {
        WorkResult::Leaf(leaf) => make_leaf(context, init_subject, leaf),
        WorkResult::Failure => T::exp(context.output_type(), sp(context.arms_loc, T::UnannotatedExp_::UnresolvedError)),
        WorkResult::VariantSwitch { subject, subject_binders, tyargs, mut arms, default: default_ndx } => {
        ...
        WorkResult::LiteralSwitch { subject, subject_binders, mut arms, default: _ }
                ...
                && arms.len() == 2 =>
        {
            ...
            let true_arm_ndx = arms.remove(&sp(Loc::invalid(), Value_::Bool(true))).unwrap();
            let false_arm_ndx = arms.remove(&sp(Loc::invalid(), Value_::Bool(false))).unwrap();
        ...
        WorkResult::LiteralSwitch { subject, subject_binders, arms: map, default } => {
            // Bindings in the arm are always immutable
            let bindings =
                subject_binders.into_iter().map(|(_mut, binder)| (binder, (Mutability::Imm, subject.clone()))).collect();
            let lit_subject = make_match_lit(subject.clone());
...
fn make_leaf(context: &mut ResolutionContext, subject: &FringeEntry, mut leaf: Vec<ArmResult>) -> T::Exp {
    ...
    while let Some(arm) = leaf.pop() {
        ice_assert!(context.hlir_context.env, arm.guard.is_some(), arm.loc, "Expected a guard");
        ...
        match pat.pat.value {
            TP::Variant(m, e, v, tys, fs) => {
                let Some((queue_entries, unpack)) = arm_variant_unpack(context, None, ploc, m, e, tys, v, fs, entry)
                    ...
                    context.hlir_context.env.add_diag(ice!((ploc, "Did not build an arm unpack for a value variant")));
                    continue;
                };
            ...
            TP::Struct(m, s, tys, fs) => {
                let Some((queue_entries, unpack)) = arm_struct_unpack(context, None, ploc, m, s, tys, fs, entry) else {
                    context.hlir_context.env.add_diag(ice!((ploc, "Did not build an arm unpack for a value struct")));
                    continue;
                };
                for entry in queue_entries.into_iter().rev() {
                    queue.push_front(entry);
...
fn make_if_else(test: T::Exp, conseq: T::Exp, alt: T::Exp, result_ty: Type) -> T::Exp {
    ...
    T::exp(result_ty, sp(loc, T::UnannotatedExp_::IfElse(Box::new(test), Box::new(conseq), Box::new(alt))))
}

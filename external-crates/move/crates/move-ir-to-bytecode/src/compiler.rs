...
fn compile_function_body_impl(context: &mut Context, ast_function: Function_) -> Result<Option<CodeUnit>> {
    Ok(match ast_function.body {
        FunctionBody::Move { locals, code } => {
            ...
            Some(compile_function_body(context, m, ast_function.signature.formals, locals, code)?)
        }
        FunctionBody::Bytecode { locals, code } => {
            let m = type_parameter_indexes(ast_function.signature.type_formals.iter().map(|formal| &formal.0))?;
            Some(compile_function_body_bytecode(context, m, ast_function.signature.formals, locals, code)?)
        }
...
fn compile_constant(_context: &mut Context, ty: Type, value: MoveValue) -> Result<Constant> {
    ...
    Constant::serialize_constant(&type_layout(ty)?, &value).ok_or_else(|| format_err!("Could not serialize constant"))
}

...
fn has_precompiled_deps(pkg_path: &Path, pkg_dependencies: Arc<Mutex<BTreeMap<PathBuf, PrecompiledPkgDeps>>>) -> bool {
...
pub fn compute_symbols(compiled_pkg_info: CompiledPkgInfo, cursor_info: Option<(&PathBuf, Position)>) -> Symbols {
    let mut symbols_computation_data = SymbolsComputationData::new();
    let cursor_context = compute_symbols_pre_process(&mut symbols_computation_data, &compiled_pkg_info, cursor_info);
    let cursor_context =
        compute_symbols_parsed_program(&mut symbols_computation_data, &compiled_pkg_info, cursor_context);
...
pub fn get_symbols(
    ...
    let (compiled_pkg_info_opt, ide_diagnostics) = get_compiled_pkg(pkg_dependencies, ide_files_root, pkg_path, lint)?;
...
fn process_typed_modules<'a>(
    ...
    for (module_ident, module_def) in typed_modules.key_cloned_iter_mut() {
        ...
        file_use_defs.entry(fpath_buffer).or_default().extend_inner(use_defs.elements());
    }
}

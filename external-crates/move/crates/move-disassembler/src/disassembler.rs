...
impl DisassemblerOptions {
    pub fn new() -> Self {
        Self { only_externally_visible: false, print_code: true, print_basic_blocks: true, print_locals: true }
    }
}
...
impl<'a> Disassembler<'a> {
    ...
    fn get_function_string(&self, module_handle: &ModuleHandle, function_handle: &FunctionHandle) -> String {
        let module_id = self.source_mapper.bytecode.module_id_for_handle(module_handle);
        let function_name = self.source_mapper.bytecode.identifier_at(function_handle.name).to_string();
    ...
    fn get_import_string(&self, module_handle: &ModuleHandle) -> Option<String> {
        ...
        if self.is_self_id(&module_id) {
            ...
            Some(format!("use {}::{} as {};", module_id.address(), module_id.name(), alias))
        } else {
            Some(format!("use {}::{};", module_id.address(), module_id.name()))
        }
    }
    ...
    fn get_enum_def(&self, enum_definition_index: EnumDefinitionIndex) -> Result<&EnumDefinition> {
        ...
        Ok(self.source_mapper.bytecode.enum_def_at(enum_definition_index))
    }
    ...
    fn format_function_coverage(&self, name: &IdentStr, function_body: String) -> String {
        ...
        if self.is_function_called(name) { function_body.green() } else { function_body.red() }.to_string()
    }
    ...
    fn name_for_field(&self, field_idx: FieldHandleIndex) -> Result<String> {
        ...
        let field_def = match &struct_def.field_information {
            ...
            StructFieldInformation::Declared(fields) => {
                fields.get(field_handle.field as usize).ok_or_else(|| format_err!("Bad field index"))?
            }
        };
    ...
    fn name_for_parameter_or_local(&self, local_idx: usize, function_source_map: &FunctionSourceMap) -> Result<String> {
    ...
    fn type_for_local(
        ...
        let sig_tok =
            locals.0.get(local_idx).ok_or_else(|| format_err!("Unable to get type for local at index {}", local_idx))?;
        self.disassemble_sig_tok(sig_tok.clone(), &function_source_map.type_parameters)
    }
    ...
    fn format_function_body(locals: Vec<String>, bytecode: Vec<String>, jump_tables: Vec<String>) -> String {
    ...
    fn disassemble_sig_tok(&self, sig_tok: SignatureToken, type_param_context: &[SourceName]) -> Result<String> {
        Ok(match sig_tok {
        ...
        match instruction {
            Bytecode::LdConst(idx) => {
                let constant = constants
                    .get(idx.0 as usize)
                    .map(|(ty_str, val_str)| format!("{}: {}", ty_str, Self::preview_string(val_str)))
                    .unwrap_or_else(|| {
                        let constant = self.source_mapper.bytecode.constant_at(*idx);
                        format!("{:?}: {}", &constant.type_, Self::preview_const(&constant.data))
                    });
            ...
            Bytecode::Pack(struct_idx) => {
                let (name, ty_params) =
                    self.struct_type_info(*struct_idx, &Signature(vec![]), &function_source_map.type_parameters)?;
            ...
            Bytecode::PackGeneric(struct_idx) => {
                ...
                let (name, ty_params) =
                    self.struct_type_info(struct_inst.def, type_params, &function_source_map.type_parameters)?;
            ...
            Bytecode::MoveToGenericDeprecated(struct_idx) => {
                ...
                let (name, ty_params) =
                    self.struct_type_info(struct_inst.def, type_params, &function_source_map.type_parameters)?;
                Ok(format!("MoveToGeneric[{}]({}{})", struct_idx, name, ty_params))
            }
            Bytecode::Call(method_idx) => {
                ...
                let type_rets = self
                    ...
                    .map(|sig_tok| self.disassemble_sig_tok(sig_tok.clone(), &[]))
                    .collect::<Result<Vec<String>>>()?;
                Ok(format!("Call {}({}){}", fcall_name, type_arguments, Self::format_ret_type(&type_rets)))
            }
            Bytecode::CallGeneric(method_idx) => {
                ...
                Ok(format!(
                    "Call {}{}({}){}",
                    fcall_name,
                    Self::format_type_params(&ty_params.into_iter().map(|(s, _)| s).collect::<Vec<_>>()),
                    type_arguments,
    ...
    fn disassemble_jump_tables(&self, code: &CodeUnit) -> Result<Vec<String>> {
        ...
        for (i, jt) in code.jump_tables.iter().enumerate() {
            ...
            let jt: Vec<_> = jt
                ...
                .collect();
            jump_tables.push(format!("[{i}]:\tvariant_switch {} {{\n\t\t{}\n\t}}", enum_name, jt.join("\n\t\t")))
        }
        Ok(jump_tables)
    }
    ...
    fn disassemble_bytecode(
        ...
        let mut instrs: Vec<String> = instrs
            ...
            .map(|(instr_index, dis_instr)| {
                self.format_with_instruction_coverage(instr_index, function_code_coverage_map, dis_instr)
            })
            .collect();
        ...
        let ty_params: Vec<String> = source_map_ty_params
            ...
            .map(|((name, _), ty_param)| {
                let abilities_str = if ty_param.constraints == AbilitySet::EMPTY {
                    ...
                    let ability_vec: Vec<_> = ty_param.constraints.into_iter().map(Self::format_ability).collect();
                    format!(": {}", ability_vec.join(" + "))
                };
                format!("{}{}{}", if ty_param.is_phantom { "phantom " } else { "" }, name.as_str(), abilities_str)
            })
            .collect();
        Self::format_type_params(&ty_params)
    }

    fn disassemble_fun_type_formals(source_map_ty_params: &[SourceName], ablities: &[AbilitySet]) -> String {
        let ty_params: Vec<String> = source_map_ty_params
            .iter()
            .zip(ablities)
            .map(|((name, _), abs)| {
    ...
    pub fn disassemble_function_def(
        ...
        let entry_modifier = if function.map(|(f, _)| f.is_entry).unwrap_or(false) { "entry " } else { "" };
        ...
        let ty_params = Self::disassemble_fun_type_formals(&function_source_map.type_parameters, type_parameters);
        let params = &self
            .source_mapper
            .bytecode
            ...
            .map(|(tok, (name, _))| {
                Ok(format!("{}: {}", name, self.disassemble_sig_tok(tok.clone(), &function_source_map.type_parameters)?))
            })
            .collect::<Result<Vec<_>>>()?;
        ...
        let body = match code {
            Some(code) => {
                let locals = self.disassemble_locals(function_source_map, code.locals, params.len())?;
                let bytecode = self.disassemble_bytecode(function_source_map, name, parameters, constants, code)?;
    ...
    pub fn disassemble_struct_def(&self, struct_def_idx: StructDefinitionIndex) -> Result<String> {
        ...
        let abilities = if struct_handle.abilities == AbilitySet::EMPTY {
            ...
            let ability_vec: Vec<_> = struct_handle.abilities.into_iter().map(Self::format_ability).collect();
        ...
        let ty_params =
            Self::disassemble_datatype_type_formals(&struct_source_map.type_parameters, &struct_handle.type_parameters);
    ...
    pub fn disassemble_enum_def(&self, enum_def_idx: EnumDefinitionIndex) -> Result<String> {
        ...
        for variant in &enum_definition.variants {
            ...
            for field_definition in &variant.fields {
                let type_sig = &field_definition.signature;
                let field_name = self.source_mapper.bytecode.identifier_at(field_definition.name);
                let ty_str = self.disassemble_sig_tok(type_sig.0.clone(), &enum_source_map.type_parameters)?;
                variant_fields.push(format!("{}: {}", field_name, ty_str))
            }
            variants_formatted.push(format!("{} {{ {} }}", variant_name, variant_fields.join(", ")));
        }

        let abilities = if enum_handle.abilities == AbilitySet::EMPTY {
            String::new()
        } else {
            let ability_vec: Vec<_> = enum_handle.abilities.into_iter().map(Self::format_ability).collect();
            format!(" has {}", ability_vec.join(", "))
        };
        ...
        let ty_params =
            Self::disassemble_datatype_type_formals(&enum_source_map.type_parameters, &enum_handle.type_parameters);
        ...
        let function_defs: Vec<String> = (0..self.source_mapper.bytecode.function_defs.len())
            .map(|i| {
                ...
                self.disassemble_function_def(
                    self.source_mapper.source_map.get_function_source_map(function_definition_index)?,
                    Some((function_definition, function_handle)),
                    self.source_mapper.bytecode.identifier_at(function_handle.name),

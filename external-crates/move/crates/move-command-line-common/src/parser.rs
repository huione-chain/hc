...
mod tests {
    ...
    fn test_parse_type_negative() {
        for s in &[
            ...
            assert!(ParsedType::parse(s).is_err(), "Parsed type {s} but should have failed");
        }
    }
    ...
    fn test_parse_struct_negative() {
        for s in &[
            ...
            assert!(ParsedStructType::parse(s).is_err(), "Parsed type {s} but should have failed");
        }
    }
    ...
    fn test_parse_valid_struct_type() {
        ...
        for s in valid {
            assert!(ParsedStructType::parse(s).is_ok(), "Failed to parse struct {}", s);
        }
    }

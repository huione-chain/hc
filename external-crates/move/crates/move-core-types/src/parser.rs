...
fn token_as_name(tok: Token) -> Result<String> {
    ...
    Ok(match tok {
        ...
        Name(s) => s,
        Whitespace(_) | Address(_) | U8(_) | U16(_) | U32(_) | U64(_) | U128(_) | U256(_) | Bytes(_) | ColonColon
        | Lt | Gt | Comma | EOF => {
...
impl<I: Iterator<Item = Token>> Parser<I> {
        ...
        Ok(match self.next()? {
            ...
            Token::Address(addr) => {
                ...
                let ty_args = if self.peek() == Some(&Token::Lt) {
                    self.next()?;
                    let ty_args = self.parse_comma_list(|parser| parser.parse_type_tag(), Token::Gt, true)?;
                    self.consume(Token::Gt)?;
                    ty_args
                } else {
                    vec![]
    ...
    fn parse_transaction_argument(&mut self) -> Result<TransactionArgument> {
        Ok(match self.next()? {
            ...
            Token::Address(addr) => TransactionArgument::Address(AccountAddress::from_hex_literal(&addr)?),
...
{
    let mut tokens: Vec<_> = tokenize(s)?.into_iter().filter(|tok| !tok.is_whitespace()).collect();
...
pub fn parse_struct_tag(s: &str) -> Result<StructTag> {
    let type_tag =
        parse(s, |parser| parser.parse_type_tag()).map_err(|e| format_err!("invalid struct tag: {}, {}", s, e))?;
...
mod tests {
    ...
    fn tests_parse_transaction_argument_positive() {
        ...
        for (s, expected) in &[
            ...
            ("340282366920938463463374607431768211455u128", T::U128(340282366920938463463374607431768211455)),
            ("  0u16", T::U16(0)),
            ("0u16", T::U16(0)),
            ("532u16", T::U16(532)),
            ...
            ("1_000_000_000u256", T::U256(u256::U256::from(1_000_000_000u128))),
            (
                "3402823669209384634633746074317682114551234u256",
                T::U256(u256::U256::from_str("3402823669209384634633746074317682114551234").unwrap()),
            ),
            ("true", T::Bool(true)),
            ("false", T::Bool(false)),
            ("0x0", T::Address(AccountAddress::from_hex_literal("0x0").unwrap())),
            ("0x54afa3526", T::Address(AccountAddress::from_hex_literal("0x54afa3526").unwrap())),
            ("0X54afa3526", T::Address(AccountAddress::from_hex_literal("0x54afa3526").unwrap())),
    ...
    fn tests_parse_transaction_argument_negative() {
        ...
        for s in PARSE_VALUE_NEGATIVE_TEST_CASES {
            assert!(parse_transaction_argument(s).is_err(), "test case unexpectedly succeeded: {}", s)
        }
    }
    ...
    fn test_parse_valid_struct_tag() {
        ...
        for text in valid {
            ...
            assert_eq!(st.to_string().replace(' ', ""), text.replace(' ', ""), "text: {:?}, StructTag: {:?}", text, st);
        }
    }

    #[test]
    fn test_parse_struct_tag_with_type_names() {
        let names = vec!["address", "vector", "u128", "u256", "u64", "u32", "u16", "u8", "bool", "signer"];

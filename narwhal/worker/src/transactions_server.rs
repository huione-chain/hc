...
impl<V: TransactionValidator> TxServer<V> {
    ...
    pub fn spawn(
        ...
        spawn_logged_monitored_task!(
            Self { address, rx_shutdown, endpoint_metrics, local_client, validator }.run(),
            "TxServer"
        )
    }
    ...
    async fn run(mut self) {
        ...
        // create the handler
        let tx_handler = TxReceiverHandler { local_client: self.local_client.clone(), validator: self.validator };
...
impl<V: TransactionValidator> Transactions for TxReceiverHandler<V> {
    async fn submit_transaction(&self, request: Request<TransactionProto>) -> Result<Response<Empty>, Status> {
    ...
    async fn submit_transaction_stream(
        ...
        while let Some(Ok(request)) = transactions.next().await {
            ...
            if let Err(err) = self.validator.validate(txn.as_ref()) {
                ...
                return Err(Status::invalid_argument(format!("Stream contains an invalid transaction {err}")));
            }
            drop(validate_scope);
            // Send the transaction to Narwhal via the local client.

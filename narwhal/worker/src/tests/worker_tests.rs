...
use test_utils::{batch, latest_protocol_version, temp_dir, test_network, transaction, CommitteeFixture};
...
impl TransactionValidator for NilTxValidator {
    ...
    fn validate_batch(&self, _txs: &Batch, _protocol_config: &ProtocolConfig) -> Result<(), Self::Error> {
...
async fn reject_invalid_clients_transactions() {
    ...
    let tx = transaction();
    let txn = TransactionProto { transactions: vec![Bytes::from(tx.clone())] };
...
async fn handle_local_clients_transactions() {
    ...
    let join_handle = tokio::task::spawn(async move {
        ...
        for txn in batch.transactions() {
            ...
            fut_list.push_back(async move {
                inner_client.submit_transactions(vec![txn.clone()]).await.unwrap();
            });
        }
...
async fn get_network_peers_from_admin_server() {
    ...
    let (tx_new_certificates, _rx_new_certificates) = test_utils::test_new_certificates_channel!(CHANNEL_CAPACITY);
    ...
    let resp = reqwest::get(format!(
        ...
        worker_1_parameters.network_admin_server.worker_network_admin_server_base_port + worker_id as u16
    ))
    .await
    .unwrap()
    .json::<Vec<String>>()
    ...
    let (tx_new_certificates_2, _rx_new_certificates_2) = test_utils::test_new_certificates_channel!(CHANNEL_CAPACITY);
    ...
    let resp = reqwest::get(format!(
        "http://127.0.0.1:{}/known_peers",
        worker_2_parameters.network_admin_server.worker_network_admin_server_base_port + worker_id as u16
    ))
    .await
    .unwrap()
    .json::<Vec<String>>()

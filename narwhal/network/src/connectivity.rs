...
impl ConnectionMonitor {
    ...
    pub fn spawn(
        ...
        (
            spawn_logged_monitored_task!(
                Self { network, connection_metrics, peer_id_types, connection_statuses, rx_shutdown }.run(),
                "ConnectionMonitor"
            ),
    ...
    async fn run(mut self) {
        ...
        for (peer_id, ty) in &self.peer_id_types {
            ...
            self.connection_metrics.network_peer_connected.with_label_values(&[&format!("{peer_id}"), ty]).set(0)
        }
    ...
    async fn handle_peer_event(&self, peer_event: PeerEvent) {
        ...
        let peer_id_str =
            if self.peer_id_types.contains_key(&peer_id) { format!("{peer_id}") } else { "other_peer".to_string() };

        if let Some(ty) = self.peer_id_types.get(&peer_id) {
            self.connection_metrics.network_peer_connected.with_label_values(&[&peer_id_str, ty]).set(int_status);
        }
    ...
    fn update_quinn_metrics_for_peer(&self, peer_id: &str, stats: &ConnectionStats) {
        // Update PathStats
        self.connection_metrics.network_peer_rtt.with_label_values(&[peer_id]).set(stats.path.rtt.as_millis() as i64);
        self.connection_metrics
            .network_peer_lost_packets
            .with_label_values(&[peer_id])
            .set(stats.path.lost_packets as i64);
        self.connection_metrics.network_peer_lost_bytes.with_label_values(&[peer_id]).set(stats.path.lost_bytes as i64);
        self.connection_metrics
        ...
        self.connection_metrics.network_peer_congestion_window.with_label_values(&[peer_id]).set(stats.path.cwnd as i64);
...
mod tests {
    ...
    async fn test_connectivity() {
        ...
        let (_h, statuses) = ConnectionMonitor::spawn(network_1.downgrade(), metrics.clone(), peer_types, None);

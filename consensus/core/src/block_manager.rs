...
impl BlockManager {
    ...
    fn try_accept_one_block(&mut self, block: VerifiedBlock) -> TryAcceptResult {
        ...
        for (found, ancestor) in dag_state.contains_blocks(ancestors.to_vec()).into_iter().zip(ancestors.iter()) {
            if !found {
    ...
    fn try_unsuspend_children_blocks(&mut self, accepted_block: &VerifiedBlock) -> Vec<VerifiedBlock> {
        ...
        while let Some(block) = to_process_blocks.pop() {
            ...
            if let Some(block_refs_with_missing_deps) = self.missing_ancestors.remove(&block.reference()) {
...
mod tests {
    ...
    impl BlockVerifier for TestBlockVerifier {
        ...
        fn check_ancestors(&self, block: &VerifiedBlock, _ancestors: &[VerifiedBlock]) -> ConsensusResult<()> {

...
impl<C: NetworkClient> RoundProber<C> {
    ...
    pub(crate) async fn probe(&self) -> (Vec<QuorumRound>, Round) {
        ...
        let last_proposed_round = self.dag_state.read().get_last_block_for_authority(own_index).round();
        ...
        let quorum_rounds: Vec<_> = self
            ...
            .collect();
        for ((low, high), (_, authority)) in quorum_rounds.iter().zip(self.context.committee.authorities()) {
            node_metrics
        ...
        if let Err(e) = self.core_thread_dispatcher.set_propagation_delay(propagation_delay) {
            tracing::warn!("Failed to set propagation delay {propagation_delay} on Core: {:?}", e);
        }
...
mod test {
    ...
    impl FakeThreadDispatcher {
        fn new(highest_received_rounds: Vec<Round>) -> Self {
            Self { highest_received_rounds, propagation_delay: Mutex::new(0) }
        }
    ...
    impl NetworkClient for FakeNetworkClient {
        ...
        async fn get_latest_rounds(&self, peer: AuthorityIndex, _timeout: Duration) -> ConsensusResult<Vec<Round>> {
    ...
    async fn test_round_prober() {
        ...
        assert_eq!(quorum_rounds, vec![(100, 105), (0, 115), (103, 130), (0, 0), (105, 150), (106, 160), (107, 170)]);

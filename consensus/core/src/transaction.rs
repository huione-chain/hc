...
impl TransactionConsumer {
    pub(crate) fn new(tx_receiver: Receiver<TransactionsGuard>, context: Arc<Context>) -> Self {
        Self {
            tx_receiver,
            max_consumed_bytes_per_request: context.protocol_config.max_transactions_in_block_bytes(),
            max_consumed_transactions_per_request: context.protocol_config.max_num_transactions_in_block(),
    ...
    pub(crate) fn next(&mut self) -> (Vec<Transaction>, Box<dyn FnOnce(BlockRef)>) {
        ...
        let mut handle_txs = |t: TransactionsGuard| -> Option<TransactionsGuard> {
            ...
            if remaining_txs.is_empty() {
                ...
                Some(TransactionsGuard { transactions: remaining_txs, included_in_block_ack: t.included_in_block_ack })
            }
        };
...
impl TransactionClient {
    pub(crate) fn new(context: Arc<Context>) -> (Self, Receiver<TransactionsGuard>) {
        ...
        (Self { sender, max_transaction_size: context.protocol_config.max_transaction_size_bytes() }, receiver)
    }
    ...
    async fn submit_large_batch_and_ack() {
        ...
        // construct a over-size-limit batch and submit, which should get broken into smaller ones.
        {
            let transactions: Vec<_> = (10..32)
                ...
                .collect();
            let w = client.submit_no_wait(transactions).await.expect("Shouldn't submit successfully transaction");
            all_receivers.push(w);
        }

...
impl Core {
    pub(crate) fn new(
        ...
        let last_proposed_block = dag_state.read().get_last_block_for_authority(context.own_index);
    ...
    fn recover(mut self) -> Self {
        ...
        if wait_ms > 0 {
            ...
            println!("Waiting for {} ms while recovering ancestors from storage", wait_ms);
        ...
        info!("Core recovery completed with last proposed block {:?}", self.last_proposed_block);
    ...
    fn try_commit(&mut self) -> ConsensusResult<Vec<CommittedSubDag>> {
        ...
        if !self.context.protocol_config.mysticeti_leader_scoring_and_schedule() {
            ...
            if !committed_leaders.is_empty() {
                debug!("Committing leaders: {}", committed_leaders.iter().map(|b| b.reference().to_string()).join(","));
            }
    ...
    fn ancestors_to_propose(&mut self, clock_round: Round) -> Vec<VerifiedBlock> {
        ...
        let ancestors = iter::once(self.last_proposed_block.clone())
            .chain(ancestors.into_iter().filter(|block| block.author() != self.context.own_index).flat_map(|block| {
...
mod test {
    ...
    async fn test_core_set_min_propose_round() {
        ...
        let our_ancestor_included = block
            .ancestors()
            .iter()
            .find(|block_ref: &&BlockRef| block_ref.author == context.own_index && block_ref.round == GENESIS_ROUND);
    ...
    async fn test_no_leader_schedule_change() {
        ...
        let (mut context, _) = Context::new_for_test(4);
        context.protocol_config.set_mysticeti_leader_scoring_and_schedule_for_testing(false);
        // create the cores and their signals for all the authorities
        let mut cores = create_cores(context, vec![1, 1, 1, 1]);
        ...
        for core_fixture in cores {
            ...
            let all_stored_commits = core_fixture.store.scan_commits((0..=CommitIndex::MAX).into()).unwrap();
            assert_eq!(all_stored_commits.len(), 27);
            assert_eq!(core_fixture.core.leader_schedule.leader_swap_table.read().bad_nodes.len(), 0);
            assert_eq!(core_fixture.core.leader_schedule.leader_swap_table.read().good_nodes.len(), 0);

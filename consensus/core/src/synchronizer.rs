...
impl<C: NetworkClient, V: BlockVerifier, D: CoreThreadDispatcher> Synchronizer<C, V, D> {
    ...
    async fn fetch_blocks_from_authorities(
        ...
        let missing_blocks =
            missing_blocks.into_iter().take(MAX_PEERS * context.parameters.max_blocks_per_fetch).collect::<Vec<_>>();
...
mod tests {
    ...
    impl NetworkClient for MockNetworkClient {
        ...
        async fn get_latest_rounds(&self, _peer: AuthorityIndex, _timeout: Duration) -> ConsensusResult<Vec<Round>> {
            unimplemented!("Unimplemented")
        }
    }

...
impl CommittedSubDag {
    ...
    pub fn new(
        ...
        Self { leader, blocks, timestamp_ms, commit_ref, reputation_scores_desc }
    }
}
...
pub fn load_committed_subdag_from_store(
    ...
    CommittedSubDag::new(leader_block_ref, blocks, commit.timestamp_ms(), commit.reference(), reputation_scores_desc)
}
...
impl CommitInfo {
    ...
    pub(crate) fn new(committed_rounds: Vec<Round>, reputation_scores: ReputationScores) -> Self {
        CommitInfo { committed_rounds, reputation_scores }
    }
}

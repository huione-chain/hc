...
pub(crate) trait BlockVerifier: Send + Sync + 'static {
    ...
    fn check_ancestors(&self, block: &VerifiedBlock, ancestors: &[VerifiedBlock]) -> ConsensusResult<()>;
}
...
impl BlockVerifier for NoopBlockVerifier {
    ...
    fn check_ancestors(&self, _block: &VerifiedBlock, _ancestors: &[VerifiedBlock]) -> ConsensusResult<()> {
        Ok(())
    }
}
...
mod test {
    ...
    async fn test_check_ancestors() {
        ...
        let ancestor_refs = ancestor_blocks.iter().map(|block| block.reference()).collect::<Vec<_>>();
        ...
        // Block respecting timestamp invariant.
        {
            ...
            assert!(verifier.check_ancestors(&verified_block, &ancestor_blocks).is_ok());
        }

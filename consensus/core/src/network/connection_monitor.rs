...
impl AnemoConnectionMonitor {
    ...
    async fn run(mut self) {
        ...
        for (peer_id, hostname) in &self.known_peers {
            self.connection_metrics.network_peer_connected.with_label_values(&[&format!("{peer_id}"), hostname]).set(0)
        }
    ...
    async fn handle_peer_event(&self, peer_event: PeerEvent) {
        ...
        if self.known_peers.contains_key(&peer_id) {
            ...
            self.connection_metrics.network_peer_connected.with_label_values(&[&peer_id_str, hostname]).set(int_status);
    ...
    fn update_quinn_metrics_for_peer(&self, peer_id: &str, hostname: &str, stats: &ConnectionStats) {
        // Update PathStats
        self.connection_metrics
            .network_peer_rtt
...
mod tests {
    ...
    async fn test_connectivity() {
        ...
        assert_eq!(*handle.connection_statuses.get(&peer_2).unwrap().value(), ConnectionStatus::Connected);
        ...
        assert_eq!(*handle.connection_statuses.get(&peer_3).unwrap().value(), ConnectionStatus::Connected);
        ...
        // THEN
        assert_network_peers(&metrics, 1).await;
        assert_eq!(*handle.connection_statuses.get(&peer_2).unwrap().value(), ConnectionStatus::Disconnected);
        ...
        assert_eq!(*handle.connection_statuses.get(&peer_3).unwrap().value(), ConnectionStatus::Disconnected);
    }

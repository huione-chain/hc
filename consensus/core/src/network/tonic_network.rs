...
use crate::{
    ...
    network::{tonic_gen::consensus_service_server::ConsensusServiceServer, tonic_tls::create_rustls_server_config},
    CommitIndex,
    Round,
};
...
impl NetworkClient for TonicClient {
    ...
    async fn get_latest_rounds(&self, peer: AuthorityIndex, timeout: Duration) -> ConsensusResult<Vec<Round>> {
        let mut client = self.get_client(peer, timeout).await?;
...
impl<S: NetworkService> NetworkManager<S> for TonicManager {
    ...
    async fn install_service(&mut self, service: Arc<S>) {
        ...
        let tls_server_config = create_rustls_server_config(&self.context, self.network_keypair.clone());

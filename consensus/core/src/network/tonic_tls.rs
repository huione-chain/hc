...
pub(crate) fn create_rustls_server_config(context: &Context, network_keypair: NetworkKeyPair) -> ServerConfig {
...
pub(crate) fn create_rustls_client_config(
    ...
    let target_public_key = context.committee.authority(target).network_key.clone().into_inner();
    let self_signed_cert = sui_tls::SelfSignedCertificate::new(
        network_keypair.private_key().into_inner(),
        &certificate_server_name(context),
    );
    let tls_cert = self_signed_cert.rustls_certificate();
    let tls_private_key = self_signed_cert.rustls_private_key();
    let mut tls_config = sui_tls::ServerCertVerifier::new(target_public_key, certificate_server_name(context))
...
impl AllowedPublicKeys {
    fn new(context: &Context) -> Self {
        let keys = context.committee.authorities().map(|(_i, a)| a.network_key.clone().into_inner()).collect();
        Self { keys }
    }
}

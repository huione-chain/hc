...
mod tests {
    ...
    async fn test_leader_schedule_from_store_with_no_scores() {
        ...
        let blocks = vec![
            VerifiedBlock::new_for_test(TestBlock::new(10, 2).set_timestamp_ms(leader_timestamp).build()),
        ...
        let commit_info = CommitInfo { reputation_scores: ReputationScores::default(), committed_rounds };
        ...
        let leader_timestamp = context.clock.timestamp_utc_ms();
        let blocks = vec![
            VerifiedBlock::new_for_test(TestBlock::new(11, 3).set_timestamp_ms(leader_timestamp).build()),
        ...
        store.write(WriteBatch::default().blocks(blocks).commits(vec![last_commit])).unwrap();
    ...
    async fn test_leader_schedule_from_store() {
        ...
        let leaders = dag_builder.leader_blocks(1..=11).into_iter().flatten().collect::<Vec<_>>();
        ...
        // Check that DagState recovery from stored CommitInfo worked correctly
        assert_eq!(last_committed_rounds, dag_state.read().last_committed_rounds());
    ...
    async fn test_leader_schedule_from_store_no_commit_info() {
        ...
        let mut expected_commits = vec![];
        let leaders = dag_builder.leader_blocks(1..=2).into_iter().flatten().collect::<Vec<_>>();
        let mut blocks_to_write = vec![];

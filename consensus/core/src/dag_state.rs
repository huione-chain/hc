...
impl DagState {
    ...
    pub(crate) fn new(context: Arc<Context>, store: Arc<dyn Store>) -> Self {
        ...
        if let Some(last_commit) = last_commit.as_ref() {
            store
                ...
                .for_each(|commit| {
                    ...
                    if context.protocol_config.mysticeti_leader_scoring_and_schedule() {
                        let committed_subdag = load_committed_subdag_from_store(store.as_ref(), commit.clone(), vec![]); // We don't need to recover reputation scores for unscored_committed_subdags
        ...
        for (i, round) in last_committed_rounds.into_iter().enumerate() {
            ...
            let blocks = state
                .store
                .scan_blocks_by_author(authority_index, Self::eviction_round(round, cached_rounds) + 1)
                .unwrap();
            for block in blocks {
    ...
    pub(crate) fn get_cached_blocks(&self, authority: AuthorityIndex, start: Round) -> Vec<VerifiedBlock> {
        let mut blocks = vec![];
        for block_ref in
            self.recent_refs[authority].range((Included(BlockRef::new(start, authority, BlockDigest::MIN)), Unbounded))
        {
    ...
    pub(crate) fn flush(&mut self) {
        ...
        let commit_info_to_write = if self.context.protocol_config.mysticeti_leader_scoring_and_schedule() {
            std::mem::take(&mut self.commit_info_to_write)
        } else if commits.is_empty() {
            vec![]
        } else {
            let last_commit_ref = commits.last().as_ref().unwrap().reference();
            let commit_info = CommitInfo::new(self.last_committed_rounds.clone(), ReputationScores::default());
        ...
        for (authority_refs, last_committed_round) in self.recent_refs.iter_mut().zip(self.last_committed_rounds.iter())
        {
            while let Some(block_ref) = authority_refs.first() {
                if block_ref.round <= Self::eviction_round(*last_committed_round, self.cached_rounds) {
...
mod test {
    ...
    async fn test_flush_and_recovery_with_unscored_subdag() {
        ...
        let mut commits = vec![];
        let leaders = dag_builder.leader_blocks(1..=num_rounds).into_iter().flatten().collect::<Vec<_>>();

        let mut last_committed_rounds = vec![0; 4];
        for (idx, leader) in leaders.into_iter().enumerate() {
            let commit_index = idx as u32 + 1;
            let (subdag, commit) =
                dag_builder.get_sub_dag_and_commit(leader.clone(), last_committed_rounds.clone(), commit_index);
    ...
    async fn test_last_block_for_authority() {
        ...
        // WHEN no blocks exist then genesis should be returned
        {
            ...
            assert_eq!(dag_state.read().get_last_block_for_authority(context.own_index), my_genesis);
        }

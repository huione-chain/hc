    ...
    N: NetworkManager<AuthorityService<ChannelCoreThreadDispatcher>>,
{
    pub(crate) async fn start(
        ...
        let leader_schedule = if context.protocol_config.mysticeti_leader_scoring_and_schedule() {
    ...
    async fn test_amnesia_recovery_success(
        ...
        for (index, _authority_info) in committee.authorities() {
            ...
            let (authority, receiver) =
                make_authority(index, &dir, committee.clone(), keypairs.clone(), network_type, boot_counters[index])
                    .await;
        ...
        let (authority, mut receiver) =
            make_authority(index_1, &dir, committee.clone(), keypairs.clone(), network_type, boot_counters[index_1])
                .await;
        assert!(
